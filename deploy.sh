  ...
  for spec in "jeju|$TOUR_EXPORT_DIR|제주·도서" "west|$TOUR_WEST_EXPORT_DIR|서부권" "east|$TOUR_EAST_EXPORT_DIR|동부권" "capital|$TOUR_CAPITAL_EXPORT_DIR|수도권"; do
    ...
    if [[ "$force" == 1 || "${cnt:-0}" == 0 ]]; then
      if [[ -f "$dir/manifest.json" ]]; then
        step "여행로그 적재($label)"; pnpm --filter friendly load:tour "$dir" --dataset "$key"; did_tour=1
      else echo "  (여행로그 $label export 없음: $dir/manifest.json — 있으면 rsync 로 올린 뒤 'pnpm --filter friendly load:tour $dir --dataset $key')"; fi
    fi
  done
  if [[ "$did_tour" == 1 ]]; then
