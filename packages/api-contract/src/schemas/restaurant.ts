...
import { RestaurantPublicListTour, RestaurantTourSummary } from './tour.js';
import {
  BlogReview,
  CrawlLogLevel,
...
// 여행로그 장소 매칭 정보 — AI 허브 71780 여행로그(TourPlace)에 좌표 100m + 상호 유사도(완전일치 300m)로 붙인 장소
// (RestaurantTourMatch)와 그 장소의 집계·국세청 사업자 상태. 어드민 상세 헤더 배지용(2차). 공개 상세는 4차에서
// 별도 요약 스키마로 — 이 객체엔 개별 방문 행이 없다. docs/PLAN-tour-log.md
export const RestaurantTourMatchInfo = z.object({
  ...
  // 표본 안내 문구(@repo/utils TOUR_SAMPLE_LABEL).
...
// strategy: balanced(만족도·긍정·여행자 점수 중 있는 것의 평균), satisfaction(만족도만), positive(긍정만),
// traveler(여행로그 보정 만족도만 — 리뷰 분석이 없어도 여행자 통계가 있는 가게가 후보에 든다).
export const RestaurantSmartPickStrategy = z.enum(['balanced', 'satisfaction', 'positive', 'traveler']);
export type RestaurantSmartPickStrategyType = z.infer<typeof RestaurantSmartPickStrategy>;

export const RestaurantSmartPickInput = z.object({
  candidatePlaceIds: z.array(z.string()).optional(),
  strategy: RestaurantSmartPickStrategy.default('balanced'),
});
...
export const RestaurantSmartPickResult = z.object({
  ...
  picked: z
    .object({
      ...
      avgSentimentScore: z.number().nullable(),
      avgSatisfactionScore: z.number().nullable(),
      // 여행로그 보정 만족도(1~5) — 매칭 없거나 평가 3건 미만이면 null.
      avgTravelerScore: z.number().nullable(),
    })
    .nullable(),
  candidates: z.number().int(),
  strategy: RestaurantSmartPickStrategy,
});
...
export const RestaurantPublicListQuery = z.object({
  ...
  bbox: z
    ...
    .optional(),
  // tourTravelers/tourScore = 여행로그(AI 허브 71780) 매칭 장소의 방문자 수 / 보정 만족도 — 매칭 없는 행은 뒤로.
  sort: z.enum(['recent', 'satisfaction', 'positive', 'rating', 'tourTravelers', 'tourScore']).default('recent'),
...
export const RestaurantPublicListItem = z.object({
  ...
  negativeCount: z.number().int(),
  neutralCount: z.number().int(),
  // 여행로그 매칭 요약(방문자 수·보정 만족도·1인 지출·재방문율) — 매칭 없으면 null. 카드 메타·정렬용.
  tour: RestaurantPublicListTour.nullable(),
});
...
export const RestaurantPublicDetail = z.object({
  ...
  store: RestaurantStoreInfo.nullable(),
  // 여행로그(AI 허브 71780) 공개 요약 — 매칭 없으면 null. 상세는 GET .../tour-stats.
  tour: RestaurantTourSummary.nullable(),
});
