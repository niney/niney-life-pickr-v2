...
const busStorage = createInjectableStorage();
...
export const useBusFavoriteStore = create<BusFavoriteState>()(
  persist(
    ...
    }),
    {
      ...
      storage: createJSONStorage(() => busStorage.storage),
    },
  ),
);

// 주입은 이 모듈 평가 이후에 일어나므로(앱 entry), 그때 저장분을 다시 읽어온다.
busStorage.bindRehydrate(() => {
  void useBusFavoriteStore.persist.rehydrate();
});
