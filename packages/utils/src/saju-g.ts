...
export const SAJU_G_ELEMENTS = ['wood', 'fire', 'earth', 'metal', 'water'] as const;
export type SajuGElementId = (typeof SAJU_G_ELEMENTS)[number];
export const SAJU_G_ELEMENT_META: Record<
  SajuGElementId,
...
export const SAJU_G_STEMS = [
  ...
  hanja: string;
  ko: string;
  element: SajuGElementId;
  symbol: string;
  title: string;
  description: string;
}[];
export const SAJU_G_BRANCHES = [
...
export function sajuGTenGod(dayStem: string | null, targetStem: string): string | null {
  const a = SAJU_G_STEMS.findIndex((s) => s.hanja === dayStem);
  const b = SAJU_G_STEMS.findIndex((s) => s.hanja === targetStem);
...
export const sajuGPronunciation = (ganZhi: string): string =>
  `${SAJU_G_STEMS.find((s) => s.hanja === ganZhi[0])?.ko ?? ''}${SAJU_G_BRANCHES.find((s) => s.hanja === ganZhi[1])?.ko ?? ''}`;
export const SAJU_G_KIND_LABEL = {
  natal: '나의 사주(G)',
  annual: '올해의 흐름',
  daily: '오늘의 흐름',
} as const;
export const SAJU_G_PILLAR_LABEL = {
  year: '연주',
  month: '월주',
  day: '일주',
  hour: '시주',
...
export const SAJU_G_RELATIONSHIP_LABEL = {
  partner: '연인·파트너',
  friend: '친구',
  family: '가족',
  colleague: '함께 일하는 사이',
...
export function sajuGElementConnection(
  first: SajuGElementId | null,
  second: SajuGElementId | null,
) {
  if (!first || !second) return 'unknown' as const;
  const offset = (SAJU_G_ELEMENTS.indexOf(second) - SAJU_G_ELEMENTS.indexOf(first) + 5) % 5;
  return (
