...
| `canonical` | 출처 가로지르는 같은 가게 묶기 (Naver/캐치테이블/다이닝코드) — 후보 매칭·merge·split·자동 검토 큐 | `apps/friendly/src/modules/canonical/`, `apps/friendly/src/lib/matching.ts`, `packages/api-contract/src/schemas/canonical.ts`, `packages/shared/src/{api,hooks}/canonical.*`, `apps/web/src/components/restaurant/{CanonicalMergePanel,MergeProposalQueue}.tsx` |
