...
공유 주소 단축: 신규 조회 ID를 타로와 동일한 10자리 난수로 바꾸고 기존 32자리의 JSON·PNG·OG·취소 호환을 유지했다. 비밀키·영수증 길이는 유지하고 DB 고유 키 충돌에 대해 총 5회까지 저장을 시도한다. 실제 중복 저장, 소진 시 503, 다른 DB 오류의 단일 시도를 검증했다.

사용자 요청으로 공개 명칭·경로를 사주(G)·`saju-g`로 변경한 뒤 내부 코드·DB·설정·저장소까지 같은 이름으로 전환했다. 토픽도 `saju-g`로 옮기고 소스를 35개로 갱신했다. 기존 DB 이력은 보존하고 새 migration에서 데이터·설정·사용량을 이전하며, 기기 데이터는 첫 접근 때 옮긴다. 전체 위키 재컴파일이 아닌 해당 범위의 직접 갱신이다.

출생 프로필·궁합 추가: 소스 31개로 갱신했다. 게스트/회원 프로필 CRUD와 수정 버전, 기존 결과에서 프로필 생성, 나의 사주/오늘/올해 바로가기, 일간 오행·상호 십성의 결정적 궁합, Kimi v2 9/9 실측, 두 사람의 공유 PNG와 취소를 반영했다. 캐시된 프로필 바로가기의 StrictMode 취소 문제를 회귀 테스트로 수정했다. 해당 소스를 직접 읽은 범위 갱신이며 전체 위키 재컴파일은 아니다.
...
후속 실행 검증: 기본 DB의 사주·한도 마이그레이션 누락을 확인해 개발용 복사본에 적용했다. 실제 서버/브라우저 Kimi K3 응답 8건과 게스트·회원 보관/공유/취소 통과 기록을 saju-g 토픽·구현 명세에 추가했다. 원본 DB·환경 파일·운영 배포는 변경하지 않았다.
...
**New topics:** air-quality(199줄·87 sources), weather(242줄·74 sources), life-map(293줄·74 sources)
**Untouched topics:** canonical, config, telegram, vote (실변화 없음 확인 — cc8399a env 조립 한 줄만 해당하는 문서는 갱신에 포함)
**New concepts:** saved-location-glance, open-data-master-load, map-sheet-shell, principal-scoped-client-state
**Concepts updated:** db-config-env-fallback, dual-mount-shared-state, external-api-proxy-fixture, guest-server-hybrid, in-memory-singleton-gates, operation-log-instrumentation, platform-ui-split, quota-proportional-loading, sse-token-auth, versioned-llm-prompts, zod-ssot-buildless
**Sources scanned:** ~1969 (토픽별 sources_count 합, 중복 포함; 고유 source_locations 는 .compile-state.json 참조 — 23차 ~994 → 이번 토픽 문서 기준 재산정)
**Sources changed:** 417 파일 — 61 커밋 (22차 위키 커밋 616c000 이후 5e25cc0 ~ 4fd6e22; 이 중 food/meal 계열 ~30 커밋은 23차가 food.md/meal.md 에만 반영, 나머지 ~40 커밋과 food/meal 의 web/mobile/shared/friendly 측면은 이번 라운드가 처음 흡수)
**Notes:** 리베이스 풀 직후의 합류 라운드. 23차(2026-08-23)가 다른 머신에서 food/meal 만 신설하고 `.compile-state.json` 은 동기화되지 않아(gitignore 아님·커밋 누락), 이 머신의 상태는 22차(08-17)에 머물러 있었다 — 그래서 22차 이후 전 커밋을 다시 훑되 food/meal 은 23차 결과 위에 델타(0906df3·037a4f2·0064ab9)만 얹었다. **핵심 줄기 6개**: (1) **대기·날씨·일상지도 3도메인 신설** — 셋 다 data.go.kr 프록시 + `BUS_API_KEY` 폴백 체인 + probe→fixture 규율을 따르되 쿼터 규모(500/10,000/지오코딩 한도)가 캐시 형태를 갈랐다(전국 1콜 팬아웃 붕괴 / 발표 슬롯 TTL / 배포 시점 예산→gz 커밋). (2) **저장한 내 위치 1곳**(`airLocationStore`+`air_user_locations`, 서버 우선·1회 업로드)이 대기→날씨→일상지도→식단 추천으로 번져 결합점이 됐고, 웹 상단바 알약·앱 홈 카드가 공용 `useMyLocationGlance` 로 같은 값을 소비 — 신규 컨셉 `saved-location-glance`. (3) **지도 페이지 골격 통일** — 웹 e84e4b9(`sheet/BottomSheet`+`useMapSheets`, 맛집 v2→버스·지하철·일상지도)와 앱 4e414aa/fdb6ab9/342b3b7(플로팅 헤더 보간·시트 6개 `enableDynamicSizing=false`·활성 시트 추적)이 같은 날 같은 결정 — 신규 컨셉 `map-sheet-shell`; 일상지도만 CSS 이중 마운트 대신 `useIsDesktopXl` JS 분기(숨은 시트의 스크롤 락) → `dual-mount-shared-state` 에 경계 조건 추가. (4) **공공데이터 마스터 적재 수명주기** — `data/open` 원본 리포 밖·로더 기본 경로·전량 교체·`*MasterSync` 게이트·`status:*` 한 줄→deploy.sh 케이스 6/7 자동 적재·지오코딩 gz 만 커밋이 life-map·bus·subway·food 에 공통 — 신규 컨셉 `open-data-master-load`. (4-1) **클라이언트 정체성 경계** — 23차 meal 신설분(요청 토큰==현재 토큰일 때만 401 이 세션을 끊는 `handleUnauthorizedForCurrentSession`·로그인 시 `removeQueries()` 전체·principal 스코프 로컬 상태 4종·`expectedPrincipalId` 가드)이 shared·web·mobile·meal·friendly 에 걸쳐 있어 컨셉 `principal-scoped-client-state` 로 승격. (5) **앱 7도메인 확장 + 빌드 운영** — 날씨·대기·일상지도 Stack 화면, 홈 허브, `TransitMapView` 브리지 공용화(아이콘 사전·fixedScale, 마커당 ~60B), 식단 principal 네임스페이스 4종, Metro 수동 설정 삭제(Hermes SharedArrayBuffer), 무료 Apple 팀 entitlements 플러그인, prebuilt RN Release 아티팩트 링크 오류 문서(`docs/mobile-ios-build.md`, `pod install` 마다 재발). (6) **운영 사고 기록** — analytics 머지 테스트가 운영 스냅샷(5,446 그룹·22,303 링크)을 갈아엎어 LLM 1,352콜 재머지(517e465) → `useIsolatedDatabase` 필수 규칙; crawl 리뷰 더보기 오클릭·쿠폰 GraphQL 오인식·fetchedAt 정렬 3원인(0d72380) → `reviewDate.ts` 공용 비교자; meal-photo 기본 모델 gemma4:31b(60장 균등 실측 55/68%·2.4s vs qwen3.5 52/63%·4.9s). **위키 위생**: 23차가 INDEX/schema 표에만 적고 본문에 빠뜨린 food/meal 인스턴스 6개 컨셉 백필, meal.md/food.md 의 코드 불일치 5건(purpose 명·웹 draft 저장 오해·"웹"=Expo Web·confidenceBucket·hasActiveRun) 정정, 삭제·이동된 파일 링크 7건(`MenuRankingCard.tsx`·`BusFavoriteSection.tsx`·`web/lib/vworld.ts`·`timetableUtils.ts`·`crawl/job-log.service.ts`·앱 `share/settlements/[token].tsx`→`s/[token].tsx`·`canonical-members.js`→`.ts`) 정정, `source_count`→`sources_count` 키 통일, schema `mobile` 행 Expo 52/RN 0.76 → SDK 54/RN 0.81. **보류 컨셉 후보**(schema Evolution Log 참조): shared-account-key-chain · layered-cost-gates · visible-partial-completion · model-family-call-options · url-single-truth (isolated-test-db 는 friendly Gotcha, publication-slot-cache 는 quota 하위 변형, status-line-deploy-contract 는 open-data-master-load 에 흡수). 신규 3토픽 + 갱신 토픽을 병렬 서브에이전트 10개(+컨셉 백필 1, 후속 2)로 컴파일, 부모는 컨셉 4 신규·12 갱신(백필 6 포함) + 스키마/INDEX/log/state/CONTEXT 담당. **다음 라운드 후보**: 화장실 지오코딩 2일차 재실행 후 gz 재커밋(78.9% 잔여), 앱 지도 시트 공용 훅 추출 여부, `airLocationStore` → `userLocation*` 리네임, 앱 대기 출처 표기(제1유형 vs 웹 제3유형) 통일, 로컬 dev.db 병의원 0건.
...
**New topics:** review-search(리뷰 RAG/문맥검색 — bge-m3 enrich→BM25⊕dense RRF 하이브리드 회수→listwise LLM 리랭크→근거 RAG 생성→2차 LLM 검증 가드레일; 어드민+공개 무인증 QA; HyDE 기각·span-grounding 채택 research/probe 실측), review-clustering(UMAP→HDBSCAN→c-TF-IDF→LLM 라벨 군집화; Python 사이드카 cluster_compute.py; CLUSTERING_VERSION=4 canonical 통합 코퍼스; 전부 노이즈면 관점집계 폴백), random-crawl(cron 지역 랜덤 발굴→텔레그램 후보 push→사용자 선택만 크롤; 사람이 끼는 2단계 상태머신; scheduleRegistry 공유 jobType 'random-crawl'), telegram(long-polling 봇 /search·/discover·/stats + 크롤 알림 in-place 편집 + DB/env fallback 설정), logs(전 기능 횡단 operation-log OperationRun/Log/Report/LogConfig + 실패 run LLM 자동분석 + 보존 정리; crawl CrawlJobLog 일반화·레거시 백필)
**Concepts updated:** in-memory-singleton-gates(+review-search corpusCache/enriching/askRateHits, +review-clustering clustering Set, +random-crawl registry, +ai accountGate/ConcurrencyGate, +logs OperationLogService/세마포어, +analytics 청크 풀/mergeInflight, +telegram 폴러 — 연결 +5), zod-ssot-buildless(신규 스키마 5 + drink-kinds 단일 사전 — 연결 +5), sse-token-auth(+review-search enrich-events, +random-crawl run-events, +ai telemetryStream), stream-driven-cache-merge(+useReviewEnrichEvents, +useRandomCrawlRunEvents, +useLlmTelemetry, +crawl allocSeq 단일 발급), versioned-llm-prompts(+ENRICH_VERSION, +CLUSTERING_VERSION=4, MENU_GROUPING_VERSION 1→2, EXTRACTION_VERSION 3→4, +logs 프롬프트/스키마 미러링), public-admin-route-split(+review-search/clustering 어드민-restaurantId vs 공개-placeId 페어, +logs/telegram 어드민 전용 경계 사례), platform-ui-split(+ClusterTopics·RoundGroupSplitEditor web/native 동형, +비동기 질문 알림 UI 분기), workspace-package-resolution(+review-search/clustering api 경로 하드코딩 namespace 우회)
**New concepts:** db-config-env-fallback(ai/map/telegram 설정 동형 — 단일행 DB+env fallback+source+마스킹+PUT 보존; ai 다축 변형), operation-log-instrumentation(8+ 도메인 횡단 startRun→log→finishRun + parentRunId 중첩 + 실패 LLM 분석; crawl SSE seq 도 allocSeq 공유), canonical-corpus-fanout(리뷰 분석이 canonical 멤버 다소스 행 전체 fan-out + primaryId 통일 + corpusSize 게이트), cross-tab-async-job-toast(비동기 잡을 React 바깥 전역 store 로 — 컴포넌트 생사·탭 무관 + App 루트 watcher 토스트/배너; 서버 게이트의 클라 거울), ssr-lite-head-injection(SPA 라우트 백엔드 가로채 index.html head OG/SEO 주입 — 정산 share-preview + 맛집 restaurant-preview 동형; 16차 보류 후보 추출)
**Deferred candidates:** probe-validated-decision(research/probe-* 로 설계 결정 실측 채택/기각 + self-bias 회피 — review-search·review-clustering 강함, analytics/crawl/map probe 와 연결되나 방법론 성격이라 다음 라운드 재평가), human-in-the-loop-2단계-잡(random-crawl·auto-discover·telegram), single-chokepoint-instrumentation(ai onEvent — telemetry/operation-log 흡수)

**Sources scanned:** ~760 (기존 544 + 신규 ~216)
**Sources changed:** ~210 파일 — ~70 커밋 (17차 위키 커밋 `27cf3c1` 이후 2026-06-07 ~ 06-24)
**Notes:** 위키 역대 가장 큰 라운드(5 신규 도메인 + 전 스택 갱신). 토픽 17→22, 컨셉 8→13. 5 신규 토픽 + 13 갱신 토픽을 병렬 서브에이전트로 컴파일(부모는 utils/schedule/config + 컨셉/스키마/INDEX/state 담당). 핵심: (1) **리뷰 지능화** — review-search(임베딩=로컬 Ollama bge-m3, Cloud 엔 임베딩 없음; 생성/리랭크/검증=ollama-cloud chat; 요약 종료 훅에서 자동 enrich→클러스터링 체이닝) + review-clustering(Python 런타임 spawn, venv/CLUSTER_PYTHON_BIN, graceful skip). 둘 다 canonical 멤버 통합 코퍼스(커밋 3e1c90b). (2) **운영 자동화** — telegram 봇 + random-crawl 자동 발굴(사람이 끼는 2단계) + logs operation-log 횡단 계측(8+ 컨슈머, parentRunId 중첩, 실패 LLM 분석 세마포어). (3) **ai** — 계정 단위 2단 동시성 게이트(어댑터 캐시와 독립 수명) + 단일 chokepoint onEvent 텔레메트리 SSE + 키 1계정 공유·용도별(chat/image/log-analysis) 모델. (4) **crawl** — 테이블링 무인증 REST 4어댑터(search/shop/place/sitemap) + SSE seq allocSeq 단일화(78% 멈춤 fix) + 정확한 방문자 리뷰 수. (5) **settlement** — 세부 분배 그룹 균등/잔수(GLASSES) + drink-kinds 단일 사전(FE제안·BE보정·프롬프트힌트) + EXTRACTION_VERSION 3→4(주류 오분류 프롬프트+matchDrinkKind 이중 안전망). (6) **analytics/menu-grouping** — 전량 머지 청크 병렬 풀+sha256 캐시(GlobalMenuMergeChunkCache, 20260619075115 마이그레이션 정식 등재 — 과거 수동생성 운영 DB 는 _prisma_migrations 정합 확인)+Link insert 배치 스트리밍 OOM fix; menu-grouping v2 분할·머지 재설계(에코→인덱스 계약, parse_failed 구조적 제거, canonical 은 코드 pickCanonicalName). (7) **맛집 공유/SEO** — /r/:placeId + sitemap + JSON-LD SSR-lite(restaurant-preview), 파노라마 503/TTL 영구 사본 캐시. (8) **지역 통계** — region-derive(주소/좌표→시군구) + RegionStatsMap choropleth(sigungu-geo.json point-in-polygon) + 텔레그램 /stats.
