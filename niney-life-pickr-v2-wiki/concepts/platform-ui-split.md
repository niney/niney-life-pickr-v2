...
이 패턴이 깨질 수 있는 위험:
- 번들러가 확장자 우선순위를 다르게 해석할 때 — 예: shared의 Button이 현재 `Button.tsx`에서 `.web.tsx`를 직접 re-export 중. 누가 무심코 `Button.tsx`에 web 구현을 넣으면 Metro가 그걸 native에서도 쓰게 됨. 항상 quad 패턴 유지 필요
- React 버전 불일치 (web R19, mobile R18) — 현재 `@repo/shared`가 React 18+ peer로 양쪽 호환. shared 코드가 R19-only API(예: 새 `use()` 훅)를 쓰는 순간 mobile이 깨진다
