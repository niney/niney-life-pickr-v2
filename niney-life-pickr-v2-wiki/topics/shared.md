...
- **`settlement-draft.api.ts` / `useSettlementDraft.ts` (신규)** — 정산 입력의 서버 임시저장(자동
  ...
  - **`useSettlementDraftHydrate(placeId)`** — 진입 시 한 번 서버 list 를 받아 placeId 매치되는
...
- `settlementApi`: `create(input: CreateSettlementInputType)` (**input 에 `fromDraftId?: string` 옵션 필드** — 임시저장 draft 에서 출발한 저장 시 서버 트랜잭션이 그 draft 도 함께 삭제, mismatch 면 silent ignore), `list(query?)`, `get(id)`, `remove(id)`, **`update(id, input: UpdateSettlementInputType)` (전체 replace — 참여자/차수/items/attendees 모두 교체)**, `createShare(id)` (멱등 — 같은 세션 여러 번 호출 시 동일 토큰), `revokeShare(id)`, `getShared(token)` (공개 read-only, 비로그인 가능).
...
- **정산 임시저장 (신규)**:
  ...
  - **`useSettlementDraftHydrate(placeId: string | null): { hydrated: boolean; matched: SettlementDraftType | null }`** — list 한 번 fetch, placeId 매치되는 draft 가 있으면 `useSettlementDraftStore.setState` 로 store 의 `participants` / `rounds` overwrite. 비로그인이거나 list 가 실패하면 즉시 `hydrated = true` 로 진행(sessionStorage-only 모드).
...
- **`useSettlementDraftHydrate` 가 하이드레이트 신호** — 진입 시 서버 draft 한 번만 fetch 후 store
  overwrite 하고 `hydrated = true` 신호. 그 다음에야 자동저장이 활성화 — hydrate 가 끝나기 전에
  저장이 일어나면 빈 store 가 서버 draft 를 덮는 사고. 비로그인이거나 list 가 실패하면 즉시
  hydrated 로 처리해 sessionStorage-only 모드로 진행(서버 미사용 fallback).
...
- [packages/shared/src/hooks/useSettlementDraft.ts](../../packages/shared/src/hooks/useSettlementDraft.ts) (NEW)
