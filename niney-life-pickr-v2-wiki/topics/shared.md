...
## 1. Purpose [coverage: high — 4 sources]
...
- 타입 안전한 fetch 래퍼와 도메인별 API 함수 (auth, picks, admin, crawl, restaurant,
  menu-grouping, analytics, ai)
- TanStack Query 훅 (서버 상태) — 메뉴 그룹핑/전역 머지 잡 SSE 훅 포함
...
## 2. Architecture [coverage: high — 14 sources]
...
## 4. API Surface [coverage: high — 20 sources]
...
- `restaurantApi`: `list`, `getByPlaceId(placeId)`, `getSummaryStatus(placeId)`, `delete(placeId)` + `buildSummaryEventsUrl(placeIds: string[])` (string 배열 → `?placeId=A&placeId=B&...&token=<jwt>`)
...
## 5. Data [coverage: high — 8 sources]
...
## 6. Key Decisions [coverage: high — 12 sources]
...
- **SSE 토큰은 쿼리스트링** — `EventSource`가 커스텀 헤더를 보낼 수 없어 `?token=` 방식으로 우회 (`buildJobEventsUrl`, `buildSummaryEventsUrl`, `buildGroupingJobEventsUrl`, `buildGlobalMergeJobEventsUrl`). 신규 두 빌더는 기존 sse-token-auth 컨셉의 새 인스턴스로, 백엔드 Routes 와 1:1 매핑된다.
- **중복 이벤트 방어** — SSE 재연결 시 서버가 `Last-Event-ID`로 리플레이하면 reducer가 `seq <= lastSeq`인 이벤트를 무시.

## 7. Gotchas [coverage: high — 12 sources]
...
- **`useMenuRanking` / `useGroupingJob` 의 404 처리** — `ApiError.statusCode === 404` 면 `null` 을 반환해 React Query 에 정상 데이터로 흘림. throw 하지 않음. UI 는 `data === null` 을 "아직 그룹핑 없음" 상태로 분기해야 함.
...
- **`useGlobalMenus` 쿼리키 8-튜플** — q/category/sort/minMentions/limit/includeUnlinked 의 기본값을 캐시키 안에서 직접 채우므로(`?? ''`, `?? 'mentions'`, ...) 호출부가 query 객체를 매번 새로 만들어도 안전하지만, 기본값을 바꾸면 캐시키 모양이 변해 기존 캐시가 즉시 cold 가 된다.
...
- **`invalidateQueries` 키 매칭** — AI provider mutation은 `['ai', 'providers']`만 무효화한다. 메뉴 그룹핑/분석 invalidate 도 prefix 매치라 키 모양을 바꾸면 done 핸들러도 같이 맞춰야 함.
...
## 8. Sources [coverage: high — 35 sources]
