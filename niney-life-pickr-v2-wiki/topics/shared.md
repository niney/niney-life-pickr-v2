...
sources_count: 37
status: active
aliases: [react-query, zustand, design-tokens, ui-primitives, "@repo/shared", useNaverSearch, "crawlApi.search", naver-search-hook]
---
...
│   ├── picks.api.ts
│   ├── admin.api.ts
│   ├── crawl.api.ts        # start/list/cancel/search + SSE 엔드포인트 URL 빌더
...
│   ├── useAdmin.ts
│   ├── useCrawl.ts         # useStartCrawl/useCrawlJobs/useCancelCrawl/useNaverSearch + useCrawlJobStream (EventSource reducer + persistedCount/lastBatch)
...
- 컨슈머:
  - `apps/web` — 어드민 콘솔(맛집/메뉴/분석/AI/지도 설정 + **발견 페이지** `AdminDiscoverPage`),
    공개 맛집 페이지(랭킹/지도/식당 상세 — 비로그인 허용), 로그인/회원가입 페이지. 동일한
    `@repo/shared` 인스턴스 위에서 `useRestaurantList`(어드민)와 `useRestaurantsPublic`(공개)이
    한 SPA 안에 공존. 어드민 발견 페이지가 `useNaverSearch` 호출자.
...
- 외부:
  ...
  - `useNaverSearch`는 friendly의 `/api/v1/admin/crawl/search` (= `Routes.Crawl.search`) 에
    GET 으로 접속. 응답은 `{ items: NaverSearchResult[], source: 'playwright' }` 모양.
...
- `crawlApi`: `start`, `list`, `cancel`, **`search({ q, bbox? })` (신규)** — `Routes.Crawl.search` 에 `URLSearchParams` 로 `q` + 선택 `bbox` 직렬화. 응답 `CrawlSearchResultType` (`{ items: NaverSearchResult[], source: 'playwright' }`). + `buildJobEventsUrl(jobId)` (SSE URL — `?token=` 인증)
- `restaurantApi`:
...
- **네이버 검색 (신규)**: `useNaverSearch(q: string, bbox: string | null)` — 네이버 PC 지도 검색.
  `enabled: q.trim().length > 0` 으로 빈 q 자동 비활성, `staleTime: 30_000` 로 같은 키워드
  토글 시 즉시 캐시 hit. 디바운스 책임은 호출자(어드민 발견 페이지가 URL `q` 갱신 단위로 디바운스).
...
- 크롤: `useCrawlJobs` → `['crawl', 'jobs']`. **`useNaverSearch(q, bbox)` (신규)** → `['crawl', 'search', q, bbox]` — `staleTime: 30_000`. queryKey 가 `q` 자체라 호출자(어드민 발견 페이지)가 URL `q` 갱신을 디바운스하면 그 단위로 새 캐시 entry 가 생성된다. 빈 q 면 `enabled` false 로 fetch 안 함.
...
- **`useNaverSearch` 의 디바운스 책임은 호출자** — 훅 자체는 디바운스를 하지 않는다. queryKey 가 `['crawl', 'search', q, bbox]` 로 `q` 자체에 묶여 있어 호출자가 매 키스트로크마다 q 를 갱신하면 그대로 새 fetch 가 폭주한다. 그래서 어드민 발견 페이지가 URL 동기화 단위(300ms 디바운스 + Enter 즉시 flush)로 `q` 를 업데이트하고 그 변화만 훅에 흘려보낸다. 훅 측은 `staleTime: 30_000` 으로 같은 키워드 빠른 토글에 캐시 hit 만 보장. 디바운스를 훅 안에 박지 않은 이유는 페이지가 URL 과 패널 표시 상태도 같이 디바운스해야 하기 때문 — 두 곳에서 따로 디바운스하면 어긋난다.
...
- **`useNaverSearch` 의 빈 q 자동 disabled + staleTime 30s** — `enabled: q.trim().length > 0` 이라 패널이 처음 mount 되는 시점(초기 q 가 빈 문자열) 에서는 fetch 가 발생하지 않는다. 사용자가 검색어를 비우면 조용히 비활성으로 전환되고 마지막 결과는 그대로 캐시에 남는다. 또 `staleTime: 30_000` 라 같은 키워드로 패널을 빠르게 껐다 켜도 새 요청 없이 캐시 hit. 단 호출자가 `q` 자체를 디바운스하지 않으면 키스트로크 단위 새 캐시 entry 가 폭증하니, 디바운스는 페이지 레이어에서 반드시 적용해야 한다.
