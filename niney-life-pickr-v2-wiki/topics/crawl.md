...
`apps/friendly/src/modules/crawl/`에 위치한 어드민 전용 크롤러. 사용자가 붙여넣은 네이버 지도/Place URL을 받아 Playwright로 모바일 페이지를 띄우고, SSR 인라인 `__APOLLO_STATE__`와 GraphQL wire 응답에서 식당 메타데이터·메뉴·블로그/방문자 리뷰를 추출해 구조화된 JSON으로 돌려주면서 동시에 DB(Restaurant + VisitorReview)에 영속화한다. 진행 상황은 SSE로 스트리밍되며, 작업은 메모리 상의 Job registry에서 actor당 FIFO 큐로 관리된다.

## Purpose [coverage: high — 5 sources]

가게를 골라주는 서비스의 데이터 인입 통로. 어드민이 네이버 Place URL을 한 줄 붙여넣으면 식당 한 곳의 모든 사실 데이터(이름·주소·영업시간·메뉴·평점·리뷰·리뷰 사진/동영상)를 한 번에 긁어 와 후속 의사결정 모델이 쓸 수 있게 정규화하고 DB에 적재한다. 적재된 방문자 리뷰는 곧장 AI 요약 큐로 흘러간다.
...
6. **GraphQL request rewrite** — `page.route('**/*')`에서 `api.place.naver.com/graphql`(+ pcmap/m.place) POST 요청을 가로채 `forceRecentSort()`로 variables 안의 `sort` 필드를 `'recent'`로 강제. URL 쿼리 `?reviewSort=recent`는 SPA 페이저가 무시하기 때문에 wire body 재작성이 더 견고.
7. **Persist tail** (service 내부) — `runJob`은 `persistTail: Promise<void>` 체인 하나를 들고 있다. `onPartial`은 `restaurants.upsertRestaurantFromCrawl(partial)`을 tail에 append해 `restaurantId`를 잡고, `onVisitorBatch`는 `restaurants.persistReviewBatch(restaurantId, batch)`를 tail에 append. 어댑터는 절대 블로킹하지 않으면서 한 Job 내에서는 batch가 직렬화돼 같은 식당의 dedup snapshot이 race 하지 않는다. `persistReviewBatch`가 반환한 `{newReviews}`로 `summary.queueSummariesForReviews()` 트리거 + `visitor_batch` SSE 이벤트(`addedCount`, `persistedReviews[]`) 발행. 마지막에 식당 스냅샷만 한 번 더 upsert (예전 같은 final `persistBatch`는 제거됨 — SSR 초기 24건이 이미 첫 batch로 통과했으므로 중복 호출 불필요). tail이 비워질 때까지 await한 뒤 `done` 이벤트.
8. **Job registry** (`job-registry.ts`) — 메모리 `Map<jobId, InternalJob>`. 내부 `phase: 'queued' | 'active' | 'finished'` 라이프사이클(공개 `status`는 기존 4-값 유지). `addEvent()`로 이벤트를 누적·subscriber에게 fan-out 하고, denormalized 필드(`stage`/`visitorCount`/`status`/`result`)를 갱신. 종료 후 5분 TTL로 GC.
9. **Queue 디스패치** — `runJob().finally(() => flushQueue(actorId))`가 슬롯이 비는 즉시 `pending`에서 같은 actor의 다음 Job을 꺼내 `markActive` + `runJob` 시작. `hasSlotForActor`가 false인 동안에는 새 Job이 enqueue만 되고 `progress: stage='queued'` 이벤트로 SSE 구독자에게 대기 상태를 즉시 알린다.
...
- `visitor_batch` — `{ reviews, addedCount, persistedReviews }` (한 페이지 DB 영속화 직후. SSR 초기 batch도 포함. `addedCount`는 dedup 후 INSERT된 개수, `persistedReviews`는 서버 id·`videos[]`·`fetchedAt` 포함한 실제 DB row들)
...
- VisitorReview N건 — 페이지마다 `persistReviewBatch`로 dedup 후 신규분만 INSERT. 각 row에 `imageUrls`(JSON), `videosJson`(`[{posterUrl, videoUrl}]` 직렬화) 컬럼이 함께 저장된다. `recrawl` 모드는 사전에 기존 리뷰를 모두 삭제. `update` 모드는 어댑터가 `existingReviewKeys`로 조기 종료해 신규분만 들어옴.
...
- **방문자 리뷰는 "최신순" 강제** — URL `?reviewSort=recent`만으로는 SPA 페이저가 자체 변수(default=recommend)를 만들어 무시하므로, `page.route()`에서 graphql POST body의 `variables.sort`를 모두 `'recent'`로 재작성(`forceRecentSort`). 셀렉터 의존 없는 견고한 방식.
- **SSR 인라인 = 첫 batch** — 방문자 서브페이지의 document HTML 안 `<script>__APOLLO_STATE__=...</script>`를 정규식으로 잡아 즉시 파싱하고 초기 ~24건을 `onVisitorBatch`로 흘려, "fetch=save" 일관성 확보. 이전 코드처럼 끝에서 한 번 더 `persistBatch(data.visitorReviews)`로 보강할 필요 없음(중복 호출 제거됨).
...
- **`visitor_batch.persistedReviews`로 follow-up GET 제거** — 서버가 INSERT된 row들을 그대로 SSE에 실어 보내 클라가 detail 캐시에 직접 머지. 한 크롤당 N개의 round-trip 절약. `videos`도 함께 실린다.
...
- **`isReviewTypename` 정규식 함정 (해결됨)** — 예전에는 `/Review/i` 한 줄로 review 노드를 식별했는데, 이게 `VisitorReviewMedia` 같은 서브엔티티에도 매치돼 미디어 배열 진입을 즉시 return시켜 리뷰 사진이 fallback 1장만 수집되던 버그가 있었다. 현재는 정확한 화이트리스트 `Set(['VisitorReview', 'BlogReview', 'FsasReview', 'BlogPost'])`로 좁혀 다중 이미지가 정상 수집된다 (commit `d9b331a`).
- **VisitorReviewMedia.type === 'video'는 imageUrls에서 분리** — `collectImagesFromContainer`는 video 타입을 명시적으로 skip. 동영상은 `collectVisitorReviewVideos`가 별도로 walk해 `videos[{posterUrl: thumbnail JPEG, videoUrl: trailerUrl 서명 mp4}]`로 추출(commit `05e12e2`). videoUrl은 akamaized.net 서명 URL이라 단명 — FE가 직접 재생, 서버 프록시 없음.
...
- **`reviewSort=recent` URL 만으로는 부족** — graphql request body 재작성(`forceRecentSort`)이 같이 가야 실제로 최신순 응답이 옴.
...
- **Apollo state ref dereferencing** — `__ref` 형태의 캐시 포인터를 `deref(state, ...)`로 풀어 가며 walk. 잘못 풀면 Menu 이미지에 BlogPost URL이 끼는 식의 cross-typed 오염이 생긴다 (그래서 `isReviewTypename` 가드가 있다).
