...
**2026-06-01 변경 흡수 (16차, perf/UX 라운드 — 렌더 코어 무변경)** — 이 라운드는 지도 도메인의 렌더링 코어(OpenLayers Map 인스턴스, 마커 빌더, 줌 임계값, declutter 정책)에 **변경 없음**. 두 파일이 터치됐지만 모두 위치 권한 UX 다듬기였다: 앱·웹 "내 위치" 버튼이 `denied`/`unavailable` 에서 비활성 대신 클릭 가능해지고 silent refetch + 설정 안내를 건다(상세는 [Gotchas](#gotchas-coverage-high----26-sources)). 17차에서 웹 측 그 로직이 `MyLocationButton.tsx` 로 추출됐다.
...
지도는 베이스맵 레이어를 일반/다크/위성 3종으로 토글할 수 있고(좌하단 `MapLayerControl`), 초기값은 **앱 테마**(라이트→일반, 다크→야간)를 따른다. 앱(WebView) 측도 같은 전략을 쓴다 — vworld 가 제공하는 실제 `midnight` 다크 타일이라 CSS invert 같은 트릭이 없다. 자세한 건 [Key Decisions](#key-decisions-coverage-high----26-sources)·[platform-ui-split](../concepts/platform-ui-split.md).
...
| `HousingMapView` (2026-08-30, `254fb76`) | 공개 `/housing` ([HousingMapView](../../apps/web/src/components/housing/HousingMapView.tsx)) | 단지 가격 알약 배지(거래 없는 단지는 회색 점) / 저줌은 평당가 셀 (한 소스) | 미사용 — `icon` + **전부 `fixedScale`**(글자를 SVG 에 새겨 라벨 미사용, [housingMarker.ts](../../packages/utils/src/housingMarker.ts)·`markerFrame.buildPinMarkerSvg`) | 미사용 | 노출 | 사용(우하단 MyLocationButton, `--map-bottom-inset`, `poolKey='housing'` 단일 마운트; 내 위치·저장 위치는 `overlayMarkers`; 목록 선택 `flyTo` 에 `sheetHalfInset` 적용 — 일상지도 외 두 번째 사용처). 렌더 코어 무변경, 도메인은 [housing](housing.md). **(2026-09-24 `ad48f96`)** 알약 배지 왼쪽에 침수 흔적 물방울 글리프(아래 마커 표) |
| `ParkingMapView` (2026-09-25, `2ff2c31`) | 공개 `/parking` ([ParkingPage](../../apps/web/src/routes/ParkingPage.tsx)) | 지금 탭 한 소스 — 주차장 'P'·충전소 번개 원/핀, 공항 비행기(14곳, 이름 라벨 상시) / 저줌(주차장 <13·충전소 <15)은 숫자 버블 셀 | 미사용 — `icon`(단계색 아이콘을 모듈 상수로 1회 생성) + 셀만 `fixedScale` | 미사용 | 노출 | 사용(우하단 MyLocationButton + `--map-bottom-inset`, `poolKey='parking'` 단일 마운트; 내 위치·저장 위치는 `overlayMarkers`; 목록·공항·URL `sel` 이동에 `sheetHalfInset` — 세 번째 사용처). 도메인은 [parking](parking.md) |
| `SeaMap` (2026-09-24, `4a2bff1`) | 공개 `/sea` ([SeaPage](../../apps/web/src/routes/SeaPage.tsx) 카드형 그리드, 지도 360px / `lg` 600px) | 생활해양예보 지점(활동별) — 파도 아이콘 원/핀, 라벨은 선택 지점 + 순위 상위 3곳 중 단계 4 이상만 | 미사용 — `icon`(단계 `none`·0~5 × 선택 = 14 data URL 모듈 상수) | 미사용 | 노출 | 오버레이 점만(버튼은 페이지 헤더 '내 위치로 거리 보기', `poolKey='sea'` 단일 마운트, 시트 없음 → `bottomInset` 미사용). 도메인은 [sea](sea.md) |

## Architecture [coverage: high — 25 sources]

저레벨 캔버스 한 개와 세 개의 화면별 wrapper 로 구성되고, 앱(WebView/iframe) 측에 동형 코어가 한 벌 더 있다. **(2026-06)** 여기에 `MapCanvas` 를 거치지 않고 타일/테마 헬퍼만 공유하는 어드민 통계용 `RegionStatsMap` 한 개가 더 붙는다. **(2026-08)** 공개 지도 wrapper 가 둘 늘었고(`AirStationsMap`·`LifeMapView`), 앱에는 맛집 전용 코어와 별개로 대중교통이 만든 범용 WebView 코어(`TransitMapView`)가 대기·일상지도 화면에 재사용된다 — 앱 지도 코어는 이제 두 벌이다. **(2026-09)** 웹 wrapper 가 둘 더(`ParkingMapView`·`SeaMap`, 아래 10·11번) — 둘 다 웹 전용(앱 주차·바다 화면 없음)이라 앱 코어는 그대로다.
...
10. **`ParkingMapView`** ([apps/web/src/components/parking/ParkingMapView.tsx](../../apps/web/src/components/parking/ParkingMapView.tsx)) — **(2026-09-25 신규, `2ff2c31`)** `/parking` 지도. 키 게이트(로딩/404 미등록)는 집값·일상지도와 같은 `Placeholder` 분기. 페이지가 만든 지금 탭의 마커(한 소스)를 `MapCanvas`(`poolKey="parking"`)에 넘기고, 오버레이는 `saved-location`(보라 점, 라벨 '내 위치' — `useAirLocation` 의 저장한 내 위치, 날씨·대기·일상지도와 공유)·`my-location`(파란 점 `buildMyLocationMarkerDataUrl`). 클릭은 [parkingMarkers.ts](../../apps/web/src/components/parking/parkingMarkers.ts) `parseParkingMarkerId` 로 `{ kind: 'lot'|'ev'|'airport', id }` / `{ kind: 'cell', layer, index }` 를 만들어 페이지로(셀은 응답 `cells` 배열 index 역참조 `parkingCellAt` — 일상지도 `lifeCellAt` 과 같은 규약). 상단 중앙 슬롯 = 로딩 칩 > 힌트(셀 모드면 "13 이상 확대하면 주차장이 하나씩 보입니다 (지금 N)"·충전소 15, `truncated` 면 "많아서 일부만 표시 중 — 더 확대해 주세요"), 우하단 `MyLocationButton`(`--map-bottom-inset` 반영). 카메라 규칙은 [ParkingPage](../../apps/web/src/routes/ParkingPage.tsx): 셀 클릭 = `flyToZoomIn(셀, floor(줌)+2)`(inset 없음), 공항 마커·목록 = `flyToZoomIn(…, AIRPORT_ZOOM=14, flyInset())`, 공항 탭 전환 시 줌>9 면 전국 뷰(`{36.1, 127.8, z7}`), 주차장·충전소 목록 선택과 URL `sel` 진입(상세 좌표 도착 1회) = `flyTo(…, flyInset())`. 라벨은 주변 목록(반경 1km 15곳) + 선택만이고 주차장 라벨엔 실시간 여석이 붙는다(`이름 · 여석 N`), 공항 14곳은 이름 라벨 상시('국제공항'/'공항' 꼬리 제거). 도메인은 [parking](parking.md).
11. **`SeaMap`** ([apps/web/src/components/sea/SeaMap.tsx](../../apps/web/src/components/sea/SeaMap.tsx)) — **(2026-09-24 신규, `4a2bff1`)** `/sea` 지점 지도. 카드(`rounded-md border`) 안의 `MapCanvas`(`poolKey="sea"`)로 subBar·시트가 없다([SeaPage](../../apps/web/src/routes/SeaPage.tsx)는 `lg:grid-cols-[minmax(0,1fr)_380px]` 그리드, 지도 `h-[360px] lg:h-[600px]`). 아이콘은 단계 키(`'none'`·0~5) × 선택 = 14개 data URL 을 모듈 레벨 `ICONS` 로 1회 생성(대기 측정소 지도와 같은 방식). 라벨은 선택 지점 + 순위(`ranked`) 상위 3곳 중 단계 4 이상만(파일 주석은 "상위 3곳"이라고만 쓴다 — 해안선에 지점이 몰려 전부 붙이면 겹침). 초기 뷰 `KOREA_CENTER {36.0, 127.8, z7}`, 선택 지점 좌표 문자열(`selectedKey`)이 바뀔 때만 `flyToZoomIn(…, SELECT_ZOOM=11)`(날짜·시간대 전환으로 객체가 새로 만들어져도 다시 날지 않음, 줌아웃 없음). 오버레이는 내 위치 하나(`my-location` 클릭 무시), 내 위치 버튼은 지도 밖 페이지 헤더('내 위치로 거리 보기', `useUserLocation({ auto: false })`). 도메인은 [sea](sea.md).
...
| `buildParkingLotMarkerDataUrl(level, selected)` ([parkingMarker.ts](../../packages/utils/src/parkingMarker.ts), 2026-09-25) | 원 + 'P' | 핀 | `parkingLevelColor` — 여유 `#16a34a` / 보통 `#ca8a04` / 혼잡 `#ea580c` / 만차 `#dc2626`(점유율 0.7·0.9·0.97 경계), 실시간 없음 `PARKING_NO_LIVE_COLOR` `#2563eb`(주차 표지 파랑, [parking.ts](../../packages/utils/src/parking.ts)) | ✗ |
| `buildEvStationMarkerDataUrl(level, selected)` | 원 + 번개 | 핀 | `EV_LEVEL_COLOR` — 사용 가능 `#16a34a` / 모두 충전 중 `#ea580c` / 이용 불가·확인 필요 `#9ca3af` | ✗ |
| `buildParkingAirportMarkerDataUrl(level, selected)` | 원 + 비행기 | 핀 | 주차장과 같은 단계색(없으면 파랑) | ✗ |
| `buildParkingCellMarkerDataUrl(kind, count)` | 건수 버킷 4단 원(26/34/40/46px, 폰트 10~13, 채움 0.85 — `lifeCountBucket` 공유) + 숫자 내장 | (동일 — `selectedSrc = src`) | `PARKING_CELL_COLOR` 주차장 `#2563eb` / 충전소 `#0f766e` | ✓ |
| `buildSeaSpotMarkerDataUrl(level, selected)` ([sea.ts](../../packages/utils/src/sea.ts), 2026-09-24) | 원 + 파도 3줄 | 핀 | `SEA_INDEX_COLOR` — 매우좋음 `#0284c7` / 좋음 `#16a34a` / 보통 `#ca8a04` / 나쁨 `#ea580c` / 매우나쁨 `#dc2626` / 체험불가(0) `#6b7280` / 결측·이상값 `#9ca3af` | ✗ |
| 집값 알약 배지 `flood` 글리프 ([housingMarker.ts](../../packages/utils/src/housingMarker.ts) `buildHousing{Badge,SelectedBadge,MutedBadge,MutedSelectedBadge}Svg(…, flood)`, 2026-09-24) | 알약 안 왼쪽 원(반지름 (h−7)/2 — 비선택 7.5, 선택 9.5) + 물방울 path, 글자는 원 오른쪽 구간 가운데로 | 선택 배지도 같은 글리프(꼬리는 알약 가운데 유지) | `housingFloodLevel` some(1~4건) = 흰 원 + `HOUSING_FLOOD_COLOR` `#0284c7` 물방울 / many(5건↑) = `#0284c7` 원 + 흰 물방울 — 알약 색(유형·회색·공시 점선)과 무관 | ✓(배지 전체가 fixedScale) |

집계 버블이 숫자를 이미지에 새기는 이유: `MapCanvas` 라벨은 줌 14 미만에서 꺼지는데 셀 모드는 바로 그 저줌에서만 쓰인다. 일상지도 5색(CCTV 4 + 화장실)은 dataviz 범주 팔레트로 CVD·정상시 전 쌍 분리를 검증한 조합(lifeMapMarker.ts 주석, 2026-08-21). 호출자는 아이콘 data URL 을 **모듈 레벨 상수**로 1회 생성해 공유한다(OL 이 같은 `src` 이미지를 1회만 디코드). **(2026-09)** 주차 `LOT_ICONS`(단계 4 + 없음)·`EV_ICONS`(3)·`AIRPORT_ICONS`(5), 바다 `ICONS`(7) 도 같은 방식이고, 건수가 새겨지는 셀 버블만 `${layer}:${count}` 키 메모(`cellIconCache`, 상한 없음 — 키가 건수 정수라 작다)로 만든다. 집값 배지는 글자·유형·선택·침수 단계가 키라 상한 5,000 메모(`badgeCache`).
...
- **parking / sea 도메인** -- **(2026-09)** [ParkingMapView](../../apps/web/src/components/parking/ParkingMapView.tsx)(`/parking`)·[SeaMap](../../apps/web/src/components/sea/SeaMap.tsx)(`/sea`)가 웹 전용 소비자(앱 화면 없음). 마커 빌더는 `@repo/utils` [parkingMarker.ts](../../packages/utils/src/parkingMarker.ts)·[sea.ts](../../packages/utils/src/sea.ts) 지도 마커 절 — `markerFrame` 프레임 재사용. 주차장·충전소 점/셀 조회(bbox·줌 임계·`PARKING_POINTS_MAX` 3,000)·실시간 단계·지수 원천은 [parking](parking.md)·[sea](sea.md). 집값 물방울 글리프의 반경·단계 규칙(`HOUSING_FLOOD_RADIUS_M` 100·`HOUSING_FLOOD_MANY_MIN` 5)은 [housing](housing.md)·[life-map](life-map.md)(침수 흔적 적재).
...
- **2026-09-24~25: 새 도메인 지도(주차·바다)도 `MapCanvas` 를 늘리지 않고 `icon` 주입 + `markerFrame` + 서버 셀 집계로 — 렌더 코어 무변경 원칙 유지** -- 주차는 전국 주차장 ~1.8만·충전소 ~10만 곳이라 일상지도와 같은 서버 점/격자 셀 응답(`mode: 'points' | 'cells'`, 점 모드 임계 주차장 줌 13·충전소 15 + 좁은 bbox, 점 상한 `PARKING_POINTS_MAX` 3,000 초과 시 `truncated`)을 받고 지도는 숫자 버블(`fixedScale`)만 그린다 — 클라 클러스터링은 여전히 없다(2026-08-21 결정의 재적용, [parking.service.ts](../../apps/friendly/src/modules/parking/parking.service.ts)·[ev.service.ts](../../apps/friendly/src/modules/parking/ev.service.ts)). 바다는 활동별 예보 지점만 그려(SeaMap 주석 기준 해안선에 50곳 규모) 셀 없이 점만. 마커 색은 도메인 파일이 소유(주차 신호등 4색 + 실시간 없음 파랑, 충전소 3색, 바다 지수 6색 + 결측) — 목록·상세는 색과 함께 글자 라벨을 쓴다(`PARKING_LEVEL_COLOR` 주석). 두 페이지의 레이아웃은 갈렸다: 주차는 일상지도·집값 계열(`useIsDesktopXl` JS 분기 + 시트 + `bottomInset`), 바다는 카드형 그리드(시트 없음) — 그래서 `bottomInset` 은 주차만 쓰고, `poolKey` 는 둘 다 단일 마운트라 페이지당 키 하나(`'parking'`/`'sea'`).
...
- **(2026-09-25) 주차장 라벨에 실시간 여석이 들어간다 — 스타일 캐시 규칙의 경계선.** [parkingMarkers.ts](../../apps/web/src/components/parking/parkingMarkers.ts)는 `labeledIds`(주변 목록 15곳 + 선택)의 주차장 라벨을 `이름 · 여석 N` 으로 만든다. 여석은 5분 폴링마다 바뀌고 라벨 문자열이 `markerStyleCache` 키에 들어가므로 갱신 때마다 라벨 붙은 마커 수(≤16)만큼 새 엔트리가 생긴다 — 이 규모라 무해하지만, 라벨 대상을 뷰포트 전체 점으로 넓히면 아래 "동적 텍스트는 라벨이 아니라 별도 채널로" 함정에 그대로 걸린다.
- **(2026-09-24) 집값 배지 메모 키 주석이 낡았다(코드 주석 어긋남).** [housingMarkers.ts](../../apps/web/src/components/housing/housingMarkers.ts) 머리 주석은 여전히 "배지 이미지는 `종류|글자|유형|선택` 키로 메모"라고 쓰지만 `ad48f96` 이후 실제 키는 `종류|글자|유형|선택|침수단계`(`badgeUrl` 의 `${flood}`) 다섯 칸이다. 동작은 맞다(침수 단계가 다르면 다른 이미지) — 주석만 미갱신.
- **(2026-09-25) "주차 = 바다·집값과 같은 골격" 이라는 PLAN 문구는 바다에 대해선 틀리다(문서 어긋남).** [PLAN-parking.md](../../docs/PLAN-parking.md) 첫 줄은 "지도 + 패널(바다·집값과 같은 골격)"이지만, 코드의 [ParkingPage](../../apps/web/src/routes/ParkingPage.tsx) 주석은 "집값·일상지도와 같다"이고 실제로 `useIsDesktopXl` + 시트 + `bottomInset` 계열이다. [SeaPage](../../apps/web/src/routes/SeaPage.tsx)는 `lg` 그리드 카드(시트·subBar·`--map-bottom-inset` 없음)라 지도 관점의 골격이 다르다 — 새 지도 페이지를 "바다처럼" 만들 땐 이 차이를 먼저 확인.
...
- **(2026-08) `poolKey` `'air'`/`'life'`/`'housing'` 은 단일 마운트 전제.** 대중교통과 달리 이 페이지들은 지도를 한 장만 마운트한다(`AirNearbySection` 1회, `LifeMapPage`·`HousingPage` 는 `useIsDesktopXl` JS 분기). CSS 이중 마운트를 도입하면 대중교통처럼 키를 레이아웃별로 나눠야 한다(take 시맨틱). **(2026-09)** `'parking'`([ParkingPage](../../apps/web/src/routes/ParkingPage.tsx) — `useIsDesktopXl` 분기라 데스크톱/모바일이 같은 `<section>` 한 장)·`'sea'`([SeaPage](../../apps/web/src/routes/SeaPage.tsx) — 그리드 안 `SeaMap` 한 장)도 같은 전제다.
...
- **(2026-08-22) `bottomInset` 은 flyTo 호출자의 몫 — MapCanvas 는 시트를 모른다.** 목록에서 항목을 골라 날아갈 때 상세 시트(half = 가용 높이 55%)에 지점이 가리면 `sheetHalfInset(headerHeight)` 를 넘겨야 한다. 지금은 일상지도와 집값(2026-08-30, `HousingPage.flyInset`)만 넘기고 맛집 v2·버스·지하철 래퍼는 미적용(지점이 시트 밑으로 갈 수 있음). **(2026-09-25)** 주차가 세 번째로 넘긴다(`ParkingPage.flyInset` — 목록 선택·공항·URL `sel` 진입; 셀 버블 클릭 확대와 지역 이동 `LifeGoToBox` 는 inset 없이). 바다는 시트가 없는 카드형이라 해당 없음.
...
## Sources [coverage: high — 60 sources]
...
- [packages/utils/src/housingMarker.ts](../../packages/utils/src/housingMarker.ts) — *new (2026-08-30): 집값 가격 알약 배지(선택은 꼬리 있는 배지, 앵커 [0.5,1] 규약)·회색 점/핀(`buildPinMarkerSvg`)·평당가 셀 — 전부 fixedScale / modified (2026-09-24 `ad48f96`): 배지 4종에 `flood` 인자 — 알약 왼쪽 원+물방울 글리프(`HOUSING_FLOOD_COLOR` #0284c7, `pillLayout`)*
- [packages/utils/src/lifeFlood.ts](../../packages/utils/src/lifeFlood.ts) — *new (2026-09-24): `HOUSING_FLOOD_RADIUS_M` 100·`HOUSING_FLOOD_MANY_MIN` 5·`housingFloodLevel`(none/some/many)*
- [packages/utils/src/parkingMarker.ts](../../packages/utils/src/parkingMarker.ts) — *new (2026-09-25): 주차장 P·충전소 번개·공항 비행기 원/핀 + 숫자 셀 버블(fixedScale)*
- [packages/utils/src/parking.ts](../../packages/utils/src/parking.ts) — *new (2026-09-25): PARKING_LEVEL_COLOR·PARKING_NO_LIVE_COLOR·EV_LEVEL_COLOR, 점 모드 임계(13·15)·PARKING_POINTS_MAX·반경 상수, 공항 좌표 14곳*
- [packages/utils/src/sea.ts](../../packages/utils/src/sea.ts) — *new (2026-09-24): SEA_INDEX_COLOR·buildSeaSpotMarkerDataUrl(파도 아이콘 원/핀)*
- [packages/utils/src/busMarker.ts](../../packages/utils/src/busMarker.ts) — *buildMyLocationMarkerDataUrl(26×26 파란 점 + 흰 링 + 후광) — 주차·바다 지도의 내 위치 오버레이도 이것*
...
- [apps/web/src/components/housing/housingMarkers.ts](../../apps/web/src/components/housing/housingMarkers.ts) — *new: 응답(단지 배지/셀) → MapMarker 변환, id `c:${id}` / `cell:${index}`, 배지 data URL 을 `종류|글자|유형|선택` 키로 메모 → (2026-09-24 `ad48f96`) 키에 침수 단계가 더해져 다섯 칸(머리 주석은 네 칸 그대로)*
- [apps/web/src/components/parking/ParkingMapView.tsx](../../apps/web/src/components/parking/ParkingMapView.tsx) — *new (2026-09-25): 주차 wrapper(poolKey 'parking', 오버레이 저장 위치·내 위치, 상단 로딩/확대 힌트, 우하단 내 위치 + --map-bottom-inset)*
- [apps/web/src/components/parking/parkingMarkers.ts](../../apps/web/src/components/parking/parkingMarkers.ts) — *new: 점/셀 → MapMarker, id `lot:`·`ev:`·`airport:`·`cell:<lot|ev>:<index>`, 라벨 = 주변 목록 + 선택(주차장은 여석 포함)*
- [apps/web/src/routes/ParkingPage.tsx](../../apps/web/src/routes/ParkingPage.tsx) — *useIsDesktopXl JS 분기, flyInset(sheetHalfInset) — bottomInset 세 번째 사용처, 셀 클릭 줌+2*
- [apps/web/src/components/sea/SeaMap.tsx](../../apps/web/src/components/sea/SeaMap.tsx) — *new (2026-09-24): 바다 wrapper(poolKey 'sea', 단계 아이콘 14개 모듈 상수, 라벨 선택 + 상위 3곳 중 단계 4↑, flyToZoomIn 11)*
- [apps/web/src/routes/SeaPage.tsx](../../apps/web/src/routes/SeaPage.tsx) — *카드형 lg 그리드(지도 360/600px) — 시트·bottomInset 없음, 내 위치 버튼은 헤더*
- [apps/friendly/src/modules/parking/parking.service.ts](../../apps/friendly/src/modules/parking/parking.service.ts) — *주차장 점/셀 조회(PARKING_POINT_MIN_ZOOM·PARKING_POINTS_MAX)*
- [apps/friendly/src/modules/parking/ev.service.ts](../../apps/friendly/src/modules/parking/ev.service.ts) — *충전소 점/셀 조회(EV_POINT_MIN_ZOOM·truncated)*
- [docs/PLAN-parking.md](../../docs/PLAN-parking.md) — *"지도 + 패널(바다·집값과 같은 골격)" 문구 — 바다에 대해선 코드와 어긋남*
