...
- **tile load 에러 1회 플래그** -- `tileSource.on('tileloaderror', ...)` 는 한 번 발화 후 `errored = true` 로 잠근다. 한 화면에 타일 수십 장이 동시에 실패할 수 있어 폭주 방지. 레이어를 `setUrl` 로 바꿔도 같은 `tileSource` 라 플래그는 유지된다.
...
- **`tileloaderror` 플래그는 세션당 1회** -- 한 번 에러 후 `errored = true` 로 잠긴다. 키 변경 후엔 새로고침(또는 `apiKey` 변경으로 effect 재실행 → 새 `tileSource`)이 확실. 레이어만 바꾸는 `setUrl` 은 `tileSource` 를 유지하므로 플래그도 유지.
...
- [apps/web/src/components/restaurant/MapCanvas.tsx](../../apps/web/src/components/restaurant/MapCanvas.tsx) — *modified (17차): 레이어 토글 + flyToZoomIn + 다크 라벨 반전 + 컨테이너 wrapper 분리*
