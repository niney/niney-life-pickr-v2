...
같은 어댑터 인스턴스(=같은 FIFO 게이트)를 공유한다. 현재 외부 사용자는 두
부류다.
...
- 백그라운드 도메인 작업 — `summary` 모듈이 리뷰 구조화 분석에서 같은
  `adapterCache`를 import해 직접 `provider.complete()` 호출.
...
├── adapter-cache.ts                 # 모듈 레벨 싱글톤. ai.route + summary 공유
...
- **`AiConfigService`** — `LlmProviderConfig` Prisma 모델을 감싼 CRUD/조회 레이어.
  생성자에 `LlmProviderEnv`를 인자로 받아 테스트가 가짜 env 주입 가능.
- **`AdapterCache`** — `(apiKey, baseUrl, maxConcurrent, timeoutMs)` 4-tuple
  ...
  큐는 in-flight를 끝낸 뒤 GC. **`ai.route` 와 `summary.service` 가 같은
  싱글톤을 공유**하는 게 중요 — 서로 다른 캐시면 두 개의 `maxConcurrent`
  게이트가 병렬로 돌아 fan-out 이 두 배로 뜬다.
...
- `summary` 모듈 — `adapterCache.get(resolved)` 로 같은 어댑터 인스턴스를
  가져다 리뷰 단위 구조화 분석을 백그라운드로 실행
...
- [`adapterCache`](../../apps/friendly/src/modules/ai/adapter-cache.ts) — 모듈
  레벨 싱글톤. `get(resolved)` 가 캐시 hit / miss 결정.
...
- **provider 별 단일 어댑터 인스턴스(`adapterCache` 싱글톤).** 모든
  ...
  와 `ai.route` 가 같은 import를 공유하는 이유.
- **Ollama 옵션을 1차 시민으로 누출(`numCtx`, `num_predict`, `format`).**
  ...
  어댑터는 무시 가능.
- **Structured output: JSON Schema 강제.** `format` 에 zod-derived JSON Schema
  객체를 그대로 전달하면 Ollama 가 토큰 샘플링 단계에서 스키마와 일치하는
  토큰만 뽑는다. 후처리 JSON 파서 실패율을 크게 낮춤. (`<think>` 블록
  제거·균형괄호 JSON 추출 같은 후처리는 호출자인 `summary.service` 책임 —
  ai 모듈은 모델이 뱉은 텍스트를 그대로 돌려준다.)
...
- **`numCtx` 명시 안 하면 Ollama 기본 2048로 입력 잘림.** 긴 시스템 프롬프트
  ...
  `summary` 모듈이 `numCtx: 8192`(또는 그 이상)를 명시적으로 설정하는
...
- **`maxTokens` 미지정 시 출력 128 토큰에서 잘림.** Ollama 의 `num_predict`
  ...
  optional — 사용자가 안 넣으면 짧은 응답이 정상으로 보인다.
- **Reasoning 모델(`gpt-oss`, `deepseek-r1` 등)은 `<think>…</think>` 블록을
  먼저 뱉는다.** ai 모듈은 그대로 돌려주고, JSON 파싱 책임은 호출자에 있음.
  `summary.service` 가 `<think|reasoning|analysis>` 블록 제거 + 균형괄호 JSON
  추출 후처리를 한다. 새 호출자도 같은 처리가 필요할 수 있음.
