...
sources_count: 25
status: active
aliases: [llm, ollama, ollama-cloud, provider, purpose, vision, image, chat, providerModelsPreview, models-preview, ai-key-preview, AdminAiKeysPage-preview, useProviderModelsPreview, mobile-ai-keys-card-layout]
---
...
- 어드민 UI(`apps/web`) — `AdminAiKeysPage` 가 `useProviders` /
  ...
  `useProviderModels` / **`usePreviewModels`** 훅으로 fetch.
  (provider × purpose) 조합당 카드 하나를 렌더링하고, 등록되지 않은 조합은
  "다른 용도 추가" 버튼으로 빈 카드를 만든 뒤 다음 PUT 에서 키를 채워 활성화.
  빈 카드 흐름은 이제 "키 + base URL 입력 → '모델 미리보기' 버튼 → `usePreviewModels`
  mutation → 응답 드롭다운에서 모델 선택 → 저장" 으로 정리됐다 — 잘못된 키로
  저장된 row 가 생기는 cycle 을 제거. `AdminAiTestPage` 는 `useCompleteAi` /
...
- [`AdminAiKeysPage.tsx`](../../apps/web/src/routes/admin/AdminAiKeysPage.tsx) —
  ...
  DB row 삭제. **"모델 미리보기" 버튼** — 폼에 입력한 키 + base URL 로
  `usePreviewModels` mutation 호출, ok 응답이면 카드 하단에 모델 드롭다운을
  띄워 사용자가 선택하면 `defaultModel` 필드에 박힌다. 실패 응답은 인라인
  에러 메시지로 표시 (`'API key not authorized'` 등 provider 원문이 거의 그대로
  내려옴). 등록되지 않은 (provider × purpose) 조합은 "다른 용도 추가" 영역에
  버튼으로 노출되며 클릭 시 빈 카드를 만든다 (`{ enabled: true }`만 PUT —
  chat 은 env fallback 으로 키가 들어오고, image 는 빈 키로 카드만 생성돼
  다음 저장에서 채워야 한다). 좁은 화면에서는 키/모델/동시성 컬럼이 세로로
  접히고 버튼 순서도 모바일 단말 친화적으로 재배치된다 (alias:
  `mobile-ai-keys-card-layout` — 식별자만 다를 뿐 같은 페이지).
...
- [`packages/api-contract/src/schemas/ai.ts`](../../packages/api-contract/src/schemas/ai.ts) (수정 — `PreviewLlmModelsInput` / `PreviewLlmModelsResult` 추가)
- [`packages/api-contract/src/routes.ts`](../../packages/api-contract/src/routes.ts) (수정 — `Routes.Ai.providerModelsPreview(id, purpose)` 추가)
- [`packages/api-contract/src/index.ts`](../../packages/api-contract/src/index.ts) (barrel — 신규 schema 자동 re-export)
- [`packages/shared/src/api/ai.api.ts`](../../packages/shared/src/api/ai.api.ts) (수정 — `aiApi.previewModels(key, input)`)
- [`packages/shared/src/hooks/useAi.ts`](../../packages/shared/src/hooks/useAi.ts) (수정 — `usePreviewModels` mutation 훅)
- [`apps/web/src/routes/admin/AdminAiKeysPage.tsx`](../../apps/web/src/routes/admin/AdminAiKeysPage.tsx) (수정 — "모델 미리보기" 버튼 + 드롭다운 흐름, 좁은 화면 카드 레이아웃)
