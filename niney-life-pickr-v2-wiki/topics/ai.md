...
같은 어댑터 인스턴스(=같은 FIFO 게이트)를 공유한다. 현재 외부 사용자는 세
부류로 늘었다.
...
세 도메인 모두 같은 `adapterCache` import + 같은 `AiConfigService.getResolved('ollama-cloud')`
경로를 거치므로 동시성 캡 + 429 재시도/백오프가 도메인을 가리지 않고 적용된다.
도메인별로 다른 건 prompt + JSON schema 모양 + 청크 사이즈뿐.

핵심 설계 목표:

- 벤더 SDK를 서비스 레이어 밖으로 격리(`LLMProvider` 어댑터 인터페이스).
- 운영자가 서버 재시작 없이 API 키·동시성 한도·기본 모델을 바꿀 수 있도록
  DB 우선 + env fallback 2단 구성.
- 단건/배치 호출 모두에서 부분 실패를 허용하고, 도메인 에러를 와이어 친화적인
  `AiErrorCodeType`으로 변환.
- Ollama 고유 옵션(`num_ctx`, `num_predict`, `format`)을 1차 시민으로 노출 —
  reasoning/structured-output 워크로드에서 컨텍스트 잘림·파싱 실패를 줄이기
  위한 의도적 누출.
...
├── adapter-cache.ts                 # 모듈 레벨 싱글톤. ai.route + summary
│                                    #   + menu-grouping + analytics 공유
...
- **`AiConfigService`** — `LlmProviderConfig` Prisma 모델을 감싼 CRUD/조회 레이어.
  ...
  도메인(summary/menu-grouping/analytics)은 모두 자체 인스턴스를 만들어
  `getResolved('ollama-cloud')`를 호출 — 같은 DB row 를 읽으므로 운영자가 키를
  바꾸면 다음 호출부터 즉시 반영된다.
- **`AdapterCache`** — `(apiKey, baseUrl, maxConcurrent, timeoutMs)` 4-tuple
  키로 `OllamaCloudAdapter` 인스턴스를 1개만 캐시하는 모듈 레벨 싱글톤
  ([`adapter-cache.ts`](../../apps/friendly/src/modules/ai/adapter-cache.ts)).
  config가 바뀌면 키가 달라지므로 새 어댑터가 생성되고, 기존 어댑터의 FIFO
  큐는 in-flight를 끝낸 뒤 GC. **`ai.route` / `summary` / `menu-grouping` /
  `analytics` 가 모두 같은 싱글톤을 공유**하는 게 중요 — 서로 다른 캐시면 N
  개의 `maxConcurrent` 게이트가 병렬로 돌아 fan-out이 그만큼 늘어난다.
...
요청 흐름(백그라운드 도메인 — summary/menu-grouping/analytics 공통):

1. 도메인 서비스가 자체 `AiConfigService`로 `getResolved('ollama-cloud')` 호출
   → null 이면 작업을 즉시 실패 처리.
...
2. `completeWithRetry()` — `doComplete()` 시도. 응답이 `LLMUpstreamError(429)`
   ...
   유지된다.
3. `doComplete()` — `/api/chat` POST. body 에 `messages`, `options.temperature`,
   `options.num_predict`, `options.num_ctx`, 최상위 `format` 을 조립.
...
- 어드민 UI(`apps/web`) — `AdminAiKeysPage` 가 `useProviders` /
  ...
  `useProviderModels` 훅으로 fetch. `AdminAiTestPage` 는 `useCompleteAi` /
  `useCompleteBatchAi` 로 단건/배치 + 모델 비교 + 샘플 N개 모드 실행.
- [`summary`](friendly.md) 모듈 — `adapterCache.get(resolved)` 로 같은 어댑터를
  ...
  JSON 추출 후처리 로직을 다른 도메인이 재사용 가능.
- [`menu-grouping`](menu-grouping.md) 모듈 — 식당당 1회, distinct 메뉴 변형 리스트를
  ...
  identity fallback (입력 키 = canonical).
...
- Ollama Cloud HTTP API — `POST {baseUrl}/api/chat` (단건 컴플리션 + 구조화
  출력), `GET {baseUrl}/api/tags` (모델 카탈로그). 두 호출 모두
  `Authorization: Bearer {apiKey}` 헤더 필수.
- `@repo/api-contract` — 모든 와이어 타입.
...
- `LLMCompleteOptions` — `prompt` / `model` 필수. `systemPrompt`, `temperature`,
  `maxTokens`, **`numCtx`**, **`format` ('json' | JSON Schema 객체)**, `signal`
  선택. `numCtx` / `format` 은 Ollama 가 1차 시민으로 받지만, 다른 어댑터는
  자유롭게 무시 가능.
- 도메인 에러 4종(같은 파일):
...
- [`adapterCache`](../../apps/friendly/src/modules/ai/adapter-cache.ts) — 모듈
  레벨 싱글톤. `get(resolved)` 가 캐시 hit / miss 결정. summary / menu-grouping /
  analytics 모두 이 import 를 공유.
...
- `AiCompleteInput.prompt` 1–8000자, `model` 1–100자, `systemPrompt` ≤2000,
  ...
  `numCtx`/`format` 이 노출되지 않는다 — `LLMCompleteOptions` 의 내부 필드로,
  `summary` / `menu-grouping` / `analytics` 같은 in-process 호출자만 쓴다.
...
- [`AdminAiKeysPage.tsx`](../../apps/web/src/routes/admin/AdminAiKeysPage.tsx) —
  provider 별 카드. write-only API 키 입력, baseUrl/defaultModel/maxConcurrent
  편집, 활성화 토글, 모델 datalist 자동완성, 연결 테스트(현재 form 의
  `defaultModel` 값을 그대로 보냄), DB row 삭제(env fallback 으로 회귀).
- [`AdminAiTestPage.tsx`](../../apps/web/src/routes/admin/AdminAiTestPage.tsx) —
  ...
  — 꺼두면 필드를 보내지 않아 provider 기본값 사용.
...
**도메인별 prompt + JSON schema + 청크 사이즈:**

| 도메인 | VERSION 상수 | 청크 | JSON Schema (additionalProperties) | 호출 빈도 |
| ------ | ------------ | ---- | ---------------------------------- | --------- |
| summary | `ANALYSIS_VERSION = 4` (traits + menus[].sentiment) | 리뷰 1건/호출 | review analysis schema (rating/menus/traits/...) | 리뷰 단위 |
| menu-grouping | `MENU_GROUPING_VERSION = 1` | 80 | `{ type: 'string' }` (입력 키 → canonical) | 식당당 1회 |
| analytics | `GLOBAL_MERGE_VERSION = 2` | 50 (v2는 출력 토큰 ↑) | `{ type: 'object', properties: { canonical, categoryPath }, required: [...] }` | 글로벌 1회 (pass1 + pass2) |

각 도메인이 자기 VERSION 상수를 row 에 함께 저장 → 프롬프트/스키마 변경 시
상수를 올리면 stored < current 인 record 가 자동으로 stale 판정되어 재계산 큐에
들어간다. 자세한 라이프사이클은
[`menu-grouping`](menu-grouping.md) / [`analytics`](analytics.md) 토픽 참고.

**큐/캐시:**

- **`adapterCache`** — 모듈 레벨 싱글톤(import 한 모든 곳이 동일 인스턴스).
  `(apiKey, baseUrl, maxConcurrent, timeoutMs)` 튜플 변경 시 새
  `OllamaCloudAdapter` 생성. 메모리 외 영속화 없음.
- **`OllamaCloudAdapter.waiters`** — 내부 FIFO 배열. `inflight < maxConcurrent`이면
  ...
  의 식당당 1회 호출, analytics 의 두-패스 청크 호출이 모두 같은 큐에 줄선다.
- **`AiService.lastCallByActor`** — `Map<userId, lastTimestampMs>`. 1초
  슬라이딩 윈도우 per-actor rate limit. **어드민 라우트에만 적용** —
  백그라운드 도메인은 `AiService` 를 거치지 않으므로 영향 없음. 영속성 없음
  (프로세스 재시작 시 리셋).
...
- **provider 별 단일 어댑터 인스턴스(`adapterCache` 싱글톤).** 모든
  in-flight 호출이 같은 FIFO 게이트를 공유해야 동시성이 의미 있게 통제된다.
  매 요청 새 어댑터를 만들면 `maxConcurrent` 가 사실상 무의미. `summary` /
  `menu-grouping` / `analytics` / `ai.route` 가 같은 import를 공유하는 이유.
- **Ollama 옵션을 1차 시민으로 누출(`numCtx`, `num_predict`, `format`).**
  추상에 깔끔히 안 맞아도 실용 우선. Ollama 의 `num_ctx` 기본값 2048 / `num_predict`
  기본값 128 때문에 입력·출력 잘림 사고가 잦았고(`#620ed6f`), 추상 인터페이스
  뒤에 가두는 것보다 호출자가 명시적으로 끌어 쓰는 게 디버깅이 빠름. 다른
  어댑터는 무시 가능.
- **Structured output: JSON Schema 강제 + caller-side identity fallback.**
  ...
  파싱 후처리(`<think>` 블록 제거·균형괄호 JSON 추출)는
  `summary.service.extractFirstJsonObject` 를 도메인 간 공유.
- **도메인별 VERSION 상수.** `ANALYSIS_VERSION` (summary) / `MENU_GROUPING_VERSION`
  (menu-grouping) / `GLOBAL_MERGE_VERSION` (analytics). 프롬프트·스키마·청크 정책
  중 하나라도 의미 있는 변경이 일어나면 상수를 올린다. record 의 stored
  version 이 current 보다 낮으면 stale 표시 → 재계산. 단순 word smithing 변경엔
  올리지 않는다.
- **429 / "too many concurrent requests" 자동 백오프.** Ollama Cloud 가 로컬
  게이트 통과 후에도 자체 한도로 거부할 수 있어, 어댑터에서 200·400·800ms +
  jitter 로 최대 3회 재시도(`#399e088`). **슬롯을 잡은 채** 재시도해 동시성이
...
- **per-actor 1초 rate-limit 은 어드민 한정.** 토큰 단위 정교한 limiter 대신,
  ...
  cheap·in-memory. 백그라운드 도메인은 `AiService` 를 거치지 않으므로
  per-actor limit 에 영향받지 않는다 (어차피 actor 가 시스템).
- **테스트 친화적 의존성 주입.** `AiConfigService(prisma, env)` /
  `AiService(provider, configService)` — 어댑터·env 를 fake 로 갈아 끼워
  단위 테스트 가능. 라우트 통합 테스트는 `Fastify({ logger: false })` 위에
  필요한 플러그인만 명시 등록(`autoload` 우회).
...
- **`numCtx` 명시 안 하면 Ollama 기본 2048로 입력 잘림.** 긴 시스템 프롬프트
  ...
  이유. 어드민 `/complete` 엔드포인트는 와이어 스키마에 `numCtx` 가 없어
  기본 2048 그대로 — 긴 prompt 디버깅 시 주의.
...
- **Reasoning 모델(`gpt-oss`, `deepseek-r1` 등)은 `<think>…</think>` 블록을
  ...
  `summary.service.extractFirstJsonObject` 가 표준 후처리. 새 도메인이 LLM 을
  쓸 때도 같은 헬퍼를 import 하면 된다.
...
- **`empty-body-parser`가 빠지면 `POST /providers/:id/test`가 빈 body로 오면
  400.**
- **`AdapterCache`는 단일 슬롯이다.** 여러 provider id가 추가되더라도 현재
  구현은 마지막 1개 캐시만 유지. 다중 provider 시 `Map<providerId, …>`로
  확장 필요.
- **한 게이트에 모든 도메인이 줄선다.** `maxConcurrent = 15` 이면 어드민
  `/complete-batch` 10건이 들어와 있을 때 background analytics 청크 호출은
  실질적으로 5 슬롯만 쓸 수 있다. 도메인 간 우선순위 / 페어링 / 페어 큐는
  현재 없음 — 필요하면 별도 설계.
...
- **모델 list의 필드 이름이 두 가지.** Ollama Cloud는 `model`, 로컬 Ollama는
  ...
  필터링되어 사라진다.
- **DELETE 후에도 `list()`는 env 기반 가상 row를 합성한다.** UI 에는 한 줄이
  여전히 보이고, `updatedAt` 이 null 인 점으로만 구분 가능.
