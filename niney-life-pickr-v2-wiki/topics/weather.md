...
sources_count: 81
status: active
aliases: [날씨, weather, 기상청, KMA, kma-api, 단기예보, 중기예보, VilageFcstInfoService, MidFcstInfoService, 15084084, 15059468, getUltraSrtNcst, getUltraSrtFcst, getVilageFcst, getFcstVersion, getMidFcst, getMidLandFcst, getMidTa, getMidSeaFcst, 초단기실황, 초단기예보, 중기전망, 중기해상예보, 기상청 API허브, apihub, AWS, 방재기상관측, nph-aws2_min, stn_inf, KmaApiError, KmaApiHubError, WeatherService, AwsService, kma-api.adapter, kma-apihub.adapter, latLngToKmaGrid, LCC 격자, nx-ny, base_time, 발표 슬롯 캐시, NO_DATA 폴백, DATA_GO_KR_API_KEY, KMA_API_KEY, KMA_APIHUB_KEY, weatherRegions, WEATHER_PLACES, 구·군 지점, regId, tmFc, WeatherPage, WeatherNowHero, WeatherMeteogram, WeatherDailyStrip, WeatherSeaSection, MyLocationChip, MyLocationCard, useWeather, useWeatherNowcast, useWeatherAws, useMyLocationGlance, mergeDailyRows, weatherUpstreamMessage, weatherGlyph, 내 위치, 상단바 알약, 홈 내 위치 카드, 바다, /sea, SeaPage, 국립해양조사원, KHOA, 생활해양예보지수, 물때, 이안류, 해상 두 곳, 중기해상 vs 바다, 사이드바 14항목, kmaTodayIsoDate, dateLabel 공용, route summary, 한국어 summary]
---

# weather — 기상청 단기·중기예보 + API허브 AWS 관측 보강 날씨 (웹 `/weather` · 앱 `/weather` · 저장한 내 위치 글랜스)

**2026-09-24~09-26 변경 흡수 — 형제 페이지 `/sea`(바다)가 사이드바 날씨 바로 다음에, 날씨 라우트 6개엔 외부 문서용 한국어 summary(`4a2bff1`·`1b621c4`)**: 날씨 모듈의 동작 변경은 0 — [weather.route.ts](../../apps/friendly/src/modules/weather/weather.route.ts) 6개 GET 에 `summary`(+ `description` 4개: aws·nowcast·forecast·mid)만 붙어 [docs/api/endpoints.md](../../docs/api/endpoints.md) weather 절(6개, 전부 공개·60/분)로 외부에 노출된다(`1b621c4`, 2026-09-24, → [api-docs](api-docs.md)). 같은 날 `4a2bff1` 로 [SeaPage](../../apps/web/src/routes/SeaPage.tsx)(`/sea`, 메뉴 '바다')가 [PublicSidebar](../../apps/web/src/components/PublicSidebar.tsx)·[PublicTopBar](../../apps/web/src/components/PublicTopBar.tsx) NAV 에서 '날씨' 바로 다음에 들어왔고, 이튿날 `2ff2c31` 로 '주차'가 대중교통 다음에 끼어 NAV 는 14항목 `홈·맛집·대중교통·주차·일상지도·집값·여행·날씨·바다·대기질·타로·사주(C)·사주(G)·식단` 이 됐다. 바다는 날씨의 해상 중기예보(`/weather?sea=` — 기상청 `getMidSeaFcst`, 12 해역 D+4~D+10 날씨·파고)와 **원천·단위·용도가 다른 별개 페이지**다: 국립해양조사원(KHOA) 생활해양예보지수 6종(해수욕·서핑·바다낚시·갯벌체험·바다갈라짐·바다여행)의 **지점별 7일 오전/오후 5단계 활동 지수** + 가까운 조석 예보지점의 만조·간조(물때) + 이안류(6~9월) — 레저 활동 적합도이지 해상 날씨가 아니다. 두 화면은 서로 링크하지 않는다. 공유하는 건 공용 키 `DATA_GO_KR_API_KEY`(데이터셋 8건 활용신청은 따로), 레이트리밋 등급 `RATE.transitRealtime`, utils 날짜 헬퍼(`kmaTodayIsoDate`·[dateLabel.ts](../../packages/utils/src/dateLabel.ts))뿐이고, 캐시·쿼터 카운터·"내 위치" 저장소는 공유하지 않는다(바다는 GPS 버튼만). 세부는 [sea](sea.md).
...
- [utils/dateLabel.ts](../../packages/utils/src/dateLabel.ts): `todayKst`(= `kmaTodayIsoDate`) · `relativeDayLabel`(오늘/내일/모레/M/D) · `formatYmdWithWeekday` — 날씨·대기 웹·앱 공용(웹 `airGrade.ts` 는 재수출). **(2026-09-24~)** 바다도 쓴다 — [SeaPage](../../apps/web/src/routes/SeaPage.tsx)의 날짜 칩·[SeaSpotDetail](../../apps/web/src/components/sea/SeaSpotDetail.tsx)의 7일 띠 제목이 `todayKst`·`relativeDayLabel`·`formatYmdWithWeekday`, 서버 [sea.service.ts](../../apps/friendly/src/modules/sea/sea.service.ts)가 이안류 제공 기간(6~9월, `isSeaRipSeason`)의 KST 월 계산에 `kmaTodayIsoDate` 를 import 한다(날씨 헬퍼가 KST 날짜 공용 헬퍼가 된 셈).

### 웹 — URL 이 유일한 진실

셸(PublicLayout·TopBar 폭 예산·테스트 인프라 MSW)은 [web](web.md), 여기서는 날씨 페이지 구조만. [WeatherPage.tsx](../../apps/web/src/routes/WeatherPage.tsx) `resolveLocation(p, ll, saved)`: `?ll=lat,lng` → `?p=지점id` → (둘 다 없고) 저장한 내 위치 → 서울. 지점이면 청사 좌표→격자 + 소속 중기 구역/기온 지점, 임의 좌표(GPS·저장 위치)면 격자는 정확히, 중기예보·표시명은 `nearestWeatherPlace` 기준("내 위치 · 서울 양천구 기준 (1.2km)"). `savedHere` 는 좌표 차 0.0005°(≈50m) 이내. 해역은 `?sea=`, 없으면 권역별 기본(`DEFAULT_SEA_BY_LAND` — 서울·인천·경기 → 서해중부 12A20000, 부산·울산·경남 → 남해동부 …). 시도→지점 2단 `<select>`(광역시 시청 행은 "서울 (전체)"), "내 위치"(`acquirePosition` 10초·2회), "저장한 내 위치(라벨)" 바로가기, "이 지점을 내 위치로 저장"(라벨 항상 채움 — 지점이면 그 이름, 좌표면 최근접 지점 이름; source `place`/`geolocation`) / "저장됨·해제". 전국 전망은 토글했을 때만 `useWeatherMid(land, ta, '108')` 을 추가로 켠다. 컴포넌트: [WeatherNowHero](../../apps/web/src/components/weather/WeatherNowHero.tsx)(56px 기온 히어로 + 타일 4 + `AwsLine` + 6시간 띠, 상태 아이콘은 **초단기 첫 시각 sky + 실황 pty** — 실황엔 하늘상태가 없다), [WeatherMeteogram](../../apps/web/src/components/weather/WeatherMeteogram.tsx)(SVG 소형 다중 — 아이콘 행 → 기온 선 → 강수확률 막대 + 강수량 글자, 호버/←→ 크로스헤어 툴팁 + `<details>` 표 쌍둥이, `useElementWidth` ResizeObserver), [WeatherDailyStrip](../../apps/web/src/components/weather/WeatherDailyStrip.tsx)(열흘 — 오전/오후·하루 아이콘, 전 기간 공통 축 기온 막대, 중기 `±오차`), [WeatherSeaSection](../../apps/web/src/components/weather/WeatherSeaSection.tsx)(해역 `<select>` + 오전/오후 파고 표), [WeatherVersions](../../apps/web/src/components/weather/WeatherVersions.tsx)(사용 중 base vs `getFcstVersion` 파일 생성 시각, 직전 발표분/저장본 비고), [WeatherLegend](../../apps/web/src/components/weather/WeatherLegend.tsx)(코드표·category 표·갱신 주기·공공누리 제1유형 출처 링크), [WeatherPrimitives](../../apps/web/src/components/weather/WeatherPrimitives.tsx)(`WeatherStaleNote`·`WeatherFallbackNote`·`Segmented`; 섹션/상태 블록은 대기의 `AirSection`/`AirStateBlock` 재사용), [weatherIcons.tsx](../../apps/web/src/components/weather/weatherIcons.tsx)(lucide 표 DAY/NIGHT + 톤 — 렌더 중 컴포넌트 생성 금지), [weatherFormat.ts](../../apps/web/src/components/weather/weatherFormat.ts)·[weatherDaily.ts](../../apps/web/src/components/weather/weatherDaily.ts)(utils/shared 재수출 + `tempTicks` 눈금·폭 훅). 차트 색은 [tailwind.css](../../apps/web/src/styles/tailwind.css)의 `--weather-temp`/`--weather-precip` = 대기 시리즈 토큰 별칭. 라우트는 [App.tsx](../../apps/web/src/App.tsx) lazy `/weather`, 메뉴는 [PublicSidebar](../../apps/web/src/components/PublicSidebar.tsx)·[PublicTopBar](../../apps/web/src/components/PublicTopBar.tsx) NAV `홈 · 맛집 · 대중교통 · 일상지도 · 날씨 · 대기질 (· 식단)`(~2026-08 기준 — 2026-09-26 현재 14항목 `홈 · 맛집 · 대중교통 · 주차 · 일상지도 · 집값 · 여행 · 날씨 · 바다 · 대기질 · 타로 · 사주(C) · 사주(G) · 식단`, 날씨 바로 다음이 바다 `4a2bff1`).
...
- **[air-quality](air-quality.md) 와 "내 위치" 공유** — 저장소는 하나: [airLocationStore](../../packages/shared/src/stores/airLocationStore.ts)(게스트 zustand persist `air-location-v1`, 주입형 storage) + [useAirLocation](../../packages/shared/src/hooks/useAirLocation.ts)(로그인 서버 값 우선, 로그인 직후 서버가 비어 있으면 게스트분 PUT 1회). 날씨가 저장하는 출처 `place` 는 `7704f8c` 에서 계약(`air-quality.ts`)과 서버 매핑([air-location.service.ts](../../apps/friendly/src/modules/air-quality/air-location.service.ts))에 추가됐다. 웹 날씨 페이지는 대기의 `AirSection`/`AirStateBlock`/`airGrade` 재수출·차트 색 토큰을 그대로 쓴다. 글랜스 훅은 대기 `useAirNearbyStations` 도 함께 부른다. **(2026-09-25)** 이 저장소의 읽기 전용 소비자가 하나 늘었다 — [ParkingPage](../../apps/web/src/routes/ParkingPage.tsx)가 `useAirLocation()` 으로 진입 중심(URL 없을 때)·지도 보라 점 오버레이·지역 이동 상자의 "저장한 내 위치" 바로가기를 만든다(저장·해제는 하지 않음). 바다([SeaPage](../../apps/web/src/routes/SeaPage.tsx))는 저장소를 읽지 않고 `useUserLocation({ auto: false })` GPS 버튼만 쓴다.
- **[sea](sea.md) — 형제 페이지 `/sea`(2026-09-24 `4a2bff1`, 메뉴 날씨 바로 다음)** — 해상 정보가 두 곳에 있지만 원천이 다르다. 날씨 `WeatherSeaSection`(`?sea=`)은 기상청 중기해상예보(`getMidSeaFcst`, `MidFcstInfoService` 15059468) — 12 해역 × D+4~D+10 오전/오후 날씨·파고 최저/최고, 해역 코드 `regId` 단위. 바다는 국립해양조사원 생활해양예보지수 6종(15142484·15142490·15142486·15142489·15142485·15142491) + 조석예보 15156018 + 이안류 15156028 — 지점 단위 7일 오전/오후 5단계 활동 지수(갯벌·바다갈라짐은 하루 한 번, 해수욕·바다여행은 D+3 이후 '일' 하루 예보 — `period` null), 물때, 이안류([sea.route.ts](../../apps/friendly/src/modules/sea/sea.route.ts) `GET /api/v1/sea/forecast?activity=`·`/sea/tide?lat&lng&date`). 서버 모듈도 따로라 `WeatherService` 의 발표 슬롯 캐시·일일 쿼터 카운터(9,000)와 무관하다 — [sea.service.ts](../../apps/friendly/src/modules/sea/sea.service.ts)는 활동별 1시간·물때 지점×날짜 12시간 캐시(stale 12시간)이고 쿼터 카운터 없이 "활동 6 × 하루 24회 ≈ 300콜" 이라는 키 공간 계산에 기댄다(파일 머리 주석). 앱은 바다 화면이 없다(웹 먼저).
...
**외부 문서(2026-09-24 `1b621c4`)** — 6개 라우트 `schema` 에 한국어 `summary`(예: nowcast "초단기실황 + 초단기예보(격자) — 기상청 단기예보 API 프록시, 다음 발표까지 캐시")와 `description` 4개(aws: 키 없으면 `enabled:false` 200 / nowcast·forecast: `nx·ny` 는 위경도가 아닌 동네예보 격자, 직전 발표분 1회 폴백 / mid: `land`·`ta`·`stn` 코드 의미)가 붙어 `export:openapi` 가 [docs/api/endpoints.md](../../docs/api/endpoints.md) weather 절(6행, 인증 "공개", 한도 60/분)과 `openapi.json` 으로 내보낸다. 응답·검증은 무변경. 수기 [docs/api/README.md](../../docs/api/README.md) 도메인 개요는 "단기·초단기는 위경도가 아니라 기상청 격자 `nx`·`ny`(`@repo/utils` `latLngToKmaGrid` 로 변환)", "공공 API 일일 쿼터는 서버 전체 공용 — 날씨 약 9000콜", 한도 표 "실시간 교통·날씨·대기·바다 60/분" 을 적는다. 파이프라인은 [api-docs](api-docs.md).
...
- **2026-09-24 바다는 날씨의 섹션이 아니라 별도 페이지 `/sea` — 메뉴는 날씨 바로 다음(`4a2bff1`)** — 커밋 본문 "메뉴 '바다'(/sea, 날씨 옆)". 날씨 페이지에 이미 해상 섹션(`?sea=`, 기상청 중기해상)이 있지만, 국립해양조사원 지수는 "이번 주 바다 어디 갈까" 를 푸는 **지점×활동 지도·순위**(활동 6 탭·날짜·오전/오후 선택, 지점 상세의 7일 띠·어종/등급 variants·물때·이안류)라 격자·지점 기준 날씨 화면과 축이 다르다 — 날씨 해상 섹션은 그대로 두고 두 화면을 잇지 않았다. 웹 먼저(앱 미구현), 적재 없이 API 프록시 + 메모리 캐시라는 날씨의 골격(DB 없음·stale 폴백·in-flight 합류)은 같다([sea](sea.md)).
...
- **2026-08-21 공개 메뉴 순서(`69ed65f`)** — 날씨를 대기질 앞에(사용 빈도). 이후 `1d92acb` 로 일상지도가 날씨 앞에 끼어 현재 `홈 · 맛집 · 대중교통 · 일상지도 · 날씨 · 대기질`(~2026-08 기준 서술 — 2026-09-26 현재 NAV 는 집값·여행·타로·사주(C)·사주(G)까지 14항목이고, 2026-09-24 `4a2bff1` 바다가 날씨와 대기질 사이에, 2026-09-25 `2ff2c31` 주차가 대중교통 다음에 들어갔다. "날씨가 대기질보다 앞" 은 그대로지만 이제 바로 인접하지 않는다).

## Gotchas [coverage: high — 12 sources]

- **(2026-09-24~) 해상 정보가 두 화면에 있고 서로 링크가 없다.** `/weather?sea=`(기상청 중기해상, 해역 D+4~D+10 날씨·파고)와 `/sea`(국립해양조사원 지점 활동 지수·물때·이안류)는 원천·단위(해역 regId vs 지점)·기간이 달라 값이 맞지 않는 게 정상이다 — 예컨대 "바다 좋음" 지수와 중기해상 파고를 대조해 버그로 보지 말 것. 두 페이지 사이 이동 링크가 없어 사용자는 메뉴(날씨 → 바다 인접)로만 오간다. 바다 쪽 세부 함정(바다낚시 `gubun`, '일' 예보, 이안류 6~9월)은 [sea](sea.md).
- **`WeatherService` 인스턴스가 둘** — weather 라우트와 [meal-recommendation.route.ts](../../apps/friendly/src/modules/meal-recommendation/meal-recommendation.route.ts)가 각자 `new WeatherService(...)` 를 만든다. 캐시·in-flight·일일 쿼터 카운터가 **인스턴스별**이라 같은 격자도 두 번 묻고, "9,000 합산" 은 인스턴스 안에서만 참이다(실제 상한은 2×9,000 까지 갈 수 있음). 쿼터·캐시 모두 메모리라 재시작하면 리셋(단일 인스턴스 전제). **(2026-09-24)** 외부 문서 [docs/api/README.md](../../docs/api/README.md)는 "공공 API 쿼터(일일)는 이 서버 전체가 함께 쓴다 … 날씨 약 9000콜" 이라고 단순화해 적는다 — 외부 호출은 날씨 라우트 인스턴스만 태우므로 외부 사용자 관점에선 맞지만, 식단 추천 인스턴스 몫은 별도 카운터라 업스트림 실사용은 9,000 을 넘을 수 있다(data.go.kr 개발계정 한도는 서비스별 10,000).
...
- [apps/friendly/src/modules/weather/weather.route.ts](../../apps/friendly/src/modules/weather/weather.route.ts) — 6 GET 에 한국어 summary + description 4(`1b621c4`, 동작 무변경)
...
- [apps/web/src/components/PublicSidebar.tsx](../../apps/web/src/components/PublicSidebar.tsx) — 메뉴 순서(2026-09-26 14항목, 날씨 다음 바다 `Waves`)
...
**형제·연관 (2026-09-24~09-26 라운드)**
- [apps/web/src/routes/SeaPage.tsx](../../apps/web/src/routes/SeaPage.tsx) — 바다 `/sea`(날씨 다음 메뉴), `todayKst`·`relativeDayLabel`·`formatYmdWithWeekday` 사용, GPS 버튼만(저장 위치 미사용)
- [apps/web/src/components/sea/SeaSpotDetail.tsx](../../apps/web/src/components/sea/SeaSpotDetail.tsx) — 7일 띠 제목에 dateLabel 헬퍼
- [apps/friendly/src/modules/sea/sea.route.ts](../../apps/friendly/src/modules/sea/sea.route.ts) — `/sea/forecast`·`/sea/tide`, 같은 `RATE.transitRealtime`
- [apps/friendly/src/modules/sea/sea.service.ts](../../apps/friendly/src/modules/sea/sea.service.ts) — 활동별 1시간·물때 12시간 캐시, 쿼터 카운터 없음, `kmaTodayIsoDate` import
- [apps/web/src/routes/ParkingPage.tsx](../../apps/web/src/routes/ParkingPage.tsx) — 저장한 내 위치(`useAirLocation`) 읽기 전용 소비자
- [docs/api/endpoints.md](../../docs/api/endpoints.md) — weather 절 6행(공개·60/분)
- [docs/api/README.md](../../docs/api/README.md) — 날씨 격자 `nx`·`ny` 안내, "날씨 약 9000콜" 공용 쿼터 서술
