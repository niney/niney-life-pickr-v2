...
status: active
aliases: [naver-search-adapter, search-route, crawl-job-log, plugins-summaries]
sources_count: 71
---
...
**plugins/summaries.ts — app-level singleton 패턴 (2026-05-19)** — 신규 [plugins/summaries.ts](../../apps/friendly/src/plugins/summaries.ts) 가 `SummaryService` + `JobLogService` + `AiConfigService` 셋을 `fastify-plugin` 으로 묶어 `app.decorate('summaries' | 'jobLog' | 'aiConfig', ...)` 로 노출. `dependencies: ['prisma']` 로 prisma 플러그인 뒤 강제. 기존엔 각 라우트(`crawl.route.ts`, `restaurant.route.ts`) 가 자체 `new SummaryService(...)` 를 만들어 `runChainByPlace` Map · `cancelledPlaces` Set 이 **라우트별로 분리** — `cancelSummaryForPlace` 가 한쪽 chain 만 끊어 다른 라우트가 큐잉한 batch 는 계속 진행되는 버그가 있었다. 전역 singleton 으로 가도록 변경하면서 `server.ts` 의 boot hook (`cleanupStaleReviewSummaries` + `rescheduleStaleSummaries`) 가 같은 인스턴스에 enqueue 한다.

**CrawlJobLog 시스템 (2026-05-19)** — 신규 [modules/crawl/job-log.service.ts](../../apps/friendly/src/modules/crawl/job-log.service.ts) 가 크롤+요약 단계별 로그를 세 곳에 동시 흘려보내는 단일 진입점: (1) `app.log` pino 콘솔, (2) `prisma.crawlJobLog` DB 영속화 (fire-and-forget), (3) SSE 채널 — `channel: 'crawl'` 이면 `jobRegistry` + (placeId 있으면) `summaryEventsBus` 양쪽으로 fan-out, `channel: 'summary'` 면 `summaryEventsBus` 단방향. 모노톤 `seq` 카운터를 발급해 `(jobId, seq)` 로 클라이언트 dedup. `crawl.service.ts` 는 크롤 단계 (`page_load`/`reviews_loaded`/`persist_done`/...) 에 `channel: 'crawl'` 로 흘리고, `summary.service.ts` 는 LLM 단계 (`summary_queue`/`summary_run`/`summary_chunk`/`summary_retry`/`summary_failed`) 에 `channel: 'summary'` 로 흘려보낸다. 크롤 SSE 가 done 으로 닫힌 뒤에도 어드민 UI 가 같은 placeId 의 요약 단계 로그를 받아 누적할 수 있다. SSE 채널과 DB 가 독립이라 한쪽 실패가 다른 쪽을 막지 않음.

**Summary 라이프사이클 확장 — queued / cancelled / 부팅 자동 재큐잉 (2026-05-19)** — `ReviewSummary.status` enum 이 5종(pending/running/done/failed)→ **6종 (queued/pending/running/done/failed/cancelled)** 로 확장. (1) `queueSummariesForReviews` 가 호출 즉시 `createMany({ status: 'queued' })` 로 DB 흔적을 남긴다 — chain 메모리 큐가 서버 재시작에 휘발되더라도 어디까지 적재됐는지 영구 보존. UNIQUE 충돌 시 행 단위 upsert + `update: {}` 폴백. (2) 어드민 "요약 중지" → `cancelSummaryForPlace` 가 `cancelledPlaces: Set<string>` 에 placeId 등록 + `runChainByPlace.delete(placeId)` + DB 의 queued/pending 행을 `cancelled` 로 마킹. 현재 청크는 끝까지 흘러간 뒤 자연 종료. (3) "요약 재개" → `resumeSummaryForPlace` 가 cancelledPlaces 해제 + `status='cancelled'` 행을 `queued` 로 flip + `queueSummariesForReviews` 재호출. failed/parse_failed 는 손대지 않음 (reanalyze 가 담당). (4) 부팅 시 `cleanupStaleReviewSummaries` 가 모든 `queued|pending|running` 을 `failed/server_restart` 로 마킹, 곧이어 `rescheduleStaleSummaries` 가 `failed AND errorCode='server_restart'` 행을 placeId/busKey 별로 묶어 자동 재큐잉 — 어드민이 재실행 안 눌러도 직전 진행 중이던 요약이 알아서 재개.
...
| GET    | `summaryEvents`                               | query token   | Multiplexed SSE. `?placeId=…&canonicalId=…&token=<jwt>`. canonicalId 는 묶인 모든 source 의 bus key 를 union 구독. **named `heartbeat` 이벤트 5 초 주기** (comment `: hb` 가 아니라 클라이언트 EventSource 콜백으로 노출되는 이벤트 이름) — FE 가 idle timeout 으로 서버 다운 자동 감지. **(NEW)** `log` 이벤트도 같은 스트림으로 흘려보낸다 — placeId 단위 요약 단계 로그 (jobId/seq/stage/level/message/meta) 동봉. |
| POST   | `cancelSummary(:placeId)`                     | bearer+admin  | **(NEW)** 이 가게의 진행 중 요약 작업 중지. queued/pending 행 → `cancelled`, chain 끊김. 현재 청크는 끝까지. 응답 `{ ok, cancelled: count }`. |
| POST   | `resumeSummary(:placeId)`                     | bearer+admin  | **(NEW)** 직전 중지된 행만 골라 재큐잉. `cancelled` → `queued` flip + chain 재등록. failed 행은 손대지 않음. 응답 `{ ok, resumed: count }`. |
| GET    | `crawlLogs(:placeId)`                         | bearer+admin  | **(NEW)** 이 가게의 누적 크롤+요약 로그 cursor pagination. 같은 가게의 모든 잡(과거 재크롤 포함) 횡단. `?cursor=…&limit=100&level=…&stage=…`. 잡 자체는 in-memory 라 잡 종료 후엔 메타가 사라져도 로그는 placeId 컬럼으로 추적 가능. |
...
| `ReviewSummary` | `review_summaries` | `reviewId @unique`, `status` (**6종 enum**: queued/pending/running/done/failed/cancelled), `sentiment?`, `sentimentScore?`, `satisfactionScore?`, `menusJson?`/`tipsJson?`/`keywordsJson?`, `analysisVersion?` | `ANALYSIS_VERSION = 4`. **`queued`/`cancelled` 2026-05-19 추가** |
| **`CrawlJobLog`** | **`crawl_job_logs`** | **`id`, `jobId`, `placeId?`, `stage`, `level` (info/warn/error), `message`, `meta?`(JSON 직렬화), `createdAt`, `@@index([jobId, createdAt])`, `@@index([placeId, createdAt])`** | **NEW (2026-05-19)**. 크롤+요약 단계별 영속 로그. jobId 단위(잡 패널) + placeId 단위(상세 페이지) 양 축으로 cursor pagination. FK 미선언 — 잡 메타가 휘발돼도 로그는 살아남음 |
...
마이그레이션 (최근순):
- **`20260518014530_add_crawl_job_log`** — **(NEW 2026-05-19)** `CrawlJobLog` 테이블 + (jobId, createdAt) / (placeId, createdAt) 인덱스 2개
...
- **부팅 시 stale 요약 행 정리 + 자동 재큐잉 (`cleanupStaleReviewSummaries` + `rescheduleStaleSummaries`)** — 서버 재시작하면 in-flight 잡은 메모리에서 사라지지만 DB 의 `ReviewSummary.status='queued'|'pending'|'running'` 행이 남아 다음 큐가 그 reviewId 를 다시 못 집는다. `server.ts` 가 `buildApp()` 직후 (1) cleanup 함수가 stale 행을 `status='failed'` + `errorCode='server_restart'` 로 마킹, (2) reschedule 함수가 같은 라운드의 `failed AND errorCode='server_restart'` 행을 placeId(=Naver) / `dc:<vRid>`(=DC) busKey 별로 묶어 `queueSummariesForReviews(key, ids)` 로 재투입한다 — 어드민이 수동 재실행 안 눌러도 직전 진행 중이던 요약이 알아서 이어진다. cleanup 의 where 절은 `queued/pending/running` 셋 다 포함 (2026-05-19 부터 queued 도 합류). **단일 Fastify 인스턴스 가정** (CLAUDE.md "단일 인스턴스 + lru-cache 로 충분") 하에서만 안전 — 같은 DB 를 가리키는 여러 인스턴스가 동시 뜨면 살아있는 인스턴스의 in-flight 잡까지 stale 로 보고 fail 시킬 수 있음.
- **`ReviewSummary.status` enum 6종 — 단계별 의미 분리** — `queued`(`queueSummariesForReviews` 진입 즉시, chain 대기), `pending`(`run()` 진입 후 청크 할당 직전), `running`(LLM 호출 중), `done`/`failed` 최종, `cancelled` (어드민 "요약 중지" 결과 — 부팅 재큐잉에서 제외, reanalyze 가 명시적으로 다시 풀어줘야 됨). queued 도입 동기: chain 휘발 윈도우(이전엔 49 batches × 60s ≈ 49분) 안에 재시작 발생하면 그 사이 적재된 reviewIds 가 통째로 사라지는 사고가 있었다 — placeId=36668856 사례 (446 reviews → 56 done, 381 missing). queued 박는 createMany 가 fire-and-forget 이지만 SQLite writer 가 ms 안에 처리하므로 윈도우가 사실상 0 으로 줄어든다.
- **SummaryService 는 app 전역 singleton (`plugins/summaries.ts`)** — 라우트별 인스턴스 분리 시 `runChainByPlace`/`cancelledPlaces` 가 라우트 단위로 갈라져 "한쪽 라우트가 큐잉한 batch 를 다른 라우트의 cancel 이 못 끊는" 버그가 났다. `fastify-plugin` + `app.decorate('summaries' | 'jobLog' | 'aiConfig', ...)` + `dependencies: ['prisma']` 로 prisma 뒤 순서 강제. 라우트는 `app.summaries` / `app.jobLog` / `app.aiConfig` 로 읽기만 한다.
- **CrawlJobLog 시스템 — 한 진입점 / 세 채널 / `(jobId, seq)` dedup** — `JobLogService.log({ channel, ... })` 가 (1) pino, (2) `prisma.crawlJobLog.create` fire-and-forget, (3) SSE — `channel='crawl'` 이면 `jobRegistry` + (placeId 있으면) `summaryEventsBus` 양쪽 fan-out, `channel='summary'` 면 `summaryEventsBus` 단방향 — 까지 한 호출에 흘려보낸다. 같은 로그가 두 SSE 양쪽으로 가도 같은 모노톤 `seq` 가 박혀 클라이언트가 `(jobId, seq)` Map 으로 dedup. 크롤 SSE 가 done 으로 닫혀도 어드민 UI 가 `summaryEventsBus` 로 placeId 단위 요약 단계 로그를 계속 받는다. DB 영속화가 분리돼 잡 종료 후 패널 재진입 시 전체 로그 복원 가능. SSE 실패와 DB 실패는 서로 막지 않음.
