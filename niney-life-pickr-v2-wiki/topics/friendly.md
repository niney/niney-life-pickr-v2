...
status: active
aliases: [naver-search-adapter, search-route, crawl-job-log, plugins-summaries, settlement, 정산, multipart, vision LLM, 단골, contacts, llm-purpose, settlement-rounds, settlement-draft, settlement-draft-module, well-known, well-known-module, universal-links, app-links, assetlinks-json, AASA, RFC1918, cors-dev, dev-cors-private-lan, multi-receipt-split, ExtractReceiptSplit, roundIndex, roundTotal, settlement-PUT, full-replace-update, ai-model-preview, models-preview, attendees-100, items-200, calculateMultiRoundShares, SettlementRound, SettlementRoundAttendee, SettlementDraft, placeIdKey-sentinel, fromDraftId]
sources_count: 88
---
...
- [plugins/cors.ts](../../apps/friendly/src/plugins/cors.ts) — `env.CORS_ORIGIN`이 `*`이면 `true`, 아니면 콤마 분리. `credentials: true`. **(2026-05-28)** dev (`isDev`) 에선 `PRIVATE_LAN_ORIGIN` regex (`localhost` / `127.0.0.1` / `10.x.x.x` / `192.168.x.x` / `172.16~31.x.x`, optional `:port`) 가 매칭되는 origin 을 자동 허용 — `.env` 에 LAN IP 를 박지 않아도 폰이 LAN IP 로 띄운 Expo Web 에서 friendly API 로 호출이 통과. production 은 regex 분기 자체가 없어 env CORS_ORIGIN 만 사용.
...
- **dev CORS RFC1918 자동 허용 — production 미적용** — `isDev` 분기 안에서만 regex 적용. production 은 env 명시 origin 만 통과. `.env` 에 dev 세션마다 바뀌는 LAN IP 를 박지 않아도 폰의 Expo Web 이 friendly API 를 호출 가능.
...
- **dev CORS regex 는 production 에서 안 돈다** — `isDev` 분기 안에서만 등록되므로 prod 환경에서 LAN IP 로 접속하는 시나리오를 가정하지 말 것. prod 에서 LAN 단말을 허용하려면 `CORS_ORIGIN` 에 명시 추가.
