...
└── src/
    ...
    ├── routes.ts         # API_PREFIX + 도메인별 경로 상수 (Auth/Users/Picks/Admin/Media/Crawl/Restaurant/Canonical/Analytics/AutoDiscover/Ai/SettingsMap/SettlementExtraction/Settlement/SettlementContact/Health)
    ├── settlement.calculator.ts # **신규** FE/BE 공통 분배 알고리즘 — 카테고리별 풀 + 제외 플래그 → shareAmounts[]. 순수 함수, calculateShares export
    └── schemas/
        ...
        ├── auto-discover.ts         # 맛집 자동 발견 잡 — AI 키워드 8개 → 다중 검색 → 그룹 5병렬 크롤. state(잡 상태) + phase(단계) 두 enum 분리
        ├── analytics.ts             # 글로벌(식당 가로지르기) 메뉴 통계 + 머지 잡 + category tree (z.lazy 재귀)
        ├── ai.ts                    # AI 호출 + 배치 + LLM Provider 관리 (**purpose=chat/image enum 추가**)
        ├── settings-map.ts          # 외부 지도 SDK provider config (admin + public reveal)
        ├── settlement-extraction.ts # **신규** 영수증 업로드(token)/추출(vision LLM) + ReceiptItemCategory enum (ALCOHOL/NON_ALCOHOL/SIDE/UNCATEGORIZED)
        ├── settlement.ts            # **신규** 정산 세션 — session/participant/item, create/list/share/shared/updateParticipants 입출력. editedAt nullable 필드 포함
        └── settlement-contact.ts    # **신규** 사용자별 단골 참여자 CRUD — list/update + lastExclude* 기억
```
...
이번 라운드에 정산 도메인 3 파일 (`settlement-extraction.ts`, `settlement.ts`,
`settlement-contact.ts`) + `settlement.calculator.ts` 가 추가됐다.
...
신규 정산 패밀리도 비슷한 좁은 의존: `settlement.ts` 가 `settlement-extraction.ts` 의
`ReceiptItemCategory` 만 import (영수증 추출 → 세션 저장이 같은 enum 을 쓰므로 중복
선언 회피). `settlement-contact.ts` 는 자체 타입만 — 단골 row 의 모양은 세션 참여자와
독립이다. `settlement.calculator.ts` 는 `settlement.ts` 의 `SettlementItemInputType` +
`SettlementParticipantInputType` 와 `settlement-extraction.ts` 의 `ReceiptItemCategoryType`
세 타입만 type import 로 잡고, 런타임 zod 스키마에는 의존하지 않는다 (순수 계산이므로).
...
## API Surface [coverage: high — 18 sources]
...
- **ai**: `AiCompleteInput/Result`, `AiCompleteBatchInput/Result`,
  ...
  `LlmModelListResult`. **이번 라운드 신규**: `LlmProviderPurpose` enum (`'chat' | 'image'`) +
  `LlmProviderConfig.purpose` 필드. provider 식별이 `provider` 단일 키 → `(provider, purpose)`
  복합 키로 바뀌어 같은 ollama-cloud row 가 chat 용/image 용 두 벌로 존재할 수 있다.
  env fallback 은 `chat` 에만 적용, `image` 는 DB row 가 있을 때만 활성화. `Routes.Ai.provider/
  testProvider/providerModels` 시그니처도 `(id, purpose)` 두 인자로 변경.
- **settings-map**: `MapProviderConfig/ListResult`, `UpdateMapProviderInput`,
  `MapProviderSecret` (admin), `MapProviderPublicConfig` (public).

### `schemas/settlement-extraction.ts` — [settlement-extraction.ts](../../packages/api-contract/src/schemas/settlement-extraction.ts) **(신규)**

영수증 사진을 vision LLM 으로 항목화. settlement 세션 생성 직전 단계. 인증된
USER+ 만 사용 가능 (정산 자체가 로그인 사용자 기능).

| Export | 용도 |
| --- | --- |
| `ReceiptItemCategory` | `'ALCOHOL' \| 'NON_ALCOHOL' \| 'SIDE' \| 'UNCATEGORIZED'`. 분배 시 참여자별 제외 플래그(주류 X 등) 의 풀 분리 기준 |
| `ReceiptItem` | `{ name(1~120), unitPrice(int>=0\|null), quantity(int>0\|null), amount(int>=0), category, matchedMenuName(string\|null) }`. unitPrice/quantity 추출 실패해도 amount 는 fallback |
| `UploadReceiptResult` | `{ imageToken, previewUrl, byteSize }`. 클라이언트는 절대 경로/파일명 없이 token 만 보관 — 임의 경로 주입 방지 |
| `ExtractReceiptInput` | `{ imageToken, placeId }`. placeId 는 LLM 프롬프트에 해당 식당 메뉴 힌트 주입에 필수 |
| `ExtractReceiptResult` | `{ items: ReceiptItem[], totalAmount(int\|null), itemsSubtotal, warning(string\|null), model }`. warning 은 소계 vs 총금액 불일치 경고 메시지 |

### `schemas/settlement.ts` — [settlement.ts](../../packages/api-contract/src/schemas/settlement.ts) **(신규)**

정산 세션 — 로그인 사용자 본인만. items 는 저장 후 불변, participants 만 수정 가능
(`UpdateSettlementParticipantsInput`). share 토큰으로 공개 read-only 공유 가능 —
공개 응답에선 `userId`/`receiptPreviewUrl` 가 빠진다.

| Export | 용도 |
| --- | --- |
| `SettlementSource` | `'MANUAL' \| 'RECEIPT'`. 세션 생성 경로 식별 |
| `SettlementItem` / `SettlementItemInput` | 한 항목(메뉴). `{ id, name(1~120), unitPrice, quantity, amount, category: ReceiptItemCategory, matchedMenuName, orderIndex }`. Input 은 `id`/`orderIndex` 생략 (서버 부여) |
| `SettlementParticipant` / `SettlementParticipantInput` | 참여자. `{ id, name, nickname, excludeAlcohol, excludeNonAlcohol, excludeSide, shareAmount, orderIndex, contactId }`. Input 은 `id/shareAmount/orderIndex` 생략 + `contactId` optional 힌트 (서버는 신뢰하지 않고 normalizedKey 로 upsert) |
| `SettlementSession` | 세션 전체. `{ id, userId, restaurantPlaceId, restaurantName, source, totalAmount, warning, receiptPreviewUrl, itemsSubtotal, items[], participants[], createdAt, updatedAt, editedAt(nullable) }`. `editedAt` 은 저장 후 participants 가 수정된 마지막 시각 — 한 번도 안 고치면 null. 공유 페이지의 '수정됨' 배지 |
| `CreateSettlementInput` | `{ restaurantPlaceId, source, totalAmount, warning, receiptImageToken(nullable), items(1~100), participants(1~20) }` |
| `UpdateSettlementParticipantsInput` | `{ participants(1~20) }`. items 는 불변 — 서버가 기존 items 와 결합해 shareAmount 재계산 |
| `ListSettlementsQuery` | `{ placeId?, offset(coerce, >=0, def 0), limit(1~50, def 20) }` |
| `SettlementSessionSummary` | 목록 행 — items/participants 본문 빼고 카운트만. `{ id, restaurantPlaceId, restaurantName, source, totalAmount, itemsSubtotal, itemCount, participantCount, createdAt }` |
| `ListSettlementsResult` | `{ items: SettlementSessionSummary[], total }` |
| `SettlementShare` | `{ token(nullable), shareUrl(nullable) }`. 회수 직후엔 둘 다 null. shareUrl 은 API 경로 — 클라이언트가 origin 을 붙여 사용 |
| `SharedSettlementSession` | `SettlementSession.omit({ userId, receiptPreviewUrl })` — 공개 공유 응답. 토큰 받은 사람도 원본 사진은 못 본다 |

### `schemas/settlement-contact.ts` — [settlement-contact.ts](../../packages/api-contract/src/schemas/settlement-contact.ts) **(신규)**

사용자별 단골 참여자 — 정산 저장 시 (userId, normalizedKey) 로 upsert. 자동완성과
`/me/contacts` 관리 페이지가 호출. `normalizedKey` 는 서버 내부 매칭 키로 클라에는
노출하지 않는다.

| Export | 용도 |
| --- | --- |
| `SettlementContact` | `{ id, name, nickname, lastExcludeAlcohol, lastExcludeNonAlcohol, lastExcludeSide, useCount, lastUsedAt, createdAt, updatedAt }`. `lastExclude*` 는 가장 최근 정산의 선택 — 다음 정산의 default 로 자동 제안 |
| `ListContactsQuery` | `{ q?(trim, 부분일치), take(1~100, def 50) }`. 관리 페이지는 한 번에 받아 인메모리 필터링 |
| `ListContactsResult` | `{ items, total }` |
| `UpdateContactInput` | `{ name(trim, <=40, nullable), nickname(trim, <=40, nullable) }`. normalizedKey 충돌 시 서버 409. `lastExclude*` 는 수정 대상 아님 — 다음 정산이 자연 덮어쓰기 |

### `settlement.calculator.ts` — [settlement.calculator.ts](../../packages/api-contract/src/settlement.calculator.ts) **(신규)**

FE/BE 공통 분배 계산기 — 순수 함수. schemas/ 가 아닌 패키지 루트에 둔다.

| Export | 용도 |
| --- | --- |
| `CalculateInput` | `{ items: Pick<SettlementItemInputType, 'amount' \| 'category'>[], participants: Pick<SettlementParticipantInputType, 'excludeAlcohol' \| 'excludeNonAlcohol' \| 'excludeSide'>[] }` |
| `CalculateOutput` | `{ shareAmounts: number[](각 참여자별, int>=0, 입력 순서), itemsSubtotal, poolBreakdown: Record<카테고리, { poolAmount, participantCount, perParticipant }> }` |
| `calculateShares(input)` | 메인 함수. 카테고리별 풀 → 제외 플래그 없는 참여자에게 균등 분배. ALCOHOL/NON_ALCOHOL/SIDE 의 전원 제외 풀은 모두에게 균등 fallback. UNCATEGORIZED 는 항상 전원 균등. 1원 단위 나머지는 첫 참여자에게 |

분배 규칙 요약:
- **풀 분리** — items.amount 합을 4 카테고리 풀로 쪼갠다.
- **풀별 참여자 산정** — ALCOHOL 풀이면 `excludeAlcohol=false` 인 참여자만, NON_ALCOHOL/SIDE 도 동일. UNCATEGORIZED 풀은 항상 전원.
- **전원 제외 풀 fallback** — 주류 항목이 있는데 모두가 `excludeAlcohol=true` 면 그 풀은 비활성, 금액을 전원에게 균등 분배 (=UNCATEGORIZED 처럼 처리).
- **나머지 흡수** — 풀별 `floor(pool/n)` 분배 후 1원 단위 잔액은 첫 참여자에게 가산해 합이 풀과 일치.
...
| `Ai` | complete, completeBatch, providers, **provider(id, purpose)**, **testProvider(id, purpose)**, **providerModels(id, purpose)** | `/admin/ai/...` (provider 경로 끝에 `/:purpose` 추가) |
...
| **`Settlement`** | **list, create, one(id), updateParticipants(id), share(id), shared(token)** | `/settlements/...` + `/share/settlements/:token` |
| **`SettlementContact`** | **list, one(id)** | `/me/contacts[/:id]` |
| `Health` | (단일 상수) | `/health` |

**이번 라운드 (2026-05-25) 신규 라우트군:**

- `Routes.SettlementExtraction.*` — 영수증 업로드 → vision LLM 추출 → 미리보기. `upload`
  가 POST multipart 로 imageToken 발급, `extract` 가 token + placeId 받아 ReceiptItem[]
  반환. `preview(token)` 은 같은 사용자의 JPEG 만 응답 (인증 필요).
- `Routes.Settlement.*` — 정산 세션 CRUD + 공유. `list/create/one(id)` 은 본인만,
  `updateParticipants(id)` 는 items 불변·participants 만 갱신, `share(id)` 는 owner 가
  공유 토큰 생성·회수, `shared(token)` 은 공개 read-only (`/share/settlements/:token`
  — `/api/v1/settlements/...` 아래가 아니라 별도 트리 — 인증 미들웨어 분기 단순화).
- `Routes.SettlementContact.*` — 단골 참여자 `/me/contacts[/:id]`. 자동완성과 관리 페이지가
  공유. 본인 데이터만 GET/PATCH/DELETE.
- `Routes.Ai.provider(id, purpose)` (시그니처 변경) — `(id)` → `(id, purpose)` 두 인자.
  같은 ollama-cloud row 가 chat/image 두 벌로 존재. `testProvider`/`providerModels` 도 동일.
...
- **정산 calculator 를 api-contract 에 둠** — `settlement.calculator.ts` 는 schemas 가 아니지만
  의도적으로 같은 패키지. 이유: 정산 결과(`shareAmounts`)를 FE 가 입력 변경 즉시 미리보기로
  보여주고, BE 도 같은 알고리즘으로 검증·영속화한다 — **둘이 어긋나면 사용자가 보는 값과
  저장된 값이 달라지는 silent breaking**. 알고리즘이 ts 1 파일로 끝나는 순수 함수라
  shared 로 끌어올릴 필요 없이 contract 패키지에 직접 두는 게 가장 작은 변경. 의존 방향은
  calculator → schemas type-only import 만 (런타임 zod 의존 없음).
- **`LlmProviderPurpose` enum 도입 — chat vs image 분리** — 같은 provider 인데 텍스트 모델과
  vision 모델은 거의 항상 다르고, `defaultModel/maxConcurrent` 도 따로 잡고 싶다. 한 row 에
  `chatModel/imageModel` 두 컬럼을 두는 대신 row 자체를 `(provider, purpose)` 복합 키로
  쪼갰다 — 영수증 추출(`extract`) 같은 vision 호출은 `purpose='image'` row 를 별도 조회.
  env fallback 은 `chat` 에만 — `image` 는 사용자가 명시적으로 DB row 를 만들어야 활성화
  되므로 LLM 비용을 우발적으로 발생시키지 않는다. 라우트도 `Routes.Ai.provider(id, purpose)`
  로 함수 시그니처를 바꾸어 호출부에서 purpose 누락 시 컴파일 에러로 잡힘.
- **`SettlementSession.editedAt` 을 nullable 로** — 저장 후 한 번도 안 고쳤으면 null, 한
  번이라도 `updateParticipants` 가 돌면 그 시각. 단순 boolean (`isEdited`) 대신 시각을
  실어 보내 공유 페이지의 '수정됨' 배지가 "언제" 까지 같이 보일 수 있게 함. createdAt /
  updatedAt 과 의미가 다르다 — `updatedAt` 은 어떤 변경이든 갱신되지만 `editedAt` 은
  사용자의 명시적 수정(participants 만) 한정. items 불변 정책과 짝.
- **참여자 옵션은 `excludeXxx` 3개 boolean** — 풀 카테고리(`ALCOHOL/NON_ALCOHOL/SIDE`) 1:1
  매칭. UNCATEGORIZED 에는 대응하는 exclude 가 없다 (전원 부담). enum 한 줄에 `'none'` 같은
  표시값을 추가하는 대신 boolean 3개로 분리한 이유 — 직교 조합을 사용자가 자유롭게 켜고
  끌 수 있어야 한다 ("주류는 빼지만 사이드는 같이 부담"). `EXCLUDE_KEY` 매핑은
  calculator 가 단일 SSOT.
...
- **`Routes.Ai.X` namespace re-export 가 vite esbuild prebundle 에서 깨질 수 있음** —
  `Routes.Canonical`, `Routes.SettingsMap`, `Routes.Analytics`, `Routes.AutoDiscover`,
  `Routes.Settlement`, `Routes.SettlementExtraction`, `Routes.SettlementContact` 도
  동일 위험. 회피책으로 friendly 측은 `const CanonicalRoutes = Routes.Canonical` 로 한
  단계 우회.
- **`Routes.Ai.provider(id)` → `provider(id, purpose)` (breaking)** — 호출부가 1 인자만
  넘기면 TS 컴파일 에러. 모든 호출 지점에서 `'chat'` 또는 `'image'` 를 명시해야 한다.
  `testProvider` / `providerModels` 도 같은 시그니처 변경.
- **`Routes.Settlement.shared(token)` 만 `/share/...` prefix** — 다른 정산 라우트는
  `/settlements/...` 인데 공개 공유 만 `/share/settlements/:token`. Fastify 인증 미들웨어가
  path prefix 로 분기하므로 의도된 분리. `/settlements/share/:token` 처럼 보이지 않게 주의.
- **`SettlementParticipantInput.contactId` 는 힌트일 뿐** — 클라가 자동완성에서 단골을
  골라 보내도 서버는 무시하고 `(userId, normalizedKey)` 로 upsert. 즉 같은 키면 같은 row
  로 합쳐지므로 stale contactId 가 와도 정상 동작. UI 가 "같은 이름이 두 row 로 보임" 같은
  안티패턴을 만들 수 없다.
- **`calculateShares` 의 전원 제외 풀 fallback 동작** — 주류 항목이 있는데 전원이
  `excludeAlcohol=true` 면 그 풀 금액이 0 으로 떨어지는 게 아니라 **전원에게 균등 분배**.
  "아무도 안 먹은 항목" 을 0 원 처리하려면 항목 자체를 삭제하거나 UNCATEGORIZED 로
  옮겨야 한다. UI 가 이 의미 차이를 사용자에게 전달하지 않으면 의외의 결과를 줄 수 있음.
- **정산 도메인 상세 설명은 `settlement.md` 에서** — api-contract 토픽은 스키마 export 목록
  + 라우트 namespace + design decision 만 다룬다. 영수증 추출 파이프라인 / share 토큰
  라이프사이클 / FE 입력 stepper / 단골 자동완성 UX 같은 도메인 흐름은
  [settlement.md](settlement.md) 토픽 참조.
