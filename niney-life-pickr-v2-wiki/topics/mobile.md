...
추가로 **관리자(ADMIN) 한정**으로 "맛집" 탭과 식당 상세 화면을 제공한다 — 등록된 식당 리스트와 메뉴 순위(SentimentBar + 글로벌 비교 + 미분류 메뉴 분류 트리거)까지를 모바일에서 직접 본다 ([restaurants.tsx](../../apps/mobile/app/(tabs)/restaurants.tsx), [restaurant/[placeId].tsx](../../apps/mobile/app/restaurant/[placeId].tsx), [MenuRankingCard.tsx](../../apps/mobile/src/components/MenuRankingCard.tsx)). 매장/메뉴/리뷰 크롤·관리 같은 본격 어드민은 여전히 web 전용이다.
...
분류·랭킹 로직은 모바일에 없다 — 전부 [`@repo/shared`](shared.md)의 훅이 들고 있고, mutation 성공 시 invalidate도 shared 쪽이 책임진다. 모바일은 sort 상태를 `useState`로 들고, 데이터 가공은 `items.slice(0, 5)`/flex 비율 계산 정도만 한다.
...
- **본격 어드민은 web 전용, 모바일은 ADMIN read-only 보기만** — `useRestaurantList`는 admin API라 비-ADMIN/게스트는 화면 자체에서 차단(안내 텍스트). 매장 등록·리뷰 크롤·요약 트리거 같은 쓰기 액션은 web에 둔다. 단 `MenuRankingCard`의 "분류하기" 버튼만은 예외로 mutation을 직접 호출 — 분류 결과 자체는 사용자에게 즉시 보이는 UX라 모바일에서 트리거할 가치가 있음
- **메뉴 순위 비즈니스 로직은 [`@repo/shared`](shared.md)에, UI 비율 계산만 모바일에** — 정렬/필터(`minMentions: 2`)/그룹핑/[menu-grouping](menu-grouping.md) invalidate는 shared 훅이, RN flex 비율(`pos`/`neu`/`100 - pos - neu`)과 sort 상태 `useState`만 모바일에. web의 메뉴 순위 카드와 동일한 데이터 모델을 RN UI로 다시 그리는 구조
...
- **모바일은 light only** — `ThemeProvider mode="light"` 고정. web/admin은 다크 모드 토글이 있지만 모바일은 미적용이라 `MenuRankingCard` 등도 단순 inline 컬러(`#1e293b`/`#10b981`/`#ef4444`/`#fef3c7` 등)로 박혀 있음
