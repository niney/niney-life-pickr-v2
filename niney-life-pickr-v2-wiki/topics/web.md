...
- **지역 칩 2행**([TourFilterBar](../../apps/web/src/components/tour/TourFilterBar.tsx)) — 7차에 10키였던 지역이 9차에 **21키**가 되면서 칩을 한 줄로 늘어놓을 수 없게 됐다. 1행은 권역(제주·서부권·동부권·수도권·전체), 2행은 고른 권역의 시도 + "〈권역〉 전체". 목록은 utils `TOUR_REGION_GROUPS` 에서 파생되므로 권역이 늘어도 UI 코드는 그대로다(테스트 4건이 이 규칙을 고정).
- **일상지도 배경 레이어 UI** — `LifeLayerBar` 에 배경 토글이 생기고([LifeCrimeCard](../../apps/web/src/components/life-map/LifeCrimeCard.tsx)·[LifeTourCard](../../apps/web/src/components/life-map/LifeTourCard.tsx)), 면 색칠은 [lifeMapAreas.ts](../../apps/web/src/components/life-map/lifeMapAreas.ts) + 시군구 경계 [useSigunguGeo](../../apps/web/src/lib/useSigunguGeo.ts) / 격자 [tourDensityGeo](../../apps/web/src/lib/tourDensityGeo.ts). 배경은 한 번에 하나(`overlay`)이고 prefs 는 v5 로 올라갔다([life-map](life-map.md)).
- **맛집 상세·목록** — 탭 배열에 `'tour'`("여행자")가 추가되되 `PublicRestaurantDetail` 이 `detail.tour !== null` 일 때만 노출하고([TourTab](../../apps/web/src/components/restaurant/detail/TourTab.tsx)·`TourSummaryBadge`·`TourMatchBadge`), 상가 매칭은 [StoreInfoBadges](../../apps/web/src/components/restaurant/detail/StoreInfoBadges.tsx)(폐업 의심 배지). 목록엔 정렬 칩 `tourTravelers|tourScore`, 골라줘엔 `traveler` 전략.
- **어드민** — [AdminTourPage](../../apps/web/src/routes/admin/AdminTourPage.tsx)(데이터셋별 적재 상태·시드 표·discover/register·매칭·폐업 조회)와 식당 상세의 [TourEvidencePanel](../../apps/web/src/components/admin/tour/TourEvidencePanel.tsx)("여행자 근거" — 원본 열람 allowlist 안에서만 200), AI 키 화면의 추론(thinking) select([ai](ai.md)). `b9da676` 이 시드 화면을 실측 보정(표 줄바꿈·상태 카드·필터 라벨)했다.
- **사주(C)** — 풀이 탭이 그룹으로 재편(`sajuPanelTabs.ts`)되고 테마 3종(`SajuThemes`)·"사주에 묻기"(`SajuAsk`)·궁합 입구 모드(`SajuPairPanel`)가 붙었다([saju-c](saju-c.md)).
...
### 여행로그 화면 — `/travel/*` · 어드민 시드 콘솔 · 배경 레이어 [신규 — 26차]

**`/travel/jeju`([TravelInsightsPage](../../apps/web/src/routes/TravelInsightsPage.tsx))** — 경로 이름은 `jeju` 로 굳었지만 실제 축은 쿼리(`?region=`)다(7~9차에 지역이 21키로 늘었어도 경로는 그대로 — 기존 링크 호환). 필터(`region`·`ageGrp`·`gender`·`accompany`·`month`·`nights`)가 **URL 쿼리 = 진실**이라 새로고침·공유가 그대로 복원되고, 칩을 누르면 `setSearchParams` → 훅 키가 바뀌어 재조회된다. 제목·설명·거점 섹션 제목이 `tourRegionLabel(region)`·응답의 `hubLabel` 로 바뀌어 "제주"가 하드코딩돼 있지 않다. 표본이 20건 미만이면 서버가 `insufficient` 를 주고 화면은 "표본이 적어요" 안내로 대체한다. 지역 비교([TourRegionSection](../../apps/web/src/components/tour/TourRegionSection.tsx))는 제주면 3집단(제주시·서귀포시·부속섬), 시도·권역이면 시군구 상위 8집단이라 집단 키가 매번 달라 막대 색을 **순서 인덱스**(`TOUR_GROUP_BAR_COLORS`)로 돌려 쓴다. 숙소 통계는 [TourLodgingSection](../../apps/web/src/components/tour/TourLodgingSection.tsx), 출처 문구는 [TourSourceNote](../../apps/web/src/components/tour/TourSourceNote.tsx)(utils `TOUR_SOURCE_NOTE` — 데이터명 4개 자동).

**`/travel/plan`([TravelPlanPage](../../apps/web/src/routes/TravelPlanPage.tsx))** — 연령·동반·박수·지역을 고르고 "비슷한 여행 찾기" → 장소 카드(`n팀 · 만족 x.x` + 점수 막대). 조건이 좁으면 서버가 `month→gender→nights→ageGrp` 순으로 풀고 "표본이 적어 … 조건은 풀었습니다" 를 돌려준다. **등록된 맛집만 체크 가능**하고(미등록은 "미등록" 배지), 2~8곳을 고르면 `navigate('/vote/new', { state: { presetTitle, presetOptions } })` 로 그룹투표 생성 폼을 채운다([vote](vote.md)). 결과 토글("전체 ↔ 식당만")은 지역 칩의 "전체"와 라벨이 겹쳐 셀렉터를 쓸 때 주의(Gotchas).

**지역 칩 2행**([TourFilterBar](../../apps/web/src/components/tour/TourFilterBar.tsx), [tourFormat.ts](../../apps/web/src/components/tour/tourFormat.ts)) — 1행 권역 4칩(utils `TOUR_REGION_GROUPS` 에서 파생) → 2행은 고른 권역의 시도 + "〈권역〉 전체". 값은 여전히 `region` 키 **하나**라 URL·API 계약은 안 바뀐다. `tourRegionGroupOf(region)` 이 "지금 어느 권역 줄을 펼칠지"를 정한다. [TourFilterBar.test.tsx](../../apps/web/src/components/tour/TourFilterBar.test.tsx) 4건이 (제주 기본=시도 행 없음 / 대전 선택 시 서부권 활성 + 서부권 시도만 / 동부권 6시도 + 시도 클릭이 region 하나로 나감 / 전체는 시도 행 없음)을 고정.

**어드민 시드 콘솔**([AdminTourPage](../../apps/web/src/routes/admin/AdminTourPage.tsx)) — 상단 Stat 카드(적재: 데이터셋별 장소 수 · 매칭 · 폐업 조회)와 지역 필터(전체/제주·도서/서부권/동부권/수도권 — utils `TOUR_DATASETS` 에서 자동), 아래는 장소 단위 집계 표(여행자·방문·만족·지출·유형·주소). 행의 "발굴"은 네이버 검색(`discover`) → 후보 선택 → `register`(기존 크롤 잡), "근거"는 [TourEvidencePanel](../../apps/web/src/components/admin/tour/TourEvidencePanel.tsx)(방문·주문·영수증·사진·포함 여행 — 원본 열람 allowlist 밖이면 404 라 패널이 안내를 띄운다). 사진은 헤더를 못 실어 `tourPhotoUrl(base, id, size)` 의 `?token=` URL 을 쓴다([shared](shared.md)).
...
- **2026-09-12 배경 레이어는 카드까지 배타로**(`bc39a79`·`9196495`) — 배경을 바꾸면 요약 카드도 통째로 바뀐다(범죄 ↔ 여행자 밀도). 두 카드를 동시에 띄우거나 선택(시군구/칸)을 유지하지 않는다 — 같은 화면 자리에서 뜻이 다른 선택을 이어 받으면 사용자가 무엇을 보고 있는지 잃는다.
...
- **여행 원본 근거 패널은 권한이 있어도 404 가 정상 응답** — `TourEvidencePanel` 은 `TOUR_RAW_USER_IDS` 밖이면 404 를 받는다(403 이 아니다). "관리자인데 안 보인다"는 대개 `.env` allowlist 누락이거나 이메일을 넣었는데 배포본이 id 만 받는 구버전인 경우([friendly](friendly.md)).
...
- [apps/web/src/components/tour/TourFilterBar.tsx](../../apps/web/src/components/tour/TourFilterBar.tsx) (+[test](../../apps/web/src/components/tour/TourFilterBar.test.tsx) 4건 — 권역/시도 2행 칩) · [tourFormat.ts](../../apps/web/src/components/tour/tourFormat.ts) · [charts.tsx](../../apps/web/src/components/tour/charts.tsx) · [TourRegionSection.tsx](../../apps/web/src/components/tour/TourRegionSection.tsx) · [TourLodgingSection.tsx](../../apps/web/src/components/tour/TourLodgingSection.tsx) · [TourSourceNote.tsx](../../apps/web/src/components/tour/TourSourceNote.tsx)
- [apps/web/src/routes/admin/AdminTourPage.tsx](../../apps/web/src/routes/admin/AdminTourPage.tsx) (NEW) · [components/admin/tour/TourEvidencePanel.tsx](../../apps/web/src/components/admin/tour/TourEvidencePanel.tsx) (NEW) · [routes/admin/AdminRoutes.tsx](../../apps/web/src/routes/admin/AdminRoutes.tsx)·[AdminLayout.tsx](../../apps/web/src/components/admin/AdminLayout.tsx)·[AdminTopBar.tsx](../../apps/web/src/components/admin/AdminTopBar.tsx) — *modified: `/admin/tour` 등록·사이드바*
- [apps/web/src/components/restaurant/detail/TourTab.tsx](../../apps/web/src/components/restaurant/detail/TourTab.tsx)·[TourSummaryBadge.tsx](../../apps/web/src/components/restaurant/detail/TourSummaryBadge.tsx)·[TourMatchBadge.tsx](../../apps/web/src/components/restaurant/detail/TourMatchBadge.tsx)·[StoreInfoBadges.tsx](../../apps/web/src/components/restaurant/detail/StoreInfoBadges.tsx) (+[test](../../apps/web/src/components/restaurant/detail/StoreInfoBadges.test.tsx)) · [tabs.ts](../../apps/web/src/components/restaurant/detail/tabs.ts)·[PublicRestaurantDetail.tsx](../../apps/web/src/components/restaurant/detail/PublicRestaurantDetail.tsx)·[HomeTab.tsx](../../apps/web/src/components/restaurant/detail/HomeTab.tsx)
- [apps/web/src/components/life-map/LifeCrimeCard.tsx](../../apps/web/src/components/life-map/LifeCrimeCard.tsx) (NEW) · [LifeTourCard.tsx](../../apps/web/src/components/life-map/LifeTourCard.tsx) (NEW) · [lifeMapAreas.ts](../../apps/web/src/components/life-map/lifeMapAreas.ts) (NEW) · [lib/useSigunguGeo.ts](../../apps/web/src/lib/useSigunguGeo.ts) (NEW) · [lib/tourDensityGeo.ts](../../apps/web/src/lib/tourDensityGeo.ts) (NEW) · [LifeLayerBar.tsx](../../apps/web/src/components/life-map/LifeLayerBar.tsx)·[LifeMapView.tsx](../../apps/web/src/components/life-map/LifeMapView.tsx)·[LifeMapFooter.tsx](../../apps/web/src/components/life-map/LifeMapFooter.tsx)·[LifeDetailCard.tsx](../../apps/web/src/components/life-map/LifeDetailCard.tsx)·[LifeNearbyList.tsx](../../apps/web/src/components/life-map/LifeNearbyList.tsx)·[lifeMapMarkers.ts](../../apps/web/src/components/life-map/lifeMapMarkers.ts)·[routes/LifeMapPage.tsx](../../apps/web/src/routes/LifeMapPage.tsx) (+[test](../../apps/web/src/routes/LifeMapPage.test.tsx))·[stores/lifeMapPrefsStore.ts](../../apps/web/src/stores/lifeMapPrefsStore.ts)(v5)
- [apps/web/src/components/saju/SajuAsk.tsx](../../apps/web/src/components/saju/SajuAsk.tsx) (NEW) · [SajuThemes.tsx](../../apps/web/src/components/saju/SajuThemes.tsx) (NEW) · [SajuPairPanel.tsx](../../apps/web/src/components/saju/SajuPairPanel.tsx) (NEW) · [sajuPanelTabs.ts](../../apps/web/src/components/saju/sajuPanelTabs.ts) (NEW) · `SajuForm`·`SajuReadingPanel`·`SajuReadingView`·`SajuTools`·[routes/SajuPage.tsx](../../apps/web/src/routes/SajuPage.tsx)(+test)·[TarotPage.tsx](../../apps/web/src/routes/TarotPage.tsx)
- [apps/web/src/App.tsx](../../apps/web/src/App.tsx)·[components/PublicSidebar.tsx](../../apps/web/src/components/PublicSidebar.tsx)·[PublicTopBar.tsx](../../apps/web/src/components/PublicTopBar.tsx)·[routes/HomePage.tsx](../../apps/web/src/routes/HomePage.tsx) — *modified: `/travel/*` 라우트 + "여행" NAV*
- [apps/web/src/routes/vote/VoteNewPage.tsx](../../apps/web/src/routes/vote/VoteNewPage.tsx) — *modified: 코스 추천 프리필(`location.state`) — [vote](vote.md)*
- [apps/web/src/routes/RestaurantsV2Page.tsx](../../apps/web/src/routes/RestaurantsV2Page.tsx)·[components/restaurant/PublicRestaurantCard.tsx](../../apps/web/src/components/restaurant/PublicRestaurantCard.tsx)·[PublicRestaurantList.tsx](../../apps/web/src/components/restaurant/PublicRestaurantList.tsx)·[SmartPickSection.tsx](../../apps/web/src/components/restaurant/SmartPickSection.tsx)·[MapCanvas.tsx](../../apps/web/src/components/restaurant/MapCanvas.tsx) — *modified: 여행자 정렬 칩·골라줘 traveler·카드 메타*
- [apps/web/src/routes/admin/AdminAiKeysPage.tsx](../../apps/web/src/routes/admin/AdminAiKeysPage.tsx) — *modified: 추론(thinking) select — [ai](ai.md)* · [AdminRestaurantDetailPage.tsx](../../apps/web/src/routes/admin/AdminRestaurantDetailPage.tsx) — *modified: 여행자 근거 카드*
