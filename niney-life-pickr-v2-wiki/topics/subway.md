...
- **FE 파생 로직 승격 2건(2026-08-17)** — (a) **혼잡도(`9206346`)**: 웹·앱 [congestionUtils.ts](../../apps/web/src/components/subway/congestionUtils.ts) 가 문자 단위 복제하던 임계표(여유<40/보통<80/붐빔<120/혼잡)·30분 슬롯 키(24+ 익일 접기)·도착/시간표 updn ↔ 혼잡 방향 조인·slotLevel 을 `@repo/utils` [subwayCongestion](../../packages/utils/src/subwayCongestion.ts) 단일 정의로. 웹·앱 파일은 **표현만 남는 facade**(웹 Tailwind 클래스 / 앱 hex 토큰 — 밴드 라벨 키 테이블이라 임계를 바꿔도 두 플랫폼 색이 어긋날 수 없다), 소비처 4곳 import 경로 무변경. utils 테스트 7건이 처음 붙은 안전망. 상세는 [utils](utils.md). (b) **지도 재검색(`df9fcbd`)**: [SubwayStationsMap](../../apps/web/src/components/subway/SubwayStationsMap.tsx) 인라인 ~50줄(패닝 종료 추적 + 자동 트레일링 스로틀 1.2s + 수동 버튼 판정)이 `@repo/shared` useMapResearch 호출로(지하철 임계 500m·z13 유지). 타이밍 계약 테스트 5건은 [web](web.md). 상세는 [shared](shared.md).
