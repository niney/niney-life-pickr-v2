...
**2026-08-22~23 신설·확장**: [meal](meal.md)의 검색·인식 교정·영양·알레르기 근거·추천 후보를 받치는 마스터 데이터 도메인이다. 여러 공공/내부 source를 하나의 정규화 카탈로그로 합치되, 필드별 관측값과 충돌을 보존해 대표값의 출처와 운영 결정을 추적한다.
...
| `AdminFoodPage` | 검색·필터·수정, nutrition coverage, observation/conflict 해결, import와 인식 품질 UI |
...
| `GET` / `POST` | `/api/v1/admin/food/items` | ADMIN 검색·source·taxonomy·nutrition 필터 목록 / 수기 등록 |
| `PATCH` | `/api/v1/admin/food/items/:id` | ADMIN 대표값·별칭·taxonomy·영양·활성 상태 수정 |
| `GET` | `/api/v1/admin/food/stats` | ADMIN total/active/classified, source/taxonomy, direct/estimated/missing 영양, observation/open conflict 수 |
...
| 모델 | 핵심 필드 |
|---|---|
| `FoodItem` | unique `nameNorm`, 이름/별칭, 3축 taxonomy, ingredients, allergen/evidence JSON, serving/영양, `nutritionFrom`, 대표 source/id/category, sourceRefs, popularity, active, classify model/version |
...
영양 coverage는 값 유무와 `nutritionFrom`을 기준으로 direct/estimated/missing을 구분한다. allergen JSON/evidence는 명시적 근거 슬롯이지만 현재 모든 import source가 이를 채우는 것은 아니다.
...
- **allergen field 공백은 무알레르겐이 아니다.** meal은 이름·재료 keyword도 보지만 숨은 재료·교차접촉을 보장할 수 없다.
