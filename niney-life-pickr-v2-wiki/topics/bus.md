...
```
웹 /bus (BusPage — q/stId/routeId/near URL 동기화)
  ├─ 좌: BusStationList | BusArrivalPanel | BusFavoriteSection
  └─ 우: BusStationsMap → MapCanvas (vworld OL + 차량 애니 레이어 + 폴리라인)
     │
  @repo/shared: useBus.ts (검색/주변/도착/위치/노선) · useBusFavorites.ts · busFavoriteStore.ts
  @repo/utils : busMarker.ts (마커 SVG) · markerFrame.ts (핀/원 골격) · routePath.ts (형상 투영)
...
- **map 도메인 (vworld)** — [BusStationsMap](../../apps/web/src/components/bus/BusStationsMap.tsx)이 `useMapPublicConfig` 로 vworld 공개 키를 받아 [MapCanvas](../../apps/web/src/components/restaurant/MapCanvas.tsx)에 넘긴다. 키 미등록(404)이면 placeholder. MapCanvas 의 차량 애니 레이어·`followVehicleId`·`routeLine` prop 은 버스가 처음 도입한 확장이다. 지도 인프라 전반은 [map](map.md).
...
- **`@repo/shared`** — `busApi`·`busFavoriteApi`(API 클라이언트), `useBus`·`useBusFavorites`(React Query 훅), `busFavoriteStore`(zustand persist).
- **`@repo/utils`** — `busMarker.ts`(마커/알약/화살표/점 SVG), `markerFrame.ts`(식당 마커와 공용 핀/원 골격), `routePath.ts`(형상 투영·슬라이스·방위각).
...
## Sources [coverage: high — 39 sources]
...
- [apps/web/src/components/bus/BusFavoriteSection.tsx](../../apps/web/src/components/bus/BusFavoriteSection.tsx)
- [apps/web/src/components/bus/BusFavoriteStar.tsx](../../apps/web/src/components/bus/BusFavoriteStar.tsx)
- [apps/web/src/components/bus/BusStationList.tsx](../../apps/web/src/components/bus/BusStationList.tsx)
