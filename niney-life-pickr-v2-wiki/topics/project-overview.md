...
선택이 고민될 때 대신 골라주는 서비스다. 핵심 도메인은 일곱 축으로 갈린다 — 선택 도우미·맛집 분석·정산(원래 세 축)에 2026-07 대중교통, 2026-08 생활정보·식단, 2026-09 **운세·선택 놀이(타로·사주)**, 2026-09-13 **여행로그(AI 허브 여행 통계 — 공개는 집계·원본은 관리자)** 가 더해졌다:
...
- **웹** (`apps/web`) — Vite + React 19 SPA. 공개 영역(`/`, `/restaurants(-v2)`, `/r/:placeId`, `/bus`, `/subway`, `/air`, `/weather`, `/life-map`, `/housing`, `/travel/jeju`, `/travel/plan`(26차), `/tarot`, `/tarot/s/:token`, `/saju-c`, `/saju-c/s/:token`, `/saju-g`, `/saju-g/pair`, `/saju-g/s/:token`, `/s/:token`, `/vote/:token`) + 인증 사용자 영역(`/me/meals`, `/me/settlements`, `/me/contacts`, `/me/tarot(/:id)`, `/me/saju-c(/:id)`, `/me/saju-g(/:id)`, `/me/saju-g/profiles`, `/vote/new`, `/restaurants/:placeId/settle/*`) + 어드민 콘솔(`/admin/*`) 세 묶음이 한 SPA 안에 공존. 공개 메뉴 순서(`PublicSidebar.NAV`, 26차 11개): 홈 · 맛집 · 대중교통 · 일상지도 · 집값 · **여행**(`/travel/jeju`, `match: ['/travel']`) · 날씨 · 대기질 · 타로 · 사주(C) · 사주(G) · 식단(로그인 시만). 어드민 사이드바엔 "여행로그 시드"(`/admin/tour`). `?embed=1`(또는 앱 브리지 주입)이면 `PublicLayout` 이 상단바·사이드바 없이 본문만 그린다 — 앱 WebView 용
- **앱** (`apps/mobile`) — Expo SDK 54 + React Native 0.81 + React 19. 탭 4개(홈·맛집·대중교통·프로필) + 스택 화면으로 맛집 상세/정산/단골/공유(`s/[token]`)/날씨/대기/일상지도/식단(`meal/{index,new,[id]}`) 풀구현. 타로·사주(C)는 네이티브 화면 대신 **WebView 임베드**(`app/tarot`·`app/saju-c` 가 웹 `/tarot?embed=1`·`/saju-c?embed=1` 을 열고 토큰·게스트 키를 주입) — 홈 `TarotEntryCard`·`SajuEntryCard` 로 진입. 사주(G)·집값·**여행로그**는 앱 미연동(여행로그는 4권역 완성 후 유일한 후속 후보 — 공개 API 는 앱이 같은 계약으로 붙일 수 있게 둠). 26차 앱 변경은 일상지도 생활편의(`store`) 표시와 홈 `SajuEntryCard` 의 "사주에 묻기" 버튼뿐. 어드민 UI 는 의도적 미포함. 로컬 빌드는 무료 Apple 팀(Universal Links·Push capability 제거), EAS/유료 팀은 `EXPO_PUBLIC_ENABLE_APPLINKS=1`
...
## Architecture [coverage: high — 37 sources]

pnpm workspaces + Turborepo 기반 모노레포. (트리의 `(신규)` 는 24차 2026-08-17~08-30 라운드 표기, 25차 2026-08-30~09-07 추가분은 `(25차)`, 이번 26차 2026-09-12~09-19 추가분은 `(26차)`)
...
│   │   ├── scripts/             87개 파일·101 스크립트 — load:*(적재) / status:*(한 줄 상태) / probe:*(실응답) / build:*(자산·표·범죄 JSON) / measure:*(골든셋) / backfill:* / eval:* / match:*·check:*·unload:*·e2e:*(26차 신설) (25차 +25, 26차 +9)
...
│   │       ├── config/env.ts                          DATA_GO_KR_API_KEY **하나**(BUS/AIRKOREA/KMA/HIRA/FOOD_API_KEY 폐기) · KMA_APIHUB · OLLAMA_{TAROT,SAJU,SAJU_G,MENU_MATCH}_MODEL · HOUSING_REFRESH_{CRON,MONTHS} · FOOD_RECIPE/MAFRA · MEAL_* (25차) · TOUR_RAW_USER_IDS(원본 allowlist, 기본 '') · TOUR_THUMBS_DIR(_WEST|_EAST|_CAPITAL)(비우면 export 폴더의 thumbs) (26차)
...
│   │       ├── components/PublicTopBar.tsx · PublicSidebar.tsx · AccountMenu.tsx   폭 예산(<md/md~lg/lg+/xl+) + 드로어 하단 계정·테마 (신규) — NAV 10개 + 계정 메뉴 "내 타로 기록" (25차) → NAV 11개("여행", 26차)
...
│   │   │   ├── air/ · weather/ · life-map/         생활정보 화면 (홈 MyLocationCard 에서 진입, 신규) — life-map 은 26차 생활편의(store) 표시(LifeDetailPanel·LifeNearbyRows·lifeMapPrefsStore 갱신), 배경 레이어·여행로그 화면은 앱 미구현
...
├── packages/
│   ├── api-contract/      Zod SSOT → api-contract 토픽 — schemas/ 44개 (+tour 26차 — TOUR_DATASET_KEYS 4·TOUR_REGION_KEYS 21 리터럴은 utils 와 이중 정의; +tarot/saju/saju-g/housing/usage-quota/menu-lexicon/menu-nutrition 25차) + routes.ts `Routes.Tour(admin*·adminPlace*·public*)/Tarot/Saju(/saju-c, +themes·ask)/SajuG/Housing/UsageQuota/LifeMap.crime`
│   │   └── eslint.config.mjs    @repo/config/eslint/base 직결
│   ├── shared/            FE 공통 (API 44 / hooks 48 / stores 30 — +tour.api·useTour(공개 집계·어드민·원본 useTourRaw*·tourPhotoUrl) 26차) → shared 토픽 (+tarot·saju·saju-g·housing·usage-quota api/hooks, guestKeyStore·tarotHistoryStore·sajuProfileStore·sajuG{History,Profile}Store·sajuGStorageMigration, **embedBridge.ts** 브리지 계약, 25차)
│   ├── utils/             순수 유틸 54 파일(테스트 31) → utils 토픽 (+tourLog(TOUR_DATASETS·TOUR_REGIONS 21키·권역 묶음·표본 판정·격자·등급·출처 문구)·lifeCrime·lifeStore·sajuThemes·sajuAsk, aiModel thinking, 26차) (+tarot·tarotCards·tarotFlow·tarotMenu / saju·sajuCalendar·sajuAstroTable(생성물)·sajuText·sajuDaily·sajuMatch·sajuDatePick·sajuFood·sajuFlow·sajuImages·sajuInsights·sajuDayPillar / saju-g·saju-g-stories / housing·housingMarker, aiModel purpose 확장, 25차)
│   └── config/            tsconfig + ESLint 베이스 → config 토픽
│       └── eslint/{base,node,react}.js   base(TS) ← node(+Node 글로벌) / react(+react-hooks v7)
├── deploy.sh              운영 서버 배포 스크립트 — 케이스 1~8 (6·7·8 = 데이터 적재 전용) + 1·2·4 에서 일상지도·음식 카탈로그·**집값** 자동 점검 (25차: housing_data + 케이스 8 + K-apt 탐지 set -e 함정 수정) (26차: `life_map_data` 에 상가 zip 적재·매칭 + 여행로그 "키|폴더|이름" 세트 루프 — `tour_<세트>=0` 만 적재 → `match:restaurant-tour` 1회, 상가 zip 탐지 glob 루프 `127e746`)
├── docs/
│   ├── data-sources.md    원본 데이터 운영 가이드 — 보관표·적재 명령·평가셋 추출 코드·백업 대상·테스트 격리 경고 (+집값 5행·원재료 DB·큐레이션·fatsecret 실측, 25차) (+tour 4행(백업 대상)·crime 2행·store 행·범죄 JSON 빌드 절, 26차)
...
| `housing` | 집값(공개) — 로컬 DB 조회(status/points/nearby/search/complexes/:id/trades), 적재 서비스(단지·거래·공시가격·K-apt·건축물·파생 통계·지오코딩), `housing-refresh` croner 월 갱신(`HOUSING_REFRESH_CRON`), 어댑터 rtms/kapt/bldg-hub. 26차: 단지 상세 `infra`(반경 500m 편의점·마트·카페·음식점·학원·병의원·약국 개수 — `LifeStore`·`LifeHospital`, LRU 10분, 좌표 없으면 null) | [housing](housing.md) |
...
| `restaurant` | 어드민 식당 CRUD + 공개 list/detail/insights/ranking (식당명 경량 조회 추가) + 공개 `menu-nutrition`(메뉴 탭 칼로리, 지연 조회, 25차) + 공개 상세/목록/smartPick 에 상가(`store`)·여행로그(`tour`) 결합, 정렬 `tourTravelers|tourScore`, 전략 `traveler` + `restaurant-store-match.service.ts`(canonical ↔ 상가업소 1:1, 26차) | — ([canonical](canonical.md) 사이드 테이블 2종) |
...
| `life-map` | 전국 CCTV·공중화장실·병의원·**상가(생활편의, 26차 `LifeStore`)** 로컬 적재 + points/nearby/detail/status + **`crime`**(범죄 통계 배경 — 커밋된 정적 JSON, 26차) + 지역 검색(행정구역·역·정류장·VWorld 주소). 셀 격자 10분 LRU, 요청 경로 외부 호출 0. 적재는 scripts + deploy.sh 자동 점검(26차 상가 zip 포함). 여행자 밀도 배경은 `tour` 모듈이 낸다 | [life-map](life-map.md) |
...
| 영역 | 레이아웃 | 라우트 | 가드 |
|---|---|---|---|
| 공개 | `PublicLayout` (TopBar 폭 예산 + lg 미만 드로어 사이드바 — md 미만에선 계정·테마도 사이드바 하단) | `/`, `/restaurants`, `/restaurants-v2`, `/r/:placeId`, `/bus`, `/subway`, `/air`, `/weather`, `/life-map`, `/housing`, `/travel/jeju`, `/travel/plan`(26차), `/tarot`, `/tarot/s/:token`, `/saju-c`, `/saju-c/s/:token`, `/saju-g`, `/saju-g/pair`, `/saju-g/s/:token` | 없음 (여행로그 공개 집계는 `RATE.tourRead` 120/분, 타로·사주 LLM 호출은 게스트 키·IP·전역 일일 한도, 옵셔널 인증) |
