...
- **맛집 분석** — 어드민이 네이버 플레이스 식당을 크롤링하고 리뷰를 LLM으로 분석해 메뉴 통계 트리까지 빌드한다 ([crawl](crawl.md), [ai](ai.md), [menu-grouping](menu-grouping.md), [analytics](analytics.md)). 결과물은 공개 페이지에서 비로그인 사용자가 그대로 본다.
...
| 인증 진입 | (단독) | `/login` | 없음 |
| 어드민 | `AdminLayout` (좌측 사이드바 6항목) | `/admin/*` | `RequireAdmin` (token + role=ADMIN) |
