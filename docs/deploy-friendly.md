...
## 여행로그 원본·사진 (AI 허브 71780 제주·도서 + 71779 서부권 + 71778 동부권 + 71581 수도권 — 관리자 allowlist)

여행로그 파생표(`data/open/tour/<export>/*.jsonl.gz`)와 썸네일(`thumbs/s`)은 git 밖이라 **rsync 로 올린다**. 원본은 AI 허브
이용조건상 재배포·국외 반출 금지 — 서버는 국내에 있어야 하고 공개 API 는 집계만 낸다(`docs/PLAN-tour-log.md`). 데이터셋이 넷이라
`--dataset` 으로 각각 적재한다(그 세트 행만 교체, 다른 세트는 그대로). 제주 export 는 `lp-2023`, 서부권 `lp-west-2023`, 동부권
`lp-east-2023`, 수도권 `lp-capital-2023`(세트 목록·폴더명은 utils `TOUR_DATASETS`).

```bash
# 로컬(tour-c 에서 npm run data:export -- --thumbs s 로 만든 폴더) → 운영. 다른 권역은 tour-c-west/east/capital 에서 TOUR_DATA_ROOT 로 실행.
...
rsync -av --delete data/open/tour/lp-east-2023/    samplepcb@<host>:/home/samplepcb/niney-life-pickr-v2/data/open/tour/lp-east-2023/
rsync -av --delete data/open/tour/lp-capital-2023/ samplepcb@<host>:/home/samplepcb/niney-life-pickr-v2/data/open/tour/lp-capital-2023/
...
pnpm --filter friendly load:tour --dataset east             # 동부권 세트 행만 교체(약 1분)
pnpm --filter friendly load:tour --dataset capital          # 수도권 세트 행만 교체(약 1분)
pnpm --filter friendly match:restaurant-tour                # 세트 다 적재한 뒤 한 번 — 수도권부터 운영 맛집(서울)과 실제로 맞는다. 먼저 --dry-run 으로 매칭률 확인
pnpm --filter friendly status:life-map   # ... tour=50271 tour_jeju=15679 tour_west=11516 tour_east=12531 tour_capital=10545 tour_matched=N ...
```

`--dataset` 이 맞는지 적재기가 여행 표의 제주 방문 비율로 검사한다(제주는 절반 이상, 다른 권역은 1할 미만 — 틀리면 중단). API 배포
(케이스 1·2·4)는 `tour_<세트>` 가 0 이고 그 폴더에 `manifest.json` 이 있으면 자동으로 세트별 적재 후 매칭을 한 번 돌린다(세트 목록은
`deploy.sh` 의 "키|폴더|이름" 루프) — 폴더를 배포 뒤에 올렸다면 직접 실행한다(`TOUR_WEST_EXPORT_DIR`·`TOUR_EAST_EXPORT_DIR`·
`TOUR_CAPITAL_EXPORT_DIR` 로 폴더 위치를 바꿀 수 있다). `./deploy.sh 6` 은 force 모드라 CCTV·화장실·병의원·상가까지 전부 다시 적재하므로
여행로그만 넣을 땐 쓰지 않는다. 한 세트만 폐기하려면 `unload:tour --yes --dataset capital`.
...
| 썸네일 폴더 | `TOUR_THUMBS_DIR`(제주, 비우면 `data/open/tour/lp-2023/thumbs`) · `TOUR_THUMBS_DIR_WEST`(서부권, 비우면 `lp-west-2023/thumbs`) · `TOUR_THUMBS_DIR_EAST`(동부권, 비우면 `lp-east-2023/thumbs`) · `TOUR_THUMBS_DIR_CAPITAL`(수도권, 비우면 `lp-capital-2023/thumbs`). cwd 기준 `apps/friendly` 또는 리포 루트 |
...
| 환수·폐기 | `pnpm --filter friendly unload:tour --yes`(전부) 또는 `--yes --dataset west|east|capital`(한 세트) + `rm -rf data/open/tour/<export>` |
