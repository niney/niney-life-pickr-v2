...
| **3차 관리자 근거·사진·코스 샘플** | `TourRawService` · `requireTourRaw` · `/admin/tour/places/:id/*`·`/trips/:id`·`/photos/:id/:size` · 어드민 식당 상세 "여행자 근거" 섹션 · 사진 rsync 절차(`docs/deploy-friendly.md`) | allowlist 계정만 원본이 보이고, 다른 admin 은 404. 응답 헤더 no-store·noindex |
| **4차 공개 — 상세·목록·골라줘** | `tour.ts` 계약 · `TourAggregateService`(장소 통계) · 공개 detail `tour` 필드 + `/tour-stats` · 목록 정렬·카드 · smartPick `traveler` · `TourTab` · 출처 표기 컴포넌트 `TourSourceNote` · 테스트(식별자 없음·k) | 매칭된 식당 상세에 "여행자" 탭, 목록 정렬, 홈 픽 전략. 문의 회신 확인 후 배포 |
| **5차 공개 — 인사이트·코스** | `/tour/public/insights`·`/plan` · `TravelInsightsPage`(필터 바) · `TravelPlanPage` · 그룹투표 프리필 연동 · 사이드바 "여행" | 필터를 바꾸면 셀이 갱신되고 20건 미만은 안내. 코스 → 그룹투표 생성까지 이어짐 |
