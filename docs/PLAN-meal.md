...
- **남은 것**:
  1. ~~식품안전나라 레시피~~ — 2026-08-22 적재 완료(1,101종, 재료 1,097행).
  2. **글로벌 메뉴 병합 1회 실행**(어드민 > AI 분석 관리) 후 `load:food-catalog --source=menu-canonical` — 현재 `global_menu_canonicals` 가 0행이라 외식 메뉴 어휘(마라탕 등)가 아직 빠져 있다.
