...
- **임베드 모드** `?embed=1`: 타로와 같은 `embed.ts`·브리지. 앱은 4차~9차엔 `app/saju-c/index.tsx` WebView 였고 **10차부터 네이티브**(아래 §10차) — 임베드 모드는 웹 쪽 호환 경로로만 남는다.
...
| **10차** ✅ | 앱 네이티브 전환 — WebView 를 걷어내고 RN 화면으로(흐름·요청 오케스트레이션은 shared `useSajuSession` 을 웹과 공유). 3D 무대 → Reanimated 2D 천문도(고리 정지·인장 낙하·파문·일간 등장), 후속으로 iOS 3D 무대(expo-gl + R3F, 느린 GPU·Android 는 2D), 이미지 22장 번들, 효과음 없음. 입력·원국·풀이 12탭·궁합·공유(웹 링크)·내 사주 기록·프로필 탭 진입 — 상세는 §10차 | shared `hooks/useSajuSession.ts`·`saju/sajuPanelTabs.ts`·`saju/sajuTheme.ts`·`hooks/useTypewriter.ts`, 앱 `app/saju-c/index.tsx`·`app/saju-c/me/*`·`src/components/saju/**`·`assets/saju/*` |
...
### 10차 설계 — 앱 네이티브 전환 (2026-09-25 사용자 결정: 권장안대로 단계적 전환, 공유는 웹 링크)

- **왜**: WebView 는 3D 무대·웹 번들을 통째로 들고 와 첫 진입이 느리고(LAN/원격 웹 로드 + WebGL), 앱 스토어·뒤로가기·키보드·공유가 브리지 너머에 있었다. 계산은 이미 utils(순수 TS)라 RN 에서 그대로 돈다.
- **흐름 공유**: 웹 `SajuPage` 의 오케스트레이션(리듀서·풀이 요청·job 병합·테마 job·궁합·딥링크 탭·프로필 저장)을 shared `useSajuSession` 으로 뽑아 웹이 먼저 쓰게 했다(0단계, 웹 테스트 그대로 통과). 탭 상수·색·문구·타자 효과도 shared 로.
- **무대**: 처음엔 3D 대신 Reanimated 2D 천문도 — 3D 와 같은 타이밍(고리 1.4·2.1·2.8초 정지, 인장 낙하 0.32초·0.55초 간격, 파문 0.9초). 흐름 전환(casting_done·stamp)은 사주 화면의 JS 타이머 한 곳이 내고 무대는 그리기만. 다 찍으면 1.1초 붙잡고 일간 캐릭터를 띄운 뒤 풀이로. 기기 "동작 줄이기" 면 연출 생략. 효과음 없음.
- **3D 무대(10차 후속, 2026-09-26 사용자 결정 "A 진행")**: iOS 는 expo-gl + react-three-fiber 로 웹 3D 장면을 옮겼다(`SajuStage3D`·`stage3d/Scene3D`). 캔버스는 화면 위쪽 70% 한 장 — 입력 땐 원판을 위쪽에 작게, 연출 땐 카메라가 거리·세로 오프셋으로 가운데 크게 옮겨 간다. 2/3 크기로 그려 1.5배 확대(3배 화면에서 2배 밀도). 재질은 Phong(expo-gl 에서 넓은 PBR 면이 안 그려지는 프레임이 있어서), 후처리 없음(부동소수 렌더 타깃 없음), 한자는 미리 구운 아틀라스(`build-saju-glyph-atlas.ts`), 인주·파문·광택·배경은 DataTexture. 매 프레임 GL 동기(getError)로 역압 — 없으면 느린 GPU 에서 GL 큐가 밀려 동기 질의에서 앱이 멈춘다(시뮬레이터 실측). GL 오류·렌더러가 소프트웨어·첫 프레임들이 120ms 를 넘으면 2D — **시뮬레이터 GL 은 "Apple Software Renderer" 라 늘 2D**(개발 빌드 `?stage=3d` 로 강제). 입력 화면 원판은 방문마다 한 번 정한다(후속 다듬기, 2026-09-26): 원판 자리를 비워 두고 3D 준비를 처음 보는 기기 0.9초·3D 로 기억된 기기 2.5초까지 기다렸다가 늦으면 그 방문은 2D, 기기 판정(`stage3dVerdict`, AsyncStorage `lp:saju-stage3d:v1`)을 기억해 다음 방문부터 처음부터 3D 또는 2D — 도중에 2D → 3D 로 바뀌지 않는다. "느림"은 확실할 때만(오류·소프트웨어 GL·다섯 프레임 중 셋 이상) 기억하고 7일 뒤나 앱 버전이 바뀌면 다시 잰다. expo-gl 네이티브 모듈이 없는 옛 빌드도 2D(모듈 확인 뒤에만 3D 를 불러온다). Android 는 기기 검증 전까지 2D.
- **화면**: 폰 한 화면 — setup(작은 천문도 + 입력 카드, 월·일·시·분은 모달 격자 피커) → 연출(전체 무대 + 건너뛰기) → 풀이(머리·그룹/서브 탭·본문·바닥줄). 대운 타임라인은 SVG 대신 View 막대, 궁합 점수 링은 반원 회전. 이미지 22장(512px webp, 약 1.3MB)은 번들(`sajuImages.ts` 정적 require 표).
- **공유 = 웹 링크**: 받는 사람 대부분은 앱이 없고, 웹 공유 페이지가 OG 미리보기·로그인 없는 보기를 이미 한다. 앱은 토큰을 발급해 웹 URL 을 OS 공유 시트에 넘기고, 세로 이미지는 서버 PNG 를 캐시에 받아 파일로 공유한다. 앱 안 공유 페이지·유니버설 링크(유료 팀·AASA·Android intent)는 두지 않는다.
- **기록**: `app/saju-c/me`(프로필 ★·삭제, 물어본 것, 풀이 기록 무한 목록) + `app/saju-c/me/[id]`(한 장 보기·공유·삭제), 프로필 탭 "내 사주 기록", 사주 화면 헤더(회원). 게스트 프로필은 앱 스토어(AsyncStorage)에 바로 저장되므로 웹→앱 프로필 미러(`saju-profiles` 브리지)는 걷어냈다.
- **발견한 RN 함정 2**: ① Hermes 의 `URLSearchParams` 에 `size` 가 없다 — `qs.size > 0` 으로 쿼리를 붙이던 shared `listMine`(사주·타로)이 앱에서 쿼리를 통째로 떨궈 "물어본 것" 이 풀이 목록으로 나왔다 → `toString()` 으로 판단. ② RN 0.81(Fabric) iOS 는 글을 프레임 높이 컨테이너로 그려서, 긴 스크롤 안쪽(절대 좌표가 큰 곳)의 여러 줄 글은 Yoga float 반올림으로 프레임이 1e-4pt 모자라 마지막 줄이 통째로 빠진다(lineHeight 로 높이가 픽셀 격자에 딱 맞을 때 잘 남). 사주 화면은 `Para`(끝에 1pt 폭 없는 줄)로 우회 — 다른 화면도 같은 위험이 있다.
...
- 2026-09-26: **3D 무대 최적화(타로와 함께).** 셰이더 예열의 인장 윗면 재질을 모듈 공유로 바꾸고 예열은 캔버스를 올리면
  3프레임만 그리고 숨긴다 — 예열이 연출(casting → stamping) 때 내려가며 R3F 가 JSX 재질을 버려 셰이더가 해제되고, 첫 인장이
  떨어지는 프레임에 다시 컴파일하며 멈칫했다(시뮬레이터 0.45초, 개발 계측으로 셰이더 수 8 → 7 → 8 확인). 별가루 점 셰이더는
  하늘이 늘 쓰므로 예열에서 뺐다. 공용 Stage3DCanvas 는 준비 판정을 rAF 로 하고, 캔버스를 내릴 때 장면 자원을 모두 dispose
  (풀이마다 캔버스를 내렸다 올려 옛 렌더러가 하나씩 남던 것). 미커밋.
- 2026-09-26: **3D 무대 공용화(타로 3D 와 함께).** 타로 앱 3D 무대(PLAN-tarot §앱 3D 무대)를 붙이며 GL 배관·문지기를 `src/components/common/stage3d/` 로 옮겼다 — `Stage3DCanvas`(캔버스·오류 경계·성능 판정·역압·틱·페이드, `SajuStage3D` 는 이걸 감싼 얇은 껍데기), `useStage3DGate`(원판 자리 pending → 3d | 2d·캔버스 올리고 내리기·연출 무대 결정), `stage3dVerdict`(무대별 키 `lp:saju-stage3d:v1`·`lp:tarot-stage3d:v1` — 사주 키는 그대로), `stage3dAvailable`, `dataTexture`(절차 텍스처 도우미). `stageConfig` 에는 타이밍만 남았다. 문지기 고침: 연출 무대 3D·2D 를 화면이 렌더 중 다음 값으로 정하던 것을 문지기가 입력 화면을 떠나는 렌더에서 정하게 했고, 준비 표시는 캔버스가 실제로 내려갈 때(onLost)만 끈다 — 렌더 중 상태 갱신으로 버려지는 렌더 패스(사주 holding 등)에서 캔버스를 잠깐 "내림"으로 보고 준비 표시를 지우던 것(사주는 풀이에서 캔버스를 내렸다 다시 올려 증상이 가려져 있었다). 시뮬레이터(iOS 26.5) 확인: 강제 3D 연출·붙잡기(일간 캐릭터)·다시 입력 → 재연출 3D, 기본 경로 2D·slow 저장. 미커밋.
- 2026-09-26: **첫 진입 2D → 3D 전환 다듬기(10차 후속).** 첫 진입에 2D 원판이 보였다가 3D 로 바뀌던 것(3D 준비 전 2D 를 먼저 깔던 방식)을 없앴다. 입력 화면 원판은 방문마다 pending → 3d | 2d 로 한 번 정한다: 원판 자리를 비워 두고(별만) 3D 준비를 기다려(처음 보는 기기 0.9초, 3D 로 기억된 기기 2.5초) 그 안에 준비되면 캔버스가 0.4초에 나타나고, 늦으면 2D 로 고정한다. 2D 로 정한 뒤에도 캔버스는 투명하게 그려 판정을 마저 재고(최대 8초) 기기 판정을 남긴다. `stage3dVerdict`(AsyncStorage) — ok 면 다음 방문부터 3D 를 기다리고, slow 면 캔버스를 아예 만들지 않는다. 판정은 다섯 프레임을 다 재서 한 프레임만 튄 건 이번 방문만 2D(기억 안 함), 셋 이상·오류·소프트웨어 GL 이면 slow 로 기억(7일·앱 버전 변경 시 재측정). 강제 `?stage=3d|2d` 는 판정을 읽지도 쓰지도 않는다. 시뮬레이터(iOS 26.5) CDP 실측: 첫 방문 소프트웨어 GL → 화면이 밀려 드는 사이 2D·slow 저장·캔버스 없음 / 재방문 1.5초 동안 캔버스 한 번도 안 만듦 / 강제 3D 는 0.4~2.4초 빈 원판 → 3.4초 3D / 판정 통과를 흉내 낸 임시 플래그로 처음 보는 기기 경로(빈 원판 → 기다림 끝 2D → 가려진 캔버스가 판정 통과·ok 저장·내려감, 원판은 2D 유지)와 ok 재방문(빈 원판 → 2.4초 3D) 확인 후 임시 코드 제거.
- 2026-09-26: **공유 카드 한자 □ 수정.** 공유 PNG(satori)의 글꼴 IBM Plex Sans KR 에 한자가 없어 본문 속 십신·오행 한자(正官·比肩·火 등)가 □로 나왔다(일간 한자만 글리프 이미지로 대체돼 있었다). `lib/share-fonts` 가 한자 대체 글꼴(Noto Sans CJK KR 의 한자 8,651자 서브셋 1.9MB, OFL, `assets/fonts/NotoSansKR-Hanja-Regular.otf`)을 satori `loadAdditionalAsset` 로 한자가 든 카드에만 보태고, 사주 C·G·타로 공유 카드에 연결. 글 속 일간 한자 이미지 대체는 걷어내 글꼴로 통일(인장은 명조 이미지 유지). satori 글꼴 배열을 고정해 호출마다 Plex 를 다시 파싱하던 것도 없앴다. 테스트 share-fonts 2(대체 훅을 끄면 실패하는 것 확인)·saju 28·saju-g 17·tarot-share 6 green, 실제 og·story PNG 로 확인.
- 2026-09-26: **앱 3D 무대(10차 후속).** 시험 구현으로 expo-gl + R3F 가 iOS 26.5·27 시뮬레이터에서 뜨는 것을 확인한 뒤 본 이식: `expo-gl`·`three`·`@react-three/fiber` 추가(개발 클라이언트 재빌드 필요), `SajuStage3D`(캔버스·오류 경계·성능 판정·역압·30fps/rAF 틱·백그라운드 정지), `stage3d/Scene3D`(원판·고리 3겹·성도·인장 낙하·파문·별가루·별·조명·카메라 리그), `stage3d/glyphs`·`textures`, 흐름 타이머를 화면으로 모음(2D·3D 공용), `stageConfig`. 부수: Xcode 27 Pod 배포 대상 오류 → `plugins/with-pod-deployment-target.js`, `metro.config.js` 가 `three` 를 한 벌(CJS)로. 시뮬레이터에서 알게 된 것: GL 이 소프트웨어 렌더러(초당 1~3프레임) → 기본 2D, 강제 3D 로 연출·마무리·풀이 전환까지 멈춤 없이 통과(iOS 26.5·27). 사용자 확인 — 첫 진입 2D → 3D 전환(의도)·3D 동작 정상. 실기기 프레임 수치는 미측정. 커밋 후 main 병합(2026-09-26).
- 2026-09-25: **10차(앱 네이티브 전환) 완료.** shared `useSajuSession`(흐름·풀이·테마·궁합·저장 오케스트레이션, 순수 조각 `toSajuBirthInput`·`defaultSajuPartner`·`mergeSajuJobResult` 테스트 7) + `saju/sajuPanelTabs`(`sajuInitialTab`·`sajuInitialMode`)·`saju/sajuTheme`·`hooks/useTypewriter` 를 웹에서 옮김 → 웹 `SajuPage` 가 훅을 쓰도록 바꿈(효과음 준비만 웹 몫). 앱 `app/saju-c/index.tsx` 네이티브(2D 무대 `SajuStage2D`·입력 `SajuForm`·격자 피커·풀이 `SajuReadingPanel` 12탭·도구·테마·질문·궁합 `SajuPairPanel`·공유 `SajuShareSheet`) + `app/saju-c/me`·`me/[id]` + 프로필 탭 진입 + 이미지 22장 번들, 사주 WebView·`saju-profiles` 미러 제거, 앱 타로 화면 `?q=&topic=` 통과. 버그 수정: shared `listMine`(사주·타로)의 `URLSearchParams.size` 의존(앱에서 쿼리 유실) / iOS 여러 줄 글 마지막 줄 사라짐 우회(`Para`). 검증: iOS 26.5(iPhone 17 Pro)·iOS 27(iPhone 17) 시뮬레이터에서 Hermes CDP 로 조작 — 연출(고리·인장·일간 등장)·AI 풀이 4섹션·12탭 전부·테마 job·질문 답·궁합·"내 사주 자세히 보기"·다시 입력·`?tool=daily` 딥링크·게스트 로컬 저장·회원 계정 저장·기록 목록/상세·공유 링크(웹 URL)·OS 공유 시트·세로 이미지 파일 공유. API 는 사용자 서버(:3000)가 새 Prisma 클라이언트 + 미적용 마이그레이션(`thinking` 컬럼)으로 500 이라, 런타임 DB 사본에 마이그레이션을 적용한 부트스트랩 없는 검증용 인스턴스(:3100, 크론 정지)로 확인. 테스트 shared 88·web 145 green, typecheck 6/6, lint 0 error. Android 는 에뮬레이터가 없어 미확인(2026-09-26 사용자 결정: 이번엔 건너뜀). 커밋 후 main 병합(2026-09-26).
...
- 2026-09-07: **앱 실기기·시뮬레이터 검증 + 홈 "오늘의 운세" 네이티브 카드.** ① 검증 — iPhone 17 Pro 시뮬레이터(WKWebView)에서 사주(C) WebView 전 흐름 통과(LAN 주소 로드·3D 무대·원국 己巳 丙子 丙寅·AI 풀이·오늘 탭·공유 링크·`share` 브리지 → iOS 공유 시트·`open` 브리지 → Safari 세로 이미지·네이티브 뒤로가기·게스트 프로필 유지). 실기기 iPhoneNS(iPhone 16 Pro, iOS 26.6.1)는 만료된 개인 팀 프로파일을 재서명 빌드로 갈아 실행했고, Metro JS 로드(8081)·앱 API(3000)·WebView 웹 로드(5173)·게스트 키 주입(앱 키 `c-…` 그대로 사용)·풀이(캐시)·공유 생성(토큰 `yw1mZlREsQ`)까지 DB 로 확인. 3D 렌더·성능은 화면을 못 봐 사용자 확인 대기. 실기기 함정: 개인 팀 프로파일 7일 만료, dev client 가 마지막 서버(8081)로만 접속, `pnpm install` 뒤 Metro 파일맵 깨짐(SHA-1 오류 → `pnpm dev` 재시작). ② 발견 — 공유 카드 PNG 본문의 십신·오행 한자(正官·比肩·火 등)가 □ 로 렌더(글리프 이미지 27자 밖, 미수정). ③ 네이티브 카드 — 브리지 `saju-profiles`(shared `embedBridge`·`stores/sajuProfileMirror`, 웹 `main.tsx` 부팅 호출, 앱 `app/saju-c` 수신 → `useSajuProfileStore` 교체)로 게스트 프로필을 앱에 미러하고, 홈 `SajuEntryCard` 가 회원=서버 프로필/게스트=미러 프로필로 `computeSajuChart`+`dailyFortune` 을 기기에서 돌려 일진·별점·헤드라인·태그를 보여준다(네트워크·LLM 없음, 누르면 WebView 오늘 탭). shared 테스트 2건 추가. 검증 — 웹 절반은 Playwright(Chromium, `ReactNativeWebView` 흉내)로 로드 시 빈 목록 1건·저장 후 프로필 1건 전달 확인, 앱 절반은 시뮬레이터 AsyncStorage 에 같은 프로필을 심어 홈 카드가 utils 기대값(갑신일·별 2·인연이 이어지는 날)과 일치함을 확인. 이어서 시뮬레이터 E2E — Metro Hermes 디버거(CDP)로 앱 JS 에 접속해 fiber 트리에서 실제 WebView ref 를 찾아 `injectJavaScript` 로 폼 라벨 "시뮬" 입력·"사주 세우기" 클릭 → 웹 저장 → 브리지 → 앱 스토어(AsyncStorage 에 새 id 로 영속) → 앱 재실행 후 홈 카드 "시뮬의 오늘 · 갑신(甲申)일" 표시까지 실제 경로 통과. 실기기는 사용자 확인 대기. 전체 typecheck 6/6, shared 84 테스트 통과. (2026-09-25 원격 9차 위로 리베이스 — 홈 카드에 9차 부제·"사주에 묻기" 버튼이 자동 병합됐고, prisma generate 후 typecheck 6/6·shared 84 재확인.)
...
- 2026-09-07: **6차(순수 계산 2순위) 완료.** utils `sajuInsights.ts` — 격국(월지 정기 십신 → 8격 + 건록·양인격, 요약·설명 10종) · 오신(용신=보완 오행, 희신=보조 또는 생하는 오행, 기신=용신을 극하는 오행, 구신·한신, 이유 문장) · 삼재(띠 삼합의 묘 지지에서 끝나는 3년 — 신자진→인묘진 등, 들/눌/날 단계·다음 3년) · 월운 12개월(절기 표로 입춘~다음 입춘, 월간 오호둔, 점수) · 향후 5년 세운(테마=천간 십신 그룹, 변동/인연/공망 표식) · 하루 12시진(그날 일간으로 시간 오서둔, 좋은 2·조심 1) · 오행 건강 힌트(장기·부위·부족/과다 문구, 최대 3) · 지장간 숨은 십신. 점수는 `sajuDaily` 의 일진 규칙을 `scoreGanzhiForChart` 로 추출해 일진·월운·세운·시진이 같은 규칙. 웹: 원국 탭 "명식 한눈에" 카드(격국·오신 칩·삼재)+표 아래 숨은 십신, 오행 탭 건강 힌트, 흐름 탭 5년 세운 표, 올해 탭 월운 12칸(이번 달 강조·점수 점·좋은 달 3), 오늘 탭 12시진 막대. 2D 뷰 동일. LLM 사실 블록에 격국·오신·삼재 줄 추가(`SAJU_PROMPT_VERSION` 2 → 기존 캐시 무효). 테스트 utils 9건 추가(총 317)·friendly 21·웹 9 green. 미커밋.
