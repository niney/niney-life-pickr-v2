...
- **3D 무대(10차 후속, 2026-09-26 사용자 결정 "A 진행")**: iOS 는 expo-gl + react-three-fiber 로 웹 3D 장면을 옮겼다(`SajuStage3D`·`stage3d/Scene3D`). 캔버스는 화면 위쪽 70% 한 장 — 입력 땐 원판을 위쪽에 작게, 연출 땐 카메라가 거리·세로 오프셋으로 가운데 크게 옮겨 간다. 2/3 크기로 그려 1.5배 확대(3배 화면에서 2배 밀도). 재질은 Phong(expo-gl 에서 넓은 PBR 면이 안 그려지는 프레임이 있어서), 후처리 없음(부동소수 렌더 타깃 없음), 한자는 미리 구운 아틀라스(`build-saju-glyph-atlas.ts`), 인주·파문·광택·배경은 DataTexture. 매 프레임 GL 동기(getError)로 역압 — 없으면 느린 GPU 에서 GL 큐가 밀려 동기 질의에서 앱이 멈춘다(시뮬레이터 실측). 3D 준비 전·GL 오류·렌더러가 소프트웨어이거나 첫 프레임이 120ms 넘게 걸리면 2D — **시뮬레이터 GL 은 "Apple Software Renderer" 라 늘 2D**(개발 빌드 `?stage=3d` 로 강제). expo-gl 네이티브 모듈이 없는 옛 빌드도 2D(모듈 확인 뒤에만 3D 를 불러온다). Android 는 기기 검증 전까지 2D.
