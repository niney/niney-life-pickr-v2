...
| `tour/lp-2023/` | AI 허브 「국내 여행로그 데이터(제주도 및 도서지역)」(데이터셋 [71780](https://aihub.or.kr/aihubdata/data/view.do?dataSetSn=71780), 내국인 로그인·승인 필요, 원본 203GB)을 별도 작업공간 `niney-tour-pickr/tour-c` 의 `npm run data:export -- --thumbs s` 가 파생표 10개(`*.jsonl.gz`) + `manifest.json`(exportVersion 1) + 선택 썸네일(`thumbs/{s,m}`)로 내보낸 것. **재배포 금지·국외 반출 금지** — 원본은 이 폴더 밖으로 복사하지 않고, 공개 API 는 집계만 낸다([PLAN-tour-log.md](PLAN-tour-log.md)) | JSONL 17MB / 썸네일 s 178MB (2026-09-13) | `pnpm --filter friendly load:tour` (27초, 전량 교체) → `match:restaurant-tour`(2차) → `check:tour-biz`(2차). 환수 시 `unload:tour --yes` | `Tour*` 10표(장소 15,679 · 여행 2,880 · 방문 51,596 · 활동 60,957 · 소비 43,664 · 전이 42,385 · 일차 7,633 · 동반 4,427 · 사진 17,513 · 코드 238) + `LifeMasterSync layer=tour`. 재취득이 어려우므로 **백업 대상** |
