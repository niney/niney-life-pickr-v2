...
- **겹친 면은 앞에서부터 그린다.** 78장이 거의 같은 자리에 쌓인 타로 덱을 아래 카드부터 그리면 윗면을 78번 덧칠해
  소프트웨어 GL 에서 프레임당 19초(앱이 멈춘 듯 보인다). 타로 덱은 인스턴스 칸을 매 프레임 카메라에 가까운 카드부터
  채운다(쌓인 덱 0.5초, 부채꼴 1.2 → 0.57초). 실기기 Apple GPU 는 HSR 로 알아서 거르지만 다른 GPU·시뮬레이터는 그대로 칠한다.
- **셰이더 예열 재질은 모듈에 둔다.** R3F 는 컴포넌트가 내려갈 때 JSX 로 선언한 재질(`<lineBasicMaterial>` 등)을 버리고,
  three 는 그 셰이더를 쓰는 재질이 없으면 프로그램을 지운다. 예열을 내렸다가 다음 연출에서 같은 셰이더가 다시 컴파일되며
  동기 대기로 멈칫했다(타로 부채꼴 첫 프레임 1.2초, 사주 첫 인장 0.45초 — 시뮬레이터). 속성으로 넘긴 재질·모양
  (`material={...}`)은 R3F 가 버리지 않는다.
- **캔버스를 내리면 장면 자원을 모두 dispose 한다**(`Stage3DCanvas` 의 DisposeOnUnmount). 모듈 공유 자원엔 렌더러마다
  dispose 리스너가 붙어, 그냥 내리면 옛 렌더러(와 GL 컨텍스트 래퍼)가 붙잡혀 캔버스를 올릴 때마다 하나씩 남는다.
- **개발 계측**: 개발 빌드는 1초마다 `globalThis.__stage3dStats`(fps·최악 프레임·갱신 JS/렌더 명령/GL 대기 ms·그리기 수·
  텍스처·지오메트리·셰이더 수), 단계(phase)별 누적 `__stage3dPhaseStats[tag][phase]`, 최근 200프레임 `__stage3dFrames` 를
  적는다 — Hermes CDP 로 읽는다(로그 없음).
