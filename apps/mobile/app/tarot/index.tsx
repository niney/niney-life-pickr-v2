...
// 타로 — 웹 `/tarot?embed=1` 을 WebView 로 임베드한다(docs/PLAN-tarot.md 결정: 웹 먼저, 앱은 v2 WebView).
...
//  - 같은 origin 안의 이동(공유 페이지·내 타로 기록)은 WebView 안에서, 밖은 외부 브라우저로.
...
export default function TarotScreen() {
  ...
  const [title, setTitle] = useState('타로');
    ...
    <View style={styles.root}>
      ...
      {failed ? (
        <View style={styles.center}>
          <Text style={styles.errorTitle}>타로를 불러오지 못했어요</Text>
      ...
      {loading && !failed ? (
        <View style={[styles.center, styles.overlay]} pointerEvents="none">
          ...
          <Text style={styles.loadingText}>무대를 준비하는 중…</Text>
        </View>
      ) : null}
    </View>
  );
