...
import { isThumbnailProxyable, reviewThumbnailUrl } from '@repo/utils';
 ...
 *
 * 프록시 서버는 허용 호스트(*.pstatic.net + 배민 메뉴 사진)만 받으므로(그 외 400),
 * 그 밖의 URL(자체 호스팅·DC 등)은 원본을 그대로 통과시킨다. 판정은 utils 의
 * isThumbnailProxyable 하나로 웹과 공유. base 가 비어있으면(설정 전) 원본 반환 —
 * 깨진 이미지 대신 원본으로 폴백.
 *
...
export function thumbUrl(
  ...
  if (!isThumbnailProxyable(url)) return url;
  if (!base) return url;
