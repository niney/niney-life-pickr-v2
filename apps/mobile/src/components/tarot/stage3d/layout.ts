...
// 3D 무대 배치 수학 — 웹 stage/layout.ts 를 옮긴 것(같은 치수·포즈라 웹과 같은 장면). 카드 치수, 카메라, 덱 쌓기·섞기
// 흩뿌림·부채꼴·자리(슬롯) 포즈. 모든 포즈는 부채꼴 그룹 로컬 좌표(그룹 원점 = 부채꼴 호의 중심, z = FAN_CZ). 그룹을
// Y 축으로 돌리면 부채꼴이 호의 중심을 축으로 회전한다(드래그로 훑기). 회전은 'YXZ'(yaw → 로컬 pitch → 로컬 roll)
// 오일러로 만들고 쿼터니언으로 보간한다. 타이밍은 앱 2D 무대와 같은 stageTiming(초 단위로 바꿔 쓴다).
...
// 부채꼴 호 — 반지름·반각·높이·기울기. 호의 중심은 카메라 반대쪽(z 음수)에 두어 카드가 카메라 쪽으로 불룩한 호를
// 그린다. 카드는 호를 따라 서되 뒷면이 모두 카메라를 향한다. 살짝 roll 을 줘 손에 든 부채처럼, 양끝은 조금 내려앉는다.
const FAN_R = 6.2;
const FAN_HALF = 0.82;
const FAN_Y = 1.0;
const FAN_TILT = 0.38;
...
export const TIMING = {
  shuffleS: TAROT_STAGE_TIMING.shuffleMs / 1000,
  shuffleBeatS: TAROT_STAGE_TIMING.shuffleBeatMs / 1000,
...
export const makePose = (x: number, y: number, z: number, pitch: number, yaw: number, roll: number, s = 1): Pose => ({
...
export const stackPose = (i: number, worldZ = 1.0): Pose => makePose(0, 0.03 + i * CARD_T, local(worldZ), Math.PI / 2, 0.05, 0);

// 뽑기가 끝난 뒤 덱이 물러나 쉬는 자리(슬롯 뒤).
export const restPose = (i: number): Pose => stackPose(i, -2.4);

// 겹침 순서 고정 — 정점 근처 이웃 카드는 카메라 거리가 거의 같아 앞뒤가 프레임마다 뒤바뀌며 문양이 번갈아 가려진다.
// 중앙에서 멀어질수록 뒤로 물린다. 순위는 엄격 단조(짝수 장 정중앙 두 장이 동점이 되지 않게 오른쪽에 반 걸음).
...
// 부채꼴 — i 번째 카드의 호 위 위치. lift 는 들림(위로 + 카메라 쪽으로 튀어나옴).
...
// 섞기 흩뿌림 — (카드, 박자) 해시로 결정적인 위치. 박자마다 자리를 바꿔 휘몰아치게.
...
export const scatterPose = (i: number, beat: number): Pose => {
  ...
  return makePose((r1 - 0.5) * 7, 0.5 + r2 * 2.6, local((r3 - 0.5) * 4.5 + 0.4), (r4 - 0.5) * 1.6, r1 * Math.PI * 2, (r2 - 0.5) * 0.9);
};
