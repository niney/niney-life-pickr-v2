...
//
// 실행: pnpm --filter friendly backfill:meal-nutrition [--dry-run]
...
const main = async (): Promise<void> => {
  ...
  const items = await prisma.mealItem.findMany({
    where: { kcal: null },
    select: { id: true, name: true, nameNorm: true, foodId: true, portion: true },
  });
  console.log(`영양이 빈 항목 ${items.length}건`);
  ...
  for (const item of items) {
    ...
    filled += 1;
    console.log(`  ${item.name} ← ${food.name} (${Math.round(food.kcal * f)}kcal${food.nutritionFrom ? `, ${food.nutritionFrom} 기준 추정` : ''})`);
  }
