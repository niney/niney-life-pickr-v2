...
import { CreateSajuPairInput } from '@repo/api-contract';
...
import { calculateSajuPair } from '../src/modules/saju/saju-pair.engine.js';
import {
  requestSajuPairLlm,
  SAJU_PAIR_PROMPT_VERSION,
} from '../src/modules/saju/saju-pair.prompts.js';
...
try {
  ...
  for (const [i, value] of cases.entries()) {
    const input = CreateSajuPairInput.parse(value);
    const chart = calculateSajuPair(input);
    const start = Date.now();
    const result = await requestSajuPairLlm(
    ...
    await writeFile(
      'research/saju/pair-evaluation.json',
      JSON.stringify(
        {
