// 일상지도 적재 상태 한 줄 — deploy.sh 가 파싱한다:
//   "ok cctv=N toilet=M geocoded=G cache=C"   (N/M = 최근 적재 건수, G = 화장실 좌표 확보, C = 지오코딩 캐시 행)
...
const main = async (): Promise<void> => {
  try {
    const [cctv, toilet, cache] = await Promise.all([
    ...
    console.log(`ok cctv=${cctv?.count ?? 0} toilet=${toilet?.count ?? 0} geocoded=${toilet?.geocoded ?? 0} cache=${cache}`);
  } catch (e) {
