...
//   "ok cctv=N toilet=M geocoded=G hospital=H store=S flood=F tour=T tour_jeju=… tour_west=… tour_east=… tour_matched=X cache=C"
//   (N/M/H/S/F = 최근 적재 건수(F = 침수 흔적), G = 화장실 좌표 확보, T = 여행로그 장소 수(unload 뒤 0), tour_<세트> = 데이터셋별 장소 수
...
const main = async (): Promise<void> => {
  try {
    ...
    const [cctv, toilet, hospital, store, flood, tour, tourByDataset, tourMatched, cache] = await Promise.all([
      ...
      prisma.lifeMasterSync.findFirst({ where: { layer: 'flood' }, orderBy: { loadedAt: 'desc' } }),
    ...
    console.log(
      `ok cctv=${cctv?.count ?? 0} toilet=${toilet?.count ?? 0} geocoded=${toilet?.geocoded ?? 0} hospital=${hospital?.count ?? 0} store=${store?.count ?? 0} flood=${flood?.count ?? 0} tour=${tour} ${perDataset} tour_matched=${tourMatched} cache=${cache}`,
    );
  } catch (e) {
