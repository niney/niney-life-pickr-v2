...
const EnvSchema = z.object({
  ...
  // 사주(C)의 OLLAMA_SAJU_MODEL과 별도로 사주(G)의 모델을 설정한다.
  OLLAMA_SAJU_G_MODEL: z.string().default('kimi-k3'),
  ...
  //  - 국립해양조사원 생활해양예보지수 6종(15142484·15142490·15142486·15142489·15142485·15142491)·조석예보(15156018)·
  //    이안류(15156028) — sea 라우트. 각 일 10,000건(활동별 1시간·물때 지점×날짜 12시간 캐시).
  ...
  // 여행로그 썸네일 폴더 — 비우면 data/open/tour/<export>/thumbs(로더 export 폴더). 제주=TOUR_THUMBS_DIR, 그 밖은 TOUR_THUMBS_DIR_<세트 키 대문자>
  // (서부권 _WEST, 동부권 _EAST, 수도권 _CAPITAL — resolveTourThumbsDir). 관리자 인증 라우트로만 서빙.
  TOUR_THUMBS_DIR: z.string().default(''),
  TOUR_THUMBS_DIR_WEST: z.string().default(''),
  TOUR_THUMBS_DIR_EAST: z.string().default(''),
  TOUR_THUMBS_DIR_CAPITAL: z.string().default(''),
