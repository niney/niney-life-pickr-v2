...
describe('parseRecognitionOutput / buildWarning', () => {
  it('코드펜스·앞뒤 설명이 섞여도 첫 JSON 객체를 뽑는다', () => {
    const parsed = parseRecognitionOutput(
...
describe('MealRecognitionService (격리 DB + FakeProvider)', () => {
  ...
  const buildService = (
    ...
    overrides: Partial<MealRecognitionDeps> = {},
  ): MealRecognitionService =>
  ...
  beforeAll(async () => {
    ...
    const jpeg = await sharp({
      create: { width: 30, height: 30, channels: 3, background: { r: 10, g: 200, b: 30 } },
    })
      .jpeg()
      .toBuffer();
  ...
  it('모델 미설정이면 no_provider', async () => {
    ...
    const svc = buildService('', { consumeQuota });
    await expect(svc.recognize({ userId: 'rec-user', photoTokens: [token] })).rejects.toMatchObject(
      { code: 'no_provider' },
    );
    expect(consumeQuota).not.toHaveBeenCalled();
  });

  it('소유권 검증 실패는 쿼터를 소비하지 않고, 검증 성공 후 모델 호출 직전에만 소비한다', async () => {
    const consumeQuota = vi.fn(() => true);
    const svc = buildService('gemma4:31b', { consumeQuota });
    provider.calls = [];

    await expect(
      svc.recognize({ userId: 'someone-else', photoTokens: [token] }),
    ...
    provider.responses = [JSON.stringify({ dishes: [] })];
    await svc.recognize({ userId: 'rec-user', photoTokens: [token] });
    expect(consumeQuota).toHaveBeenCalledTimes(1);
    expect(consumeQuota).toHaveBeenCalledWith('rec-user');
    expect(provider.calls).toHaveLength(1);
  });

  it('쿼터 초과는 모델을 호출하지 않고 quota 오류로 중단한다', async () => {
    const consumeQuota = vi.fn(() => false);
    const svc = buildService('gemma4:31b', { consumeQuota });
    provider.calls = [];
  ...
  it('정상 인식 — 이미지·프롬프트 힌트 전달, 카탈로그 매칭 부착', async () => {
        ...
        dishes: [
          {
            name: '김치 찌개',
            candidates: [
              { name: '김치 찌개', confidence: 0.7 },
              { name: '부대찌개', confidence: 0.2 },
            ],
            confidence: 0.7,
            isMain: true,
            portion: 'large',
    ...
    const res = await buildService().recognize({
      userId: 'rec-user',
      photoTokens: [token],
      placeId: '123',
      slot: 'dinner',
    ...
    expect(res.dishes[0]?.recognitionDishId).toMatch(/^[0-9a-f-]{36}$/);
    expect(res.dishes[0]?.foodId).not.toBeNull();
    // photoIndex 는 사진 수 안으로 클램프.
    expect(res.dishes[1]).toMatchObject({
      name: '콜라',
      isDrink: true,
      photoIndex: 0,
      foodId: null,
    ...
    expect(res.dishes[1]?.recognitionDishId).toMatch(/^[0-9a-f-]{36}$/);
    expect(res.dishes[1]?.recognitionDishId).not.toBe(res.dishes[0]?.recognitionDishId);
  });

  it('저신뢰면 경고, 빈 결과면 안내', async () => {
    provider.responses = [
      JSON.stringify({
        dishes: [{ name: '무언가', confidence: 0.2, isMain: true, photoIndex: 0 }],
      }),
    ];
    expect(
      (await buildService().recognize({ userId: 'rec-user', photoTokens: [token] })).warning,
    ).toContain('확신이 낮');
  ...
  it('퍼지 카탈로그 매칭을 정규화 일치와 구분해 저장한다', async () => {
    provider.responses = [
      JSON.stringify({
        dishes: [
          {
    ...
    const result = await buildService().recognize({
      userId: 'rec-user',
      photoTokens: [token],
    });
    expect(result.dishes[0]).toMatchObject({
      matchedName: '김치찌개',
      catalogMatchedBy: 'fuzzy',
      catalogMatchScore: 0.5,
    });
  });
  ...
  it('JSON 이 깨지면 1회 수리 재시도 — 성공하면 결과, 실패하면 parse_failed', async () => {
    ...
    await expect(
      buildService().recognize({ userId: 'rec-user', photoTokens: [token] }),
    ).rejects.toMatchObject({
      code: 'parse_failed',
    });
  });
  ...
  it('업스트림 실패는 llm_failed, 남의 사진은 forbidden', async () => {
    ...
    await expect(
      buildService().recognize({ userId: 'rec-user', photoTokens: [token] }),
    ).rejects.toBeInstanceOf(MealRecognitionError);

    await expect(
      buildService().recognize({ userId: 'someone-else', photoTokens: [token] }),
    ).rejects.toMatchObject({
      code: 'forbidden',
    });
  });
  ...
  it('작업 로그를 남긴다(성공 1건 = OperationRun 1건)', async () => {
    ...
    const svc = new MealRecognitionService(
      app.prisma,
      new AiConfigService(app.prisma, envBlock('gemma4:31b')),
      {
        photos,
        cache,
    ...
    provider.responses = [
      JSON.stringify({
        dishes: [{ name: '김치찌개', confidence: 0.9, isMain: true, photoIndex: 0 }],
      }),
    ];
    await svc.recognize({ userId: 'rec-user', photoTokens: [token] });
    expect(logged.startRun).toHaveBeenCalledWith(
      expect.objectContaining({ feature: 'meal-recognition', trigger: 'user' }),
    );
    expect(logged.finishRun).toHaveBeenCalledWith(
      'run-1',
      expect.objectContaining({ status: 'done' }),
    );
    // 로그 meta 에 사용자 식별자·음식명이 실리지 않는다.
    const metas =
      JSON.stringify(logged.startRun.mock.calls) + JSON.stringify(logged.finishRun.mock.calls);
