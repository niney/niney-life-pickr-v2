...
import { mealQuota, recognizeQuotaKey } from '../meal/meal-quota.js';
...
// 비용 방어: 라우트 레이트리밋(연타) + per-user 일일 한도(인메모리) + 계정 동시성 게이트.
...
const mealRecognitionRoutes: FastifyPluginAsync = async (app) => {
  ...
  typed.post(Routes.Meal.recognize, {
    ...
    handler: async (req) => {
      ...
      if (!mealQuota.consume(recognizeQuotaKey(userId), env.MEAL_RECOGNIZE_DAILY_LIMIT)) {
