...
// 다이닝코드 일괄 저장 잡의 in-memory 상태. 메뉴 그룹핑 잡 레지스트리와 동형 —
// 단순 batch + AbortController 1개 + per-item progress + 종료. 서버 재시작 시
// in-flight 잡은 사라지지만, 어드민이 다시 실행 가능 (다이닝코드 추가 호출만 발생).

const FINISHED_TTL_MS = 10 * 60_000;
const EVENT_BUFFER_MAX = 1000;

export type BulkSaveJobEvent =
  | { type: 'item'; item: DiningcodeBulkSaveJobItemType }
  | { type: 'done'; state: DiningcodeBulkSaveJobStateType; finishedAt: string };

export type BulkSaveJobSubscriber = (event: BulkSaveJobEvent) => void;
...
interface InternalJob {
  ...
  items: DiningcodeBulkSaveJobItemType[];
  events: BulkSaveJobEvent[];
  subscribers: Set<BulkSaveJobSubscriber>;
...
export class DiningcodeBulkSaveRegistry {
  ...
  create(input: { actorId: string; vRids: string[] }): {
    id: string;
    abortSignal: AbortSignal;
  } {
    const id = randomUUID();
    const startedAt = new Date().toISOString();
    const items: DiningcodeBulkSaveJobItemType[] = input.vRids.map((vRid) => ({
      vRid,
      state: 'pending',
      restaurantId: null,
      fetchedPages: null,
  ...
  markItemStart(jobId: string, vRid: string): void {
    const job = this.jobs.get(jobId);
    if (!job) return;
    const it = job.items.find((i) => i.vRid === vRid);
    if (!it) return;
    it.state = 'running';
  ...
  get(id: string, actorId: string): DiningcodeBulkSaveJobSnapshotType | null {
  ...
  private toPublic(job: InternalJob): DiningcodeBulkSaveJobSnapshotType {
  ...
  private publish(jobId: string, event: BulkSaveJobEvent): void {
    const job = this.jobs.get(jobId);
    if (!job) return;
...
export const diningcodeBulkSaveRegistry = new DiningcodeBulkSaveRegistry();
