import type { CreateSajuPairInputType, SajuPairChartType } from '@repo/api-contract';
import {
  SAJU_ELEMENT_META,
  SAJU_GOD_MEANING,
  sajuElementConnection,
  sajuTenGod,
} from '@repo/utils';
...
export function calculateSajuPair(
  input: Pick<CreateSajuPairInputType, 'first' | 'second'>,
  now = new Date(),
): SajuPairChartType {
  const first = calculateSaju({ birth: input.first, kind: 'natal', note: '' }, now);
  const second = calculateSaju({ birth: input.second, kind: 'natal', note: '' }, now);
  const connection = sajuElementConnection(
    first.dayMaster?.element ?? null,
    second.dayMaster?.element ?? null,
  );
  const firstToSecond = second.dayMaster
    ? sajuTenGod(first.dayMaster?.hanja ?? null, second.dayMaster.hanja)
    : null;
  const secondToFirst = first.dayMaster
    ? sajuTenGod(second.dayMaster?.hanja ?? null, first.dayMaster.hanja)
    : null;
    ...
    return [
      ...
      ...chart.pillars
        ...
        .map((p) => ({
          ...
          description: `${p.pronunciation}(${p.ganZhi}). ${p.stemElement ? SAJU_ELEMENT_META[p.stemElement].meaning : ''}, ${p.branchElement ? SAJU_ELEMENT_META[p.branchElement].meaning : ''}의 상징. 강약이나 실제 성격을 뜻하지 않음.`,
        })),
    ];
  });
