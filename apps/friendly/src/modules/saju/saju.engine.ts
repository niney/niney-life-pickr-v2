...
import type { CreateSajuReadingInputType, SajuBirthType, SajuChartType } from '@repo/api-contract';
import {
  SAJU_BRANCHES,
  SAJU_ELEMENTS,
  SAJU_ELEMENT_META,
...
export const SAJU_CALCULATION_VERSION = 1;
...
export function normalizeSajuDate(
  birth: SajuBirthType,
  now = new Date(),
  ...
  if (!ok)
    throw new SajuInputError('존재하지 않는 날짜이거나 윤달이에요. 생년월일을 확인해 주세요.');
  ...
  if (solarDate < '1900-01-01' || solarDate > today)
    throw new SajuInputError('대한민국에서 1900년부터 오늘까지 태어난 분의 사주를 지원해요.');
...
function birthInstants(date: string, birth: SajuBirthType): Temporal.Instant[] {
  const times =
  ...
  for (const time of times) {
    ...
    for (const disambiguation of choices) {
      try {
        ...
        if (error instanceof SajuInputError) throw error;
        if (birth.timeAccuracy === 'exact')
          throw new SajuInputError(
  ...
  if (!found.size)
    throw new SajuInputError('이 날짜와 시간대를 확인할 수 없어요. 입력을 확인해 주세요.');
  return [...found.values()].sort((a, b) => a.epochMilliseconds - b.epochMilliseconds);
}

function pillarsAt(t: Temporal.Instant, boundary: SajuBirthType['dayBoundary']) {
  ...
  return {
    ...
    day: SAJU_STEMS[stem]!.hanja + SAJU_BRANCHES[branch]!.hanja,
    hour: SAJU_STEMS[((stem % 5) * 2 + hourBranch) % 10]!.hanja + SAJU_BRANCHES[hourBranch]!.hanja,
  };
}
...
>();
export function sajuMonthBoundaries(year: number) {
...
function periodFor(
  kind: CreateSajuReadingInputType['kind'],
  dayStem: string | null,
  boundary: SajuBirthType['dayBoundary'],
  now: Date,
): SajuChartType['period'] {
  ...
  if (kind === 'daily') {
    ...
    return {
      ...
      key: day,
      ganZhi,
      relation: sajuTenGod(dayStem, ganZhi[0]!),
      months: [],
    };
  }
  const terms = sajuMonthBoundaries(today.year);
  const next = sajuMonthBoundaries(today.year + 1)[0]!;
  ...
  const months = terms.map((term, i) => {
    ...
    const relation = sajuTenGod(dayStem, ganZhi[0]!);
    return {
      month: term.month,
      term: term.term,
  ...
  return {
    ...
    ganZhi: yearGanZhi,
    relation: sajuTenGod(dayStem, yearGanZhi[0]!),
    months,
  };
}

export function calculateSaju(input: CreateSajuReadingInputType, now = new Date()): SajuChartType {
  const normalized = normalizeSajuDate(input.birth, now);
  ...
  const pillars = keys.map((key) => {
    ...
    const ganZhi =
      candidates.length === 1 && !(key === 'hour' && input.birth.timeAccuracy === 'unknown')
        ? candidates[0]!
        : null;
    const stem = SAJU_STEMS.find((s) => s.hanja === ganZhi?.[0]);
    const branch = SAJU_BRANCHES.find((s) => s.hanja === ganZhi?.[1]);
    return {
      key,
      label: SAJU_PILLAR_LABEL[key],
      ganZhi,
      pronunciation: ganZhi ? sajuPronunciation(ganZhi) : null,
      stemElement: stem?.element ?? null,
      branchElement: branch?.element ?? null,
      tenGod: key === 'day' ? null : sajuTenGod(dayStem, ganZhi?.[0] ?? ''),
      candidates,
      hiddenStems: [...(branch?.hidden ?? '')].map((char) => {
        const hidden = SAJU_STEMS.find((s) => s.hanja === char)!;
        return {
          hanja: char,
          ko: hidden.ko,
          element: hidden.element,
          tenGod: sajuTenGod(dayStem, char),
        };
      }),
    };
  });
  const elements = SAJU_ELEMENTS.map((element) => ({
    element,
  ...
  const master = SAJU_STEMS.find((s) => s.hanja === dayStem) ?? null;
  ...
  const facts: SajuChartType['facts'] = [
    {
      id: 'scope',
      label: '해석의 범위',
    ...
    ...pillars
      ...
      .map((p) => ({
        ...
        description: `${p.ganZhi}, 천간 ${SAJU_ELEMENT_META[p.stemElement!].name}, 지지 ${SAJU_ELEMENT_META[p.branchElement!].name}${p.tenGod ? `, 일간과의 십성 ${p.tenGod}(${SAJU_GOD_MEANING[p.tenGod]})` : ''}`,
      })),
    ...elements.map((e) => ({
      id: `element-${e.element}`,
      label: `${SAJU_ELEMENT_META[e.element].name}의 구성`,
      description: `확정된 글자 중 ${e.count}개. 전통 상징은 ${SAJU_ELEMENT_META[e.element].meaning}. 개수가 많거나 적다는 사실만으로 좋고 나쁨·용신을 판단하지 않습니다.`,
    })),
  ];
  if (master)
    facts.push({
      id: 'day-master',
      label: `${master.ko}${SAJU_ELEMENT_META[master.element].name} · ${master.symbol}`,
