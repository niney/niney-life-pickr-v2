...
import { CreateSajuPairInput, PublicSajuShare, Routes, SajuPairResult } from '@repo/api-contract';
import { SAJU_ELEMENTS, sajuElementConnection } from '@repo/utils';
...
import { basicSajuPairReport, parseSajuPairReport } from './saju-pair.prompts.js';
import { SajuPairService } from './saju-pair.service.js';

const input = CreateSajuPairInput.parse({
...
describe('궁합의 결정적 근거', () => {
  it.each([
    ...
    expect(sajuElementConnection(a, b)).toBe('first-nurtures');
    expect(sajuElementConnection(b, a)).toBe('second-nurtures');
  });
  it.each([
    ['wood', 'earth'],
    ['earth', 'water'],
    ...
    expect(sajuElementConnection(a, b)).toBe('first-regulates');
    expect(sajuElementConnection(b, a)).toBe('second-regulates');
  });
  it('동일 오행과 미확정을 구분한다', () => {
    for (const e of SAJU_ELEMENTS) expect(sajuElementConnection(e, e)).toBe('same');
    expect(sajuElementConnection(null, 'wood')).toBe('unknown');
    expect(sajuElementConnection('wood', null)).toBe('unknown');
  });
  it('두 사람을 바꾸면 개인 명식과 서로 바라보는 관계도 바뀐다', () => {
    const swapped = calculateSajuPair({ first: input.second, second: input.first });
  ...
  it('미확정 일간으로 상생·상극이나 십성을 만들지 않는다', () => {
    const uncertain = calculateSajuPair({ ...input, first: { ...input.first, dayBoundary: 'zi' } });
    ...
    const report = basicSajuPairReport(uncertain);
    expect(parseSajuPairReport(JSON.stringify(report), uncertain)).not.toBeNull();
    expect(
      parseSajuPairReport(
  ...
  it('없는 근거·점수·계산하지 않은 합·문단 복사를 거절한다', () => {
    const report = basicSajuPairReport(chart);
    expect(parseSajuPairReport(JSON.stringify(report), chart)).not.toBeNull();
    for (const summary of [
      ...
      expect(
        parseSajuPairReport(JSON.stringify({ ...report, summary }), chart, input.note),
      ).toBeNull();
    expect(
      parseSajuPairReport(
        JSON.stringify({
...
describe('프로필·궁합 API와 개인정보 경계', () => {
  ...
  it('프로필 생성·조회·수정·삭제와 계정 소유권을 검증한다', async () => {
    expect((await app.inject({ method: 'GET', url: Routes.Saju.profiles })).statusCode).toBe(401);
    const created = await app.inject({
      method: 'POST',
      url: Routes.Saju.profiles,
      headers: auth(),
    ...
    expect(
      (await app.inject({ method: 'DELETE', url: Routes.Saju.profile(p.id), headers: auth() }))
        .statusCode,
    ).toBe(204);
    expect(
      (await app.inject({ method: 'GET', url: Routes.Saju.profiles, headers: auth() })).json()
        .items,
    ).toEqual([]);
  });
  it('없는 날짜·윤달은 프로필에도 저장하지 않는다', async () => {
    ...
    expect(await app.prisma.sajuProfile.count()).toBe(0);
  });
  it('프로필 상한을 넘으면 기존 항목을 보존한다', async () => {
    await app.prisma.sajuProfile.createMany({
    ...
    expect(await app.prisma.sajuProfile.count()).toBe(20);
  });
  it('궁합 기본 풀이와 공개 공유·PNG·취소를 검증한다', async () => {
    const reading = await app.inject({
      method: 'POST',
      url: Routes.Saju.pairReading,
      payload: input,
    });
    expect(reading.statusCode).toBe(200);
    expect(SajuPairResult.parse(reading.json()).source).toBe('basic');
    const share = (
      await app.inject({
        method: 'POST',
        url: Routes.Saju.shares,
        payload: { pair: { first: input.first, second: input.second } },
      })
    ).json();
    const response = await app.inject({ method: 'GET', url: Routes.Saju.shared(share.token) });
    const shared = PublicSajuShare.parse(response.json());
    ...
    const png = await app.inject({ method: 'GET', url: Routes.Saju.shareImage(share.token) });
    ...
    expect(
      (
        await app.inject({
          method: 'DELETE',
          url: Routes.Saju.shared(share.token),
    ...
    expect(
      (await app.inject({ method: 'GET', url: Routes.Saju.shared(share.token) })).statusCode,
    ).toBe(404);
    expect(
      (await app.inject({ method: 'GET', url: Routes.Saju.shareImage(share.token) })).statusCode,
    ).toBe(404);
  });
  it('같은 소유자의 동시 요청만 합류하고 실패 뒤 즉시 재시도한다', async () => {
    ...
    const complete = vi.fn(async (options: LLMCompleteOptions) => {
      ...
      return {
        text: JSON.stringify(basicSajuPairReport(chart)),
        model: options.model,
        promptTokens: 1,
        completionTokens: 1,
      };
    });
    const service = new SajuPairService(
