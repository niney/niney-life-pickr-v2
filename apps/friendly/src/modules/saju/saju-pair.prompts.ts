...
import { SAJU_RELATIONSHIP_LABEL, sajuPronunciation, thinkOptionForModel } from '@repo/utils';
import { extractFirstJsonObject } from '../../lib/json.js';
import type { LLMProvider } from '../ai/adapters/llm-provider.js';

export const SAJU_PAIR_PROMPT_VERSION = 2;
const interpretationFacts = (chart: SajuPairChartType) =>
  chart.facts.filter((f) => !/^(first|second)-(year|month|day|hour)$/.test(f.id));
const ids = ['connection', 'differences', 'conversation'];
export function basicSajuPairReport(chart: SajuPairChartType): SajuReportType {
...
export function parseSajuPairReport(
  text: string,
  chart: SajuPairChartType,
  note = '',
): SajuReportType | null {
  try {
    ...
    if (
      ...
      return null;
    const allowedKo = new Set([...allowed].map(sajuPronunciation));
    if (
      [
        ...prose.matchAll(
...
export async function requestSajuPairLlm(
  provider: LLMProvider,
  model: string,
  chart: SajuPairChartType,
  relationship: SajuRelationshipType,
  note: string,
  signal?: AbortSignal,
) {
  const prompt = JSON.stringify({
    relationship: SAJU_RELATIONSHIP_LABEL[relationship],
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const report = parseSajuPairReport(response.text, chart, note);
