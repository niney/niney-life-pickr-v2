...
import { CreateSajuGReadingInput, SajuGChart } from '@repo/api-contract';
import { calculateSajuG, sajuGMonthBoundaries } from './saju-g.engine.js';

const now = new Date('2026-09-06T03:00:00Z');
const calc = (birth: Record<string, unknown>, kind = 'natal') =>
  calculateSajuG(CreateSajuGReadingInput.parse({ birth, kind }), now);
...
describe('한국 역법과 시간 기준', () => {
  it('KASI 2026-08-18 일진 갑자와 월주·시주', () => {
    ...
    expect(SajuGChart.safeParse(v).success).toBe(true);
  });
  ...
  it('KASI 월력요항 2026 입춘 05:02 KST 전후에만 연월이 바뀐다', () => {
    ...
    expect(sajuGMonthBoundaries(2026)[1]!.instant.toString()).toMatch(/^2026-02-03T20:02:/);
    expect(
  ...
  it('올해는 열두 절입 구간, 오늘은 열람 시각과 무관한 일진', () => {
    ...
    const input = CreateSajuGReadingInput.parse({ birth: { date: '1990-05-21' }, kind: 'daily' });
    expect(calculateSajuG(input, new Date('2026-09-06T00:00:00+09:00')).period).toEqual(
      calculateSajuG(input, new Date('2026-09-06T23:59:00+09:00')).period,
    );
  });
});
