...
import { calculateSajuG, sajuGMonthBoundaries, standardOffsetSeconds } from './saju-g.engine.js';
...
describe('한국천문연구원 외부 기준 대조', () => {
  it.each(reference.lunarDates)('$lunarDate 윤달=$leapMonth → $solarDate / $dayGanZhiKo', (row) => {
    const chart = calculateSajuG(
      CreateSajuGReadingInput.parse({
        birth: {
          date: row.lunarDate,
          calendar: 'lunar',
  ...
  it.each(reference.terms)('$year $term 절입은 KASI 분 단위 표에서 60초 이내', (row) => {
    const actual = sajuGMonthBoundaries(row.year).find((t) => t.term === row.term)!;
