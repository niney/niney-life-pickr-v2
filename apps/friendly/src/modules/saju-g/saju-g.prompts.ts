import { SajuGReport, type SajuGChartType, type SajuGReportType } from '@repo/api-contract';
import {
  SAJU_G_ELEMENT_META,
  SAJU_GOD_MEANING,
  SAJU_G_KIND_LABEL,
...
export function basicSajuGReport(chart: SajuGChartType): SajuGReportType {
  ...
  const distribution = chart.elements
    .map((e) => `${SAJU_G_ELEMENT_META[e.element].name} ${e.count}개`)
    .join(' · ');
  ...
  return {
    ...
    sections: sajuGSectionIds(chart.period.kind).map((id, i) => ({
      id,
      title: titles[i]!,
      text: texts[i]!,
...
export function parseSajuGReport(
  text: string,
  ...
  try {
    ...
    const ids = sajuGSectionIds(chart.period.kind);
    ...
    const allowedKorean = new Set([...allowed].map(sajuGPronunciation));
    ...
    const masterNames = SAJU_G_STEMS.map((s) => s.ko + SAJU_G_ELEMENT_META[s.element].name).join(
      '|',
    );
...
export async function requestSajuGLlm(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const report = parseSajuGReport(response.text, chart, note);
