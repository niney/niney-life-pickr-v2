...
export function SajuGBirthForm({
  initial,
  onSubmit,
  mode = 'reading',
  heading,
  ...
  initial?: CreateSajuGReadingInputType;
  onSubmit: (input: CreateSajuGReadingInputType) => void;
  mode?: 'reading' | 'profile' | 'pair';
  heading?: string;
  submitLabel?: string;
  disabled?: boolean;
}) {
  ...
  const [accuracy, setAccuracy] = useState<SajuGBirthType['timeAccuracy']>(
  ...
  const [boundary, setBoundary] = useState<SajuGBirthType['dayBoundary']>(
    initial?.birth.dayBoundary ?? 'midnight',
  );
  const [disambiguation, setDisambiguation] = useState<SajuGBirthType['disambiguation']>(
  ...
  const submit = (event: FormEvent) => {
    ...
    const result = CreateSajuGReadingInput.safeParse({
      birth: {
        date,
        calendar,
  ...
  return (
    <form className="saju-g-birth-form" onSubmit={submit}>
      <div className="saju-g-form-heading">
        <span className="saju-g-eyebrow">YOUR BEGINNING</span>
        <h2>{heading ?? '당신의 시작을 알려주세요'}</h2>
        <p>
          {mode === 'reading'
            ? '태어난 순간의 네 기둥을 펼쳐 볼게요.'
            : mode === 'profile'
              ? '한 번 입력해 두고, 이야기가 필요할 때 다시 꺼내 보세요.'
              : '두 사람의 이야기에 사용할 출생정보를 알려주세요.'}
        </p>
      </div>
      <div className="saju-g-label-row">
        <label htmlFor={`${formId}-date`}>생년월일</label>
        <div className="saju-g-segment" aria-label="달력 종류">
      ...
      />
      <p id={`${formId}-date-hint`} className="saju-g-field-hint">
        대한민국 출생 · 1900년부터 지원해요.
        {calendar === 'lunar' && ' 음력 날짜를 YYYY-MM-DD 형식으로 입력해 주세요.'}
      </p>
      {calendar === 'lunar' && (
        <label className="saju-g-check">
          <input type="checkbox" checked={leap} onChange={(e) => setLeap(e.target.checked)} />
          음력 윤달에 태어났어요
        </label>
      )}
      <label htmlFor={`${formId}-accuracy`} className="saju-g-field-label">
        태어난 시간
      </label>
      <div className="saju-g-time-row">
        <select
          id={`${formId}-accuracy`}
          value={accuracy}
          onChange={(e) => setAccuracy(e.target.value as SajuGBirthType['timeAccuracy'])}
        >
        ...
        {accuracy === 'range' && (
          <select
            aria-label="대략의 시간대"
            value={range}
            onChange={(e) => setRange(e.target.value as NonNullable<SajuGBirthType['timeRange']>)}
          >
      ...
      <p className="saju-g-field-hint">시간을 몰라도 괜찮아요. 알 수 있는 부분부터 읽어 드려요.</p>
      <details className="saju-g-form-details">
        <summary>
          {mode === 'reading' ? '관심사와 계산 기준' : '계산 기준'} <ChevronDown size={14} />
        </summary>
        {mode === 'reading' && (
          <>
        ...
        <label htmlFor={`${formId}-boundary`}>하루가 바뀌는 기준</label>
        <select
          id={`${formId}-boundary`}
          value={boundary}
          onChange={(e) => setBoundary(e.target.value as SajuGBirthType['dayBoundary'])}
        >
          <option value="midnight">자정 00시 (기본)</option>
          <option value="zi">자시 23시</option>
        </select>
        <label htmlFor={`${formId}-disambiguation`}>과거 시계 변경으로 겹치는 시각</label>
        <select
          id={`${formId}-disambiguation`}
          value={disambiguation}
          onChange={(e) => setDisambiguation(e.target.value as SajuGBirthType['disambiguation'])}
        >
      ...
      <button className="saju-g-primary" type="submit" disabled={disabled}>
        {submitLabel ?? '나의 사주(G) 펼치기'} <ArrowUpRight size={19} />
      </button>
      {mode === 'reading' && (
