...
//
// 추가 동작:
//   - viewKey prop 으로 뷰별 스크롤 메모리 — list↔detail 전환 시 자동 저장/복원.
...
interface Props {
  ...
  topOffset?: number;
  // 컨텐츠 식별자. 바뀌면 이전 viewKey 의 스크롤을 저장하고 새 viewKey 의
  // 저장된 스크롤(없으면 0=top)로 복원. 예: list↔detail 전환.
  viewKey?: string;
}
...
export const BottomSheet = ({
  ...
  // viewKey 별 스크롤 메모리. viewKey 가 바뀌면 이전 키의 스크롤을 저장하고
  // 새 키의 저장된 스크롤(없으면 0)로 scrollPosRef 를 교체.
  const scrollByViewKeyRef = useRef<Record<string, number>>({});
  ...
  // unmount 시 body overflow 정리 (혹시 모를 보호)
  useEffect(() => {
    return () => {
  ...
  // mode·viewKey 변화에 따라 (a) html overflow 잠금/해제 (b) 스크롤 위치 적용.
  // viewKey 가 바뀌면 이전 뷰의 스크롤을 메모리에 저장하고, 새 뷰의 저장된
  // 스크롤(없으면 0)을 scrollPosRef 로 가져와 적용.
  useLayoutEffect(() => {
    const isViewChange = prevViewKeyRef.current !== viewKey;
    if (isViewChange) {
      const outgoing =
        mode === 'scroll'
          ? window.scrollY
          : innerContentRef.current?.scrollTop ?? 0;
      scrollByViewKeyRef.current[prevViewKeyRef.current] = outgoing;
      scrollPosRef.current = scrollByViewKeyRef.current[viewKey] ?? 0;
  ...
  useLayoutEffect(() => {
    ...
    if (snap === 'full' && mode === 'fixed') {
      const t = window.setTimeout(() => {
        if (snapRef.current === 'full') setMode('scroll');
      }, TRANSITION_MS);
  ...
  return (
    <div
      ...
      style={{ minHeight: `calc(100dvh - ${topOffset}px)` }}
    >
      <div
