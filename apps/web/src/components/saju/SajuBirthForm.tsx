...
export function SajuBirthForm({
  ...
  return (
    <form className="saju-birth-form" onSubmit={submit}>
      ...
      />
      <p id={`${formId}-date-hint`} className="saju-field-hint">
      ...
      <label htmlFor={`${formId}-accuracy`} className="saju-field-label">
        태어난 시간
      </label>
      ...
      <button className="saju-primary" type="submit" disabled={disabled}>
        {submitLabel ?? '나의 사주 펼치기'} <ArrowUpRight size={19} />
      </button>
      {mode === 'reading' && (
