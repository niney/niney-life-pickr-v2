...
export function SajuShareDialog({
  ...
  const create = async () => {
    ...
    try {
      ...
      setShare(value);
      if (!sajuShareCredential.set(value.token, value.revokeToken))
        setMessage(
  ...
  const revoke = async () => {
    ...
    setBusy(true);
    try {
      await sajuApi.revokeShare(share.token, share.revokeToken);
      sajuShareCredential.remove(share.token);
      setShare(null);
  ...
  const download = async () => {
    ...
    setBusy(true);
    try {
      const response = await fetch(sajuApi.imageUrl(share.token));
  ...
  return (
    <dialog
      ...
      aria-labelledby="saju-share-title"
    >
      <div className="saju-dialog-body">
        <button className="saju-dialog-close" onClick={onClose} aria-label="공유 창 닫기">
          <X size={20} />
        </button>
        ...
        {share ? (
          <>
            ...
            />
            <div className="saju-share-actions">
              <button onClick={copy}>
                <Copy size={15} />
                링크 복사
              </button>
            ...
            <button className="saju-text-button" disabled={busy} onClick={revoke}>
              <Link2Off size={14} />
              공유 링크 취소
            </button>
          ...
          <button className="saju-primary" onClick={create} disabled={busy}>
            {busy ? '공유 카드 준비 중…' : '이 내용으로 공유 링크 만들기'}
          </button>
        )}
        {message && (
          <p role="status" className="saju-action-message">
            {message}
          </p>
        )}
      </div>
