...
import type { SajuChartType } from '@repo/api-contract';
import { SAJU_ELEMENT_META, type SajuElementId } from '@repo/utils';
import { detectTarotRender } from '../tarot/tarotQuality';
const Stage = lazy(() => import('./SajuStage'));
...
export function SajuSymbol({
  element,
  className = '',
}: {
  element: SajuElementId | null;
  className?: string;
}) {
  return (
    <svg
...
export function SajuVisual({
  chart,
  active,
  revealing,
}: {
  chart: SajuChartType | null;
  active: SajuElementId | null;
  revealing: boolean;
}) {
  ...
  return (
    <div
      className={`saju-visual ${revealing && motion ? 'is-revealing' : ''}`}
      ref={ref}
      data-render-mode={motion ? '3d' : 'lite'}
      aria-hidden="true"
      style={{ color: element ? SAJU_ELEMENT_META[element].color : '#a8c5ae' }}
    >
      <div className="saju-orbit-fallback" />
      <div className="saju-orbit-fallback inner" />
      {motion && (
        <div className="saju-canvas">
          <StageBoundary>
      ...
      <div className="saju-symbol-center">
        <SajuSymbol element={element} />
        <span>
          {active
            ? `${SAJU_ELEMENT_META[active].name} · ${SAJU_ELEMENT_META[active].meaning}`
            : chart?.dayMaster
              ? `${chart.dayMaster.ko}${SAJU_ELEMENT_META[chart.dayMaster.element].name}`
              : '木 · 火 · 土 · 金 · 水'}
        </span>
      </div>
      <span className="saju-orbit-label top">하늘의 시간</span>
      <span className="saju-orbit-label bottom">나만의 결을 발견하는 순간</span>
      <span className="saju-star s1">✦</span>
      <span className="saju-star s2">✧</span>
      <span className="saju-star s3">·</span>
    </div>
  );
}
