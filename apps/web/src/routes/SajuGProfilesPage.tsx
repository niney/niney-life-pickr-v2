...
import { useAuthStore, useSajuGProfiles } from '@repo/shared';
import { SAJU_G_PROFILES_MAX, SajuGProfileInput, type SajuGProfileType } from '@repo/api-contract';
...
export function SajuGProfilesPage() {
  const principal = useAuthStore((s) => s.user?.id ?? 'guest');
  return <Profiles key={principal} />;
}
function Profiles() {
  const profiles = useSajuGProfiles();
  const [editing, setEditing] = useState<SajuGProfileType | null | undefined>();
  const [name, setName] = useState('');
  const [deleting, setDeleting] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState('');
  const edit = (value: SajuGProfileType | null) => {
    setEditing(value);
    setName(value?.name ?? '');
    setMessage('');
  };
      ...
      <div className="saju-g-shell">
        <header className="saju-g-nav">
          ...
          <div className="saju-g-next-nav">
            ...
            <Link to="/me/saju-g">보관함</Link>
          </div>
        </header>
        <main className="saju-g-profiles-main">
          <span className="saju-g-eyebrow">PEOPLE IN YOUR UNIVERSE</span>
          <h1>
            기억해 두고 싶은
            <br />
            <em>우리의 시작</em>
          ...
          <p className="saju-g-field-hint">
            {profiles.principal === 'guest'
              ...
              : '내 계정에 보관돼요.'}{' '}
            최대 {SAJU_G_PROFILES_MAX}명 · {profiles.profiles.length}명 보관 중
          </p>
          ...
          {editing !== undefined ? (
            <section className="saju-g-profile-editor" aria-label="프로필 편집">
              <div className="saju-g-profile-editor-top">
              ...
              <SajuGBirthForm
                ...
                onSubmit={async (input) => {
                  const parsed = SajuGProfileInput.safeParse({ name, birth: input.birth });
                  if (!parsed.success) {
            ...
            <button
              className="saju-g-primary saju-g-profile-add"
              disabled={
                profiles.profiles.length >= SAJU_G_PROFILES_MAX ||
                profiles.isLoading ||
                profiles.isError
              }
              onClick={() => edit(null)}
          ...
          <div className="saju-g-profile-grid">
            {profiles.profiles.map((p, i) => (
              <article key={p.id} className="saju-g-profile-card">
                <div className="saju-g-profile-card-top">
                  <span className="saju-g-profile-seal">{p.name.slice(0, 1)}</span>
                  <span className="saju-g-eyebrow">PERSON {String(i + 1).padStart(2, '0')}</span>
                </div>
                <h2>{p.name}</h2>
                <p>
                ...
                <div className="saju-g-profile-quick">
                  {(['natal', 'daily', 'annual'] as const).map((kind, n) => (
                    <Link
                      key={kind}
                      to={`/saju-g?profile=${encodeURIComponent(p.id)}&kind=${kind}`}
                    >
                      {['나의 사주(G)', '오늘', '올해'][n]}
                ...
                <Link
                  className="saju-g-profile-pair"
                  to={`/saju-g/pair?first=${encodeURIComponent(p.id)}`}
                >
                  <Users size={15} />이 사람과 궁합 보기
                </Link>
                <div className="saju-g-profile-edit-actions">
                ...
                {deleting === p.id && (
                  <div className="saju-g-profile-delete">
                    <p>
