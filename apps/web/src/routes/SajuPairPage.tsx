...
import {
  ...
  type SajuBirthType,
  type SajuRelationshipType,
} from '@repo/api-contract';
import { useAuthStore, useSajuProfiles, useSajuPairReading } from '@repo/shared';
import { SAJU_RELATIONSHIP_LABEL } from '@repo/utils';
...
function BirthDialog({
  ...
  side: number;
  initial: SajuBirthType | null;
  onChoose: (birth: SajuBirthType) => void;
  onClose: () => void;
}) {
  ...
  return (
    <dialog
      ref={dialog}
      className="saju-share-dialog saju-person-dialog"
      onCancel={onClose}
    ...
    >
      <div className="saju-dialog-body">
        <button className="saju-dialog-close" aria-label="입력 창 닫기" onClick={onClose}>
          <X size={20} />
        </button>
        <SajuBirthForm
          mode="pair"
...
function Pair() {
  ...
  const reading = useSajuPairReading();
  const resetReading = reading.reset;
  const [params] = useSearchParams();
  const firstParam = params.get('first');
  const loaded = useRef('');
  const [selected, setSelected] = useState(['', '']);
  const [manual, setManual] = useState<[SajuBirthType | null, SajuBirthType | null]>([null, null]);
  const [editing, setEditing] = useState<number | null>(null);
  const [relationship, setRelationship] = useState<SajuRelationshipType>('partner');
  const [note, setNote] = useState('');
  const [input, setInput] = useState<CreateSajuPairInputType | null>(null);
  ...
  const start = () => {
    const parsed = CreateSajuPairInput.safeParse({
      ...
      <div className="saju-shell">
        <header className="saju-nav">
          ...
          <div className="saju-next-nav">
            ...
            <Link to="/saju">나의 사주</Link>
          </div>
        </header>
        {!active ? (
          <main className="saju-pair-landing">
            <span className="saju-eyebrow">TWO WORLDS, ONE CONVERSATION</span>
            <h1>
              서로 다른 두 우주가
              <br />
              <em>마주하는 순간</em>
            ...
            <SajuPairVisual />
            <section className="saju-pair-input" aria-label="궁합 입력">
              <div className="saju-pair-people">
                {people.map((person, i) => (
                  <article key={i} className="saju-pair-person">
                    <span className="saju-eyebrow">{i ? 'THE OTHER WORLD' : 'ONE WORLD'}</span>
                    <h2>{person.name}</h2>
                    <label className="saju-field-label" htmlFor={`pair-person-${i}`}>
                      {i + 1}번째 사람 선택
                    </label>
                    <select
                      id={`pair-person-${i}`}
                    ...
                    {person.birth ? (
                      ...
                      <p className="saju-field-hint">아직 출생정보를 선택하지 않았어요.</p>
                    )}
                    {!selected[i] && (
                      <button className="saju-secondary" onClick={() => setEditing(i)}>
              ...
              {profiles.isError && (
                <p className="saju-error">
                  프로필을 불러오지 못했어요.{' '}
                  <button onClick={() => void profiles.refetch()}>다시 불러오기</button>
                </p>
              )}
              <fieldset className="saju-relationship">
                <legend>어떤 사이인가요?</legend>
                <div>
                  {(Object.keys(SAJU_RELATIONSHIP_LABEL) as SajuRelationshipType[]).map((r) => (
                    <button
                      key={r}
                      type="button"
                    ...
                    >
                      {SAJU_RELATIONSHIP_LABEL[r]}
                    </button>
                  ))}
                </div>
              </fieldset>
              <label className="saju-field-label" htmlFor="pair-note">
              ...
              <button
                className="saju-primary saju-pair-submit"
          ...
          <main className="saju-pair-result" ref={resultTop}>
            <button className="saju-text-button" onClick={reading.reset}>
              <ArrowLeft size={15} />두 사람 다시 선택
            </button>
            <div className="saju-pair-result-heading">
              <span className="saju-eyebrow">
                OUR CONSTELLATION · {SAJU_RELATIONSHIP_LABEL[relationship]}
              </span>
            ...
            />
            {reading.chart && (
              <div className="saju-pair-basis">
                <span>두 일간의 관계</span>
                <p>{reading.chart.description}</p>
              </div>
            )}
            {reading.pending && (
              <div role="status" className="saju-pair-loading">
                <span className="saju-pair-loading-star">✦</span>
                <h2>우리의 이야기를 읽고 있어요</h2>
                <p>닮은 부분과 다른 속도, 함께 나눌 대화를 차분히 살펴보고 있어요.</p>
              </div>
            )}
            {reading.error && (
              <div role="alert" className="saju-error">
                {reading.error}
            ...
            {reading.result && (
              <>
                ...
                <section className="saju-pair-story">
                  <span className="saju-eyebrow">A STORY BETWEEN US</span>
                  <h2>{reading.result.report.headline}</h2>
                  <p>{reading.result.report.summary}</p>
                </section>
                <div className="saju-pair-chapters">
                ...
                <div className="saju-pair-practice">
                  <span>오늘, 우리 둘이</span>
                  <h2>{reading.result.report.reflection}</h2>
                  <p>{reading.result.report.practice}</p>
                </div>
                <div className="saju-result-actions">
                  <button className="saju-primary" onClick={() => setShareKey(selectionKey)}>
                    <Share2 size={17} />
                    우리의 오행 지도 공유
                  </button>
                  <Link to="/me/saju/profiles" className="saju-secondary">
                    프로필 관리
                  </Link>
                </div>
            ...
            {reading.chart && (
              <details className="saju-pair-notices">
